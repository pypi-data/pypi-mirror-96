...
class Formats():
	...
	class FilePath(object):
		...
		def size(self, mode="auto", options=["auto", "bytes", "kb", "mb", "gb", "tb"], type=str, path=None):
			...
			if type in [int, "int", "integer", "Integer", Integer]:
		...
		def replace(self, from_, to_):
			return Formats.FilePath(self.path.replace(str(from_), str(to_)))
		def lower(self, path):
			return Formats.FilePath(self.path.lower(str(path)))
		def upper(self, path):
			return Formats.FilePath(self.path.upper(str(path)))
		# support subscriptionable.
		def __getitem__(self, index):
			return Formats.String(self.path[Formats.denitialize(index)])
		...
		def __add__(self, path):
			...
			return Formats.String(self.path + path)
		def __sub__(self, path):
			if isinstance(path, str):
				...
				raise exceptions.FormatError(f"Can not add object {self.__class__} & {path.__class__}.")
			return Formats.String(self.path.replace(path, ""))
		# support +.
	...
	class String(str):
			...
			for line in string.split("\n"):
				...
				if (not opened and line_indent == indent) or (opened and line_indent >= indent):
					if remove_indent:
						...
						s += line+"\n"
					opened = True
				elif len(line) > 0 and opened and line_indent < indent:
					#s += line+"\n"
					break
			return Formats.String(s)
		# support default str functions.
		...
		def replace(self, from_, to_):
			return Formats.String(self.string.replace(str(from_), str(to_)))
		def lower(self, string):
			return Formats.String(self.string.lower(str(string)))
		def upper(self, string):
			return Formats.String(self.string.upper(str(string)))
		# support "+" & "-" .
		def __add__(self, string):
			...
			return Formats.String(self.string + string)
		...
		def __sub__(self, string):
			if isinstance(string, str):
				...
				raise exceptions.FormatError(f"Can not add object {self.__class__} & {string.__class__}.")
			return Formats.String(self.string.replace(string, ""))
		...
		# support subscriptionable.
		def __getitem__(self, index):
			return Formats.String(self.string[Formats.denitialize(index)])
		def __setitem__(self, index, value):
...
class Files():
	...
	def save(path, data, format="str", indent=4, ensure_ascii=False):
		# correct format.
		if format in [str, String, "String", "string", "file"]: format = "str"
		if format in [dict, Dictionary, "Dictionary", "dict", "array"]: format = "json"
		if format in [bytes, Bytes, "Bytes"]: format = "bytes"
	...
	class Array(object):
		def __init__(self, 
			...
			if load: self.load()
		def save(self, array=None, path=None, ensure_ascii=False, indent=4):
			if array != None: array = self.array
			if path == None: path = self.file_path.path
			utils.__check_memory_only__(path)
			self.array = array
			return Files.save(path, array, format="json", indent=indent, ensure_ascii=ensure_ascii)
		def load(self, default=None):
			utils.__check_memory_only__(self.file_path.path)
			if not os.path.exists(self.file_path.path) and default != None: 
				self.save(default)
		...
		def __add__(self, array):
			...
			return Files.Array(self.array + array)
		...
		def __getitem__(self, index):
			...
			if isinstance(index, slice):
				return Files.Array(self.array[Formats.denitialize(index)])
			else:
				v = self.array[Formats.denitialize(index)]
				if isinstance(v, list): return Files.Array(v)
				return v
		...
		# str representation.
		def __str__(self):
			return str(Formats.denitialize(self))
		# content count.
		def __len__(self):
	...
	class Dictionary(object):
		...
		def save(self, dictionary=None, path=None, ensure_ascii=False, indent=4):
			...
			if path == None: path = self.file_path.path
			self.dictionary = dictionary
			return Files.save(path, dictionary, format="json", indent=indent, ensure_ascii=ensure_ascii)
		def load(self, default=None):
			utils.__check_memory_only__(self.file_path.path)
			if not os.path.exists(self.file_path.path) and default != None: 
				self.save(default)
		...
		def divide(self, into=2):
			...
			for k,v in self.dictionary.items():
				...
				if idx < into-1:  # indexes start at 0
					...
					idx = 0
			return Files.Array(return_list)
		def append(self, 
			...
			if save: 
				...
				self.save()
			return Files.Dictionary(dictionary_)
		# unpack attribute(s).
		def unpack(self, 
		...
		def remove(self, keys=[], values=[], update=True, save=False, dictionary=None):
			...
			if save: 
				self.dictionary = dictionary
				self.save()
			return Files.Dictionary(dictionary)
		...
		def keys(self, sorted=False, reversed=False, dictionary=None):
			...
			if reversed: 
				...
				for _ in range(len(keys)):
					...
					c -= 1
				return Files.Array(reversed_keys)
			else: return Files.Array(list(dictionary.keys()))
		...
		def reversed(self, update=True, dictionary=None):
			...
			return Files.Dictionary(reversed_dict)
		def sort(self, alphabetical=True, ascending=False, reversed=False, update=True, dictionary=None):
			if dictionary == None: dictionary = self.dictionary
			new = {}
			if alphabetical or ascending:
				_sorted_ = Files.Array(path=False, array=list(dictionary.keys())).sort(alphabetical=alphabetical, ascending=ascending, reversed=reversed)
			else: raise ValueError("Unknown behaviour, alphabetical=False.")
			for key in _sorted_:
				new[Formats.denitialize(key)] = dictionary[Formats.denitialize(key)]
			if update:
				self.dictionary = new
			return Files.Dictionary(new)
		# dump json string.
		...
		def __eq__(self, dictionary):
			if isinstance(dictionary, dict):
				return str(self.sort()) == str(Files.Dictionary(dictionary).sort())
			elif not isinstance(dictionary, self.__class__):
				return False
	...
	class Bytes(object):
		...
		def save(self, bytes=None):
			...
			self.bytes = bytes
			return Files.save(self.fp.path, bytes, format="bytes")
...
class Code():
	...
	class Script(Formats.String,Files.File):
		def __init__(self, 
			...
			if isinstance(data, (Files.File, Code.Script)):
				data = data.data
			if data == None: data = ""
			Formats.String.__init__(self, data,)
	...
	class Python(Formats.String,Files.File):
		def __init__(self, 
			# the script data (str) (param #1).
			data=None, 
			# the path (str) (param #2).
		...
		def slice_classes(self,
			# the data (str).
			data=None, 
			# use before="\n" for the no indent classes & before=" " for the indent classes.
			indent=False
		):
			if data == None:
				data = self.data
			...
			for class_ in slices:
				class_ = Formats.String(class_)
				class_line = class_.split("\n")[0]
				class_name = class_.split("(")[0]
				class_indent = class_.line_indent(line=class_.split("\n")[1])
				class_code = class_.slice_indent(indent=class_indent, depth=1, remove_indent=not indent)
				l_classes, functions = Files.Array(), Files.Array()
				if ("\nclass " in class_code or " class " in class_code) and class_code != data and len(class_code) > 0:
					l_classes = self.slice_classes(data=class_code, indent=indent)
					for i in l_classes:
						class_code = class_code.replace(i["init_line"]+"\n"+i["code"], "")
				if ("\ndef " in class_code or " def " in class_code) and class_code != data and len(class_code) > 0:
					functions = self.slice_functions(data=class_code, indent=indent)
					for i in functions:
						class_code = class_code.replace(i["init_line"]+"\n"+i["code"], "")
				classes.append({
					"name":class_name,
					"code":class_code,
					"indent":class_indent,
					"init_line":Formats.String("class ")+class_line,
					"classes":l_classes,
					"functions":functions,
					"type":"class",
				})
		...
		def slice_functions(self,
			...
			if not indent:
				...
				before = " "
			data = data.replace("	","    ")
			functions = Files.Array()
			slices = data.split(f"{before}def ")
			if len(slices) > 1:
				slices = slices[1:]
			for class_ in slices:
				class_ = Formats.String(class_)
				...
				class_indent = class_.line_indent(line=class_.split("\n")[1])
				class_code = class_.slice_indent(indent=class_indent, depth=1, remove_indent=not indent)
				class_parameters = class_.slice_tuple(depth=1)
				classes, l_functions  = Files.Array(), Files.Array()
				if ("\nclass " in class_code or " class " in class_code) and class_code != data and len(class_code) > 0:
					classes = self.slice_classes(data=class_code, indent=indent)
					for i in classes:
						class_code = class_code.replace(i["init_line"]+"\n"+i["code"], "")
				if ("\ndef " in class_code or " def " in class_code) and class_code != data and len(class_code) > 0:
					l_functions = self.slice_functions(data=class_code, indent=indent)
				...
				functions.append({
					...
					"code":class_code,
					"indent":class_indent,
					"init_line":Formats.String("def ")+class_line,
