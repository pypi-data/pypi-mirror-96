		...
		def variable_format(self, 
			...
			for i in self.string:
				...
				try:
					...
					p = "none"
				if c > 0 and i.lower() != i and str(n).lower() == str(n) and str(p).lower() == str(p):
					s += "_"
				s += i.lower()
				c += 1
...
class Files():
	...
	class Dictionary(object):
		...
		def __eq__(self, dictionary):
			...
			elif dictionary.__class__.__name__ == "ResponseObject":
				return str(self.sort()) == str(dictionary.sort())
			else:
				return False
			#if isinstance(dictionary, dict):
	...
	class Python(Files.File):
						...
						if class_name not in banned_classes:
							...
							if inside_classes:
								while True:
									if ("\nclass " in class_code or " class " in class_code or (len(class_code) >= len("class ") and class_code[:len("class ")] == "class ")) and class_code != data and len(class_code) > 0:
										...
										#quit()
										classes += self.slice_classes(data=full_inside_class, keep_indent=keep_indent, inside_classes=inside_classes, inside_functions=True, system_functions=system_functions, banned_functions=banned_functions, banned_classes=banned_classes)
							...
							if inside_functions:
								if ("\ndef " in class_code or " def " in class_code or (len(class_code) >= len("def ") and class_code[:len("def ")] == "def ")) and class_code != data and len(class_code) > 0:
									functions = []
									for i in self.slice_functions(data=class_code, keep_indent=keep_indent, inside_classes=False, inside_functions=False, system_functions=system_functions, banned_functions=banned_functions, banned_classes=banned_classes):
										skip = False
										old = str(class_code)
										class_code = class_code.replace("):  \n","):\n").replace("):   \n","):\n").replace("):    \n","):\n").replace("):     \n","):\n").replace("):	\n","):\n").replace(i["raw_full_code"], "")
										if class_code == old:
											if skip_errors:
												skip = True
											else:
												raise ValueError(f"Replaced class code is the same as old class code, replacement: [{i['raw_full_code']}], code: [{class_code}].")
										if i["classes"] != {}:
											classes += i["classes"]
										del i["classes"]
										if not skip:
		...
		def slice_functions(self,
			...
			def parse_variable_format(variable, code):
				...
				return return_
				#
			def func_allowed(name):
				if "." in name: name = name.split(".")[len(name.split("."))-1]
			...
			for class_ in slices:
				if class_ not in [""," "]:
					...
					if indent == None or str(init_line_indent) == str(int(indent)):
						...
						if func_allowed(class_name):
							for attempt in range(100):
								try:
									class_indent = class_.line_indent(line=class_.split("\n")[attempt+1])
									if class_indent != 0: break
								except IndexError:
									raise IndexError(f"Unable to determine the class indent of code: [{class_}].")
							...
							if inside_classes and (("\nclass " in class_code or " class " in class_code) and class_code != data and len(class_code) > 0):
								classes = self.slice_classes(data=class_code, keep_indent=keep_indent, inside_functions=False, inside_classes=inside_classes, system_functions=system_functions, banned_functions=banned_functions, banned_classes=banned_classes)
								for i in classes:
							...
							if inside_functions and (("\ndef " in class_code or " def " in class_code) and class_code != data and len(class_code) > 0):
								l_functions = self.slice_functions(data=class_code, keep_indent=keep_indent, inside_classes=False, inside_functions=False, system_functions=system_functions, banned_functions=banned_functions, banned_classes=banned_classes)
		...
		def build_code_examples(self,
			...
			def build_class_doc(info):
				...
				initialized_name = String(info["name"]).variable_format()
				# fill traceback name.
				...
				for i in info["functions"]:
					if "__init__" not in i["name"]:
						...
						i["name"] = f'{initialized_name}.{i["name"]}'
						if i["type"] == "property":
							properties.append(i)
						else:
							if c == 0:
			...
			for _path_ in path:
				...
				first_indent_functions = self.slice_functions(data=data, indent=0, system_functions=system_functions, banned_functions=banned_functions, banned_classes=banned_classes)
				classes = self.slice_classes(data=data, system_functions=system_functions, banned_functions=banned_functions, banned_classes=banned_classes)
		...
		def build_readme(self,
			...
			# code examples.
			code_examples = self.build_code_examples(format=dict, readme=True, system_functions=system_functions, banned=banned, banned_names=banned_names, banned_basenames=banned_basenames, banned_class_types=banned_class_types, banned_classes=banned_classes, banned_functions=banned_functions, log_level=log_level)
			_code_examples_ = ""
