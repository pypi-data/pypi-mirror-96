...
class WindFarm:
    ...
    def observe(self):
        ...
        nn, en = g.number_of_nodes(), g.number_of_edges()

        g.ndata['wind_speed'] = torch.ones(nn, 1) * self.wind_speed
        g.edata['wind_speed'] = torch.ones(en, 1) * self.wind_speed
        ...
        # node feature
        g.ndata['feat'] = torch.ones(nn, 1) * self.wind_speed
        ...
        # global feature
        ws = torch.ones(1) * self.wind_speed
        wd = torch.ones(1) * self.wind_direction
        u = torch.cat([ws, wd], dim=0)
