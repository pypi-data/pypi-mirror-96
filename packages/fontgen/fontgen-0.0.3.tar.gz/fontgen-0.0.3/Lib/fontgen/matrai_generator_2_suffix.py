...
SCRIPT_ABBR_CURRENT_suffix = "-takr"
match_mI_variants = 1
...
TEST = False
INPUT_FILE_NAME = "ITF Takri.designspace"
...
def main():
    ...
    print(SCRIPT_ABBR_CURRENT_suffix)

    if  "mI_short" in master1:
        print("Found "+ SCRIPT_ABBR_CURRENT_suffix +"mI_short: Going ahead...")
    else:
        print("No "+ SCRIPT_ABBR_CURRENT_suffix +"mI_short found: Check the font file")
    ...
    for base in bases:
        ...
        new_name = "mI." + first_digit[0] + second_digit[0]
    ...
    # SCRIPT_ABBR_CURRENT_suffix = "dv"
    substitute_rule_lines = []
    name_default = "signi" + SCRIPT_ABBR_CURRENT_suffix
...
class MatraIUtil:
    ...
    class Base(object):
        def __init__(self, feature, base_glyph_sequence):
            ...
            self.glyphs = feature.glyphSet[base_glyph_sequence[0]]
            #rahul
            # print("Yes, I am here!!!!")
            self.target = None
    ...
    def bases_alive(self):
        ...
        for glyph in self.glyphSet:
            name = glyph
            if name == "cha-takr.alt":
                print("here")
            main, sep, suffix = name.partition(".")
            end = main
            if end.endswith(SCRIPT_ABBR_CURRENT_suffix):
                # main, sep, suffix = name[2:].partition(".")
                # end = main.split("_")[-1]
                end = end[:-5]
                if end.startswith('sign') :
                    continue
                elif end.startswith('anusvara') :
                    continue
                elif end.startswith('visarga'):
                    continue
                elif end.startswith('virama'):
                    continue
                elif end.startswith('nukta'):
                    continue
                elif end.startswith('a'):
                    continue
                    ...
                    continue
                elif end.startswith('zero'):
                    continue
                elif end.startswith('one'):
                    continue
                elif end.startswith('two'):
                    continue
                elif end.startswith('three'):
                    continue
                elif end.startswith('four'):
                    continue
                elif end.startswith('five'):
                    continue
                elif end.startswith('six'):
                    continue
                elif end.startswith('seven'):
                    continue
                elif end.startswith('eight'):
                    continue
                elif end.startswith('nine'):
                    continue
                else:
                    aliveBases.append(end +SCRIPT_ABBR_CURRENT_suffix + sep + suffix)
        for i in aliveBases:
            if i not in res:
                res.append(i)
        return res
    ...
    def bases_dead(self):
        ...
        for glyph in self.glyphSet:
            ...
            if name.endswith(SCRIPT_ABBR_CURRENT_suffix):
                main, sep, suffix = name.partition("-")
                end = main.split("_")[-1]
                if end.endswith("xA"):
                    end = end[:-2] + "A"
    ...
    def generate_matrai_match(self):
        ...
        # self.project.SCRIPT_ABBR_CURRENT_suffix = dv
        ###########################

        abvm_position_in_mE = self._get_abvm_position(
            self.font["signaa" + SCRIPT_ABBR_CURRENT_suffix],
            in_base=False,
        )
        ...
        self.tolerance = self._get_stem_position(
            self.font["va" + SCRIPT_ABBR_CURRENT_suffix]
        ) * 0.5
        ...
        self.name_default = "signi" + SCRIPT_ABBR_CURRENT_suffix
