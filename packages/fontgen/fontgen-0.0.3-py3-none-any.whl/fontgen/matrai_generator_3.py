...
def main():
    ...
    regular.interpolate_matrai(master3)
    regular.generate_matrai_match()

    matches_bold = bold.matches
    matches_light = light.matches
    matches_regular = regular.matches

    bases = bold.bases

    generatedmi_1 = Font.open(bold.tempPath)
    generatedmi_2 = Font.open(light.tempPath)
    generatedmi_3 = Font.open(regular.tempPath)
    ...
    for base in bases:
        ...
        thirdpair = find_match_matra(base, matches_regular)

        first_digit = re.findall("\d{2}", firstpair)
        second_digit = re.findall("\d{2}", secondpair)
        third_digit = re.findall("\d{2}", thirdpair)

        new_name = SCRIPT_ABBR_CURRENT + "mI." + first_digit[0] + second_digit[0] + third_digit[0]
        ...
        if not match_dict[new_name]:
            ...
            new_glyph_3 = generatedmi_3[thirdpair].copy(new_name)
            generatedmi_3.addGlyph(new_glyph_3)

        print("new glyph added {}".format(new_name))

        match_dict[new_name].append(base.glyphs.name)

        print("Found all three glyphs")
    ...
    for g3 in generatedmi_3:
        if not g3.name in master3:
            master3.addGlyph(g3)
            print("Added glyphs is: " + g3.name)
        else:
            del master3[g3.name]
            print("Deleted glyphs is: " + g3.name)
    ...
    if TEST:
        ...
        master3.save(os.path.join(TEMP_DIR, "generated3.ufo"), overwrite=True)
    else:
        master1.save(master1.path, overwrite=True)
        master2.save(master2.path, overwrite=True)
        master3.save(master3.path, overwrite=True)
...
class MatraIUtil:
    ...
    def _get_adjustment(self):
        ...
        if "Regular" in self.tempPath:
            adjustment_for_matching_mI_variants = adjustment_for_matching_mI_variants_regular
