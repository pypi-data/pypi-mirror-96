...
def check_for_custom_token():
    home = expanduser("~")
    dotinfinstor = home + separator + ".infinstor"
    print("Checking for custom token in " + dotinfinstor)
    if (not os.path.exists(dotinfinstor)):
        print('check_for_custom_token: dir ' + str(dotinfinstor)
                + ' does not exist. No custom token')
        return False
    tokfile = dotinfinstor + separator + "token"
    if (not os.path.exists(tokfile)):
        print('check_for_custom_token: file ' + str(tokfile)
                + ' does not exist. No custom token')
        return False
    ...
    if (ftoken and ftoken.startswith('Custom ')):
        ...
        print('check_for_custom_token: success. found custom token')
        return True
    else:
        return False;
