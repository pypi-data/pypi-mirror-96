...
from openmodule.utils.backend.backend import Backend
from openmodule.utils.backend.models import AccessRequest, CountMessage, Access
from openmodule.core import core
from openmodule.models import Gateway
...
class BackendTestMixin(OpenModuleCoreTestMixin):
    ...
    * set the backend and a USER_PREFIX
    """
    backend_class = None
    USER_PREFIX = None
    ...
    def setUpClass(cls) -> None:
        assert cls.USER_PREFIX is not None, "set a valid USER_PREFIX"
    ...
    def create_count_message(self, transaction_id=None, *, resource="arivotest", user="user", gate="gate1",
        ...
        """
        user = f"{self.USER_PREFIX}-{category}_{user}"
        gateway = Gateway(gate=gate, direction=direction)
    ...
    def check_out_user_changed(self, transaction_id, user="user1") -> CountMessage:
        ...
        user = f"{self.USER_PREFIX}-{message.category}_{user}"
