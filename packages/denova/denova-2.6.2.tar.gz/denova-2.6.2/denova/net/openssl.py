...
'''
    ...
    Requires denova.net.cli, which requires pexpect to be installed from pypi.

    To do: Sanitize denova.net.cli inputs.
...
def verify_certificate(hostname, port, ca_certs_dir=None):
    '''
        ...
        >>> if whoami() == 'root':
        ...     ok, __, error_message = verify_certificate('DeNova.private.server', 443)
        ...     ok
    ...
    def verify_date(cert):
        ...
        not_before, not_after = get_valid_dates(cert)
        try:
