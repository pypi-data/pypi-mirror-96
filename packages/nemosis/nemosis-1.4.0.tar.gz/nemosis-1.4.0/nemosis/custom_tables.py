...
def fcas4s_scada_match(start_time, end_time, table_name, raw_data_location, select_columns=None, filter_cols=None,
    ...
    fcas4s_variable_types = data_fetch_methods.static_table(table_name_variable_types,
                                                            raw_data_location)
    ...
    inter_flows = inter_flows.groupby(
        ['SETTLEMENTDATE', 'INTERCONNECTORID'], as_index=False).last()
    inter_flows = inter_flows.loc[:,
                                  ('SETTLEMENTDATE', 'INTERCONNECTORID', 'METEREDMWFLOW')]
    inter_flows['SETTLEMENTDATE'] = inter_flows['SETTLEMENTDATE'] - \
        timedelta(minutes=5)
    ...
    profile_comp = pd.merge(fcas4s, scada_elements, 'inner',
                            left_on='TIMESTAMP', right_on='SETTLEMENTDATE')
    ...
    error_comp = error_comp.groupby(
        ['MARKETNAME', 'ELEMENTNUMBER'], as_index=False).sum()
    ...
    best_matches_scada['ERROR'] = best_matches_scada['ERROR'] / \
        best_matches_scada['SCADAVALUE']
...
def capacity_factor_over_90th_percentile_of_nodal_demand(capacity_and_scada_grouped):
    ...
    capacity_and_scada_grouped = capacity_and_scada_grouped.reset_index(
        drop=True)
...
def stats_by_month_and_plant(capacity_and_scada):
    ...
    capacity_and_scada['MONTH'] = capacity_and_scada['effective_set_date'].dt.year.astype(str) + '-' + \
        capacity_and_scada['effective_set_date'].dt.month.astype(
            str).str.zfill(2)
    capacity_factors = capacity_and_scada.groupby(
        ['MONTH', 'DUID'], as_index=False).apply(stats_for_group)
...
def merge_tables_for_plant_stats(timeseries_df, gen_max_cap, gen_region, scada, trading_load, dispatch_price,
    ...
    merged_data = pd.merge_asof(
        scada, gen_max_cap, left_on='SETTLEMENTDATE', right_on='EFFECTIVEDATE', by='DUID')
    gen_region = gen_region.sort_values('START_DATE')
    merged_data = pd.merge_asof(
        merged_data, gen_region, left_on='SETTLEMENTDATE', right_on='START_DATE', by='DUID')
    merged_data = merged_data[~merged_data['REGIONID'].isnull()]
    merged_data = pd.merge(merged_data, trading_load,
                           'left', on=['SETTLEMENTDATE', 'DUID'])
    merged_data = pd.merge(merged_data, dispatch_price, 'left', on=[
                           'SETTLEMENTDATE', 'REGIONID'])
    merged_data = pd.merge(merged_data, trading_price, 'left', on=[
                           'SETTLEMENTDATE', 'REGIONID'])
    merged_data = pd.merge(merged_data, region_summary, 'left', on=[
                           'SETTLEMENTDATE', 'REGIONID'])
...
def select_intervention_if_present(data, primary_key):
    ...
    data = data.groupby([col for col in primary_key if col !=
                         'INTERVENTION'], as_index=False).last()
    return data
...
def select_highest_version_number(data, primary_key):
    ...
    data = data.groupby([col for col in primary_key if col !=
                         'VERSIONNO'], as_index=False).last()
    return data


def calc_trading_vwap_price(regional_demand, dispatch_price):
    demand_and_price = pd.merge(regional_demand, dispatch_price, 'inner', on=[
                                'SETTLEMENTDATE', 'REGIONID'])
    demand_and_price['COST'] = demand_and_price['RRP'] * \
        demand_and_price['TOTALDEMAND']
    ...
    demand_and_price['TRADING_VWAP'] = demand_and_price['COST'] / \
        demand_and_price['TOTALDEMAND']
...
def calc_trading_price(dispatch_price):
    ...
    dispatch_price = dispatch_price.groupby('REGIONID').resample(
        '30T', label='right', closed='right').aggregate({'RRP': 'mean'})
...
def plant_stats(start_time, end_time, table_name, raw_data_location, select_columns=None, filter_cols=None,
    ...
    ix = pd.date_range(start=datetime.strptime(start_time, '%Y/%m/%d %H:%M:%S'),
    ...
    gen_region = data_fetch_methods.dynamic_data_compiler(start_time, end_time, 'DUDETAILSUMMARY', raw_data_location,
                                                          select_columns=[
                                                              'START_DATE', 'END_DATE', 'DUID', 'REGIONID'],
...
def trading_and_dispatch_cost():
    ...
    ix = pd.date_range(start=datetime.strptime('2017/01/01 00:00:00', '%Y/%m/%d %H:%M:%S'),
    ...
    scada_and_regions['DISPATCHCOST'] = (
        scada_and_regions['SCADAVALUE'] * scada_and_regions['RRP']) / 12
    ...
    scada_and_regions['TRADINGCOST'] = (
        scada_and_regions['SCADAVALUE'] * scada_and_regions['RRP']) / 2
