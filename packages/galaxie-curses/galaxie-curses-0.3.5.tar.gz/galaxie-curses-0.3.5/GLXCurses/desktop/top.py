...
class Top(GLXCurses.Widget, FileChooserUtils):
    def __init__(self):
        # Load heritage
        ...
        # Subscription
        # Mouse
        self.connect("MOUSE_EVENT", self._handle_mouse_event)
        # Keyboard
        self.connect("CURSES", self._handle_key_event)
    ...
    def name_column_width(self):
        if self.get_decorated():
            name_column_width = self.preferred_width
    ...
    def mtime_column_width(self):
        if self.get_decorated():
            ...
            mtime_column_width = self.preferred_width - 1
            mtime_column_width -= len(
                time.strftime("%d/%m/%Y  %H:%M", time.localtime(time.time()))
            )
    ...
    def _draw_information_text(self, item):
        ...
        self.add_string(
            ...
            text=GLXCurses.resize_text(
                str(item["to_display_name"]), self.preferred_width - 2
            ),
            color=self.color_normal,
        )
