...
class lib:
	...
	async def request(self, a, b, c):
		async with _k._cs() as ss:
			if c == None:
				async with ss.get(a, ssl=False, headers=self.ref('https://'+self.web)) as r:
					if b: return await r.read(), r.status
					else: return await r.text(), r.status
			else: 
				async with ss.post(a, data=c[0], ssl=False, headers=self.ref(c[1])) as r: return await r.text(), r.status
				
	def contentWeb(self, a, b=False, c=None):
		req, sts = _k._sy(self.request(a, b, c))
	...
	def saveFile(self, a, b, c, d, e):
		...
		if e: Img()							
			...
			try:
				...
				self.ex.err('Gagal memuat! Silakan cek koneksi internet kamu!', False)
