...
class download:
	...
	def getData(self):
		def result(url):
			...
			none = ''.join(['%s\40\u2022\40'% key for key in dict_url.keys()])
		...
		if self.mode != False:
			...
			if url != '':
				if self.alias != 'mt':
					title = _k._uq(_k._c(r'(\/manga\/|\/komik\/|\/mg\/)(.*)' if self.alias != 'mb' else r'(\/baca-komik-)(.*)-bahasa-indonesia').findall(url)[0][1].translate(str.maketrans({'-':' ', '_':' ','/':''}))).title()
				else:
					title = _k._uq(_k._c(r'<h1 class="comics-title">(.*?)</h1>').findall(self.lib.contentWeb(url)[0])[0])
