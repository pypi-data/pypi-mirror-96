...
class SocketIOFastTickerAPI:
    ...
    def __init__(self, symbol):  # FX_BTC_JPY or BTC_JPY
        self.symbol = symbol
        ...
        ws.register_handler(
            channel=f'lightning_ticker_{self.symbol}',
            handler=on_ticker)

        ws.register_handler(
            channel=f'lightning_board_snapshot_{self.symbol}',
            handler=on_order_book_snapshot)

        ws.register_handler(
            channel=f'lightning_board_{self.symbol}',
