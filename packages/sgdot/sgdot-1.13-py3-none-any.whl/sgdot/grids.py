...
class Grid:
    ...
    def __init__(self,
        ...
        if image_path is None:
            visu = GridVisualizer()
            self.__initial_image = np.ones((512, 512, 3), np.uint8) * 255
            self.__image = visu.add_upper_margin(np.zeros((512, 512, 3),
                                                          np.uint8),
                                                 int(0.15 * 512))
    ...
    def shift_node(self,
        ...
        new_coord_within_boundary = (
            self.__nodes['pixel_x_axis'][node] +
            delta_x <= self.__image.shape[0]
            and self.__nodes['pixel_x_axis'][node] + delta_x >= 0
            and self.__nodes[
                'pixel_y_axis'][node] + delta_y <= self.__image.shape[1]
            and self.__nodes[
    ...
    def clear_image(self):
        ...
        self.__image = visu.add_upper_margin(self.__image,
                                             int(0.15 * self.__image.shape[0]))
