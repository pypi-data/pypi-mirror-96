...
def is_backup_task_of_current_user(task, connection):
    ...
    return task_details.get('type', None) == 'backup' and task_details.get('queuedBy', None) == connection.identity
...
def check_backup_needed(project, connection):
    ...
    if last_modified_date is not None:
        ...
        if parsed_last_modified_date < earliest_task_retrieval_date:
            closed_tasks = connection.tasks('closed', project_id, start=earliest_task_retrieval_date, end=now)
        else:
