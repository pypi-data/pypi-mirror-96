...
class QRequest:
    ...
    def exec(self, result=None):
        ...
        logger.warning('%s: EXECUTING REQUEST WITH:\n\trequest=%s\n\tused_fields=%s\n\tidents=%s\n\tliterals=%s\n\t',
                       self, self.request, self.used_fields, self.identifiers, self.literals)
        data = self.connector.exec(self.request, self.identifiers, self.literals, result=result)

        if self.auto_commit:
            self.connector.commit()

        logger.warning('%s: EXECUTED REQUEST:\n\trequest=%s\n\tused_fields=%s\n\tdata=%s\n\t',
                       self, self.request, self.used_fields, data)
