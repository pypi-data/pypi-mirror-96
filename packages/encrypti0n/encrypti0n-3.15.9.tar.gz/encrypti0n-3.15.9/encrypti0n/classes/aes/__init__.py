...
class Database(object):
	...
	def load(self, 
		...
		if format in ["list", "Array"]:
			return r3sponse.success(f"Successfully parsed {path}.", {
				"format":"Array",
				"content":self.Array(path=path, default=default),
			})
		elif format in ["dict", "Dictionary"]:
			return r3sponse.success(f"Successfully parsed {path}.", {
				"format":"Dictionary",
				"content":self.Dictionary(path=path, default=default),
			})
		elif format in ["file", "File"]:
			return r3sponse.success(f"Successfully parsed {path}.", {
				"format":"File",
				"content":self.File(path=path, default=default),
			})
		else:
	...
	class Dictionary(Dictionary):
		...
		def load(self):
			...
			self.dictionary = Dictionary(content)
			self.format = format
			self.loaded = True
			return r3sponse.success(f"Successfully loaded content {self.path}.")
		def save(self):
