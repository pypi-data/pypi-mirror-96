...
class Database(object):
	...
	def load(self, 
		...
		response = r3sponse.check_parameters({
			"path:str":path,})
		if not response.success: return response
		content = self.DataObject(path=f"{self.path}/{path}", default=default, aes=self.aes)
	...
	class DataObject(object):
		...
		def check(self, default=None, save=True):
			...
			if not Files.exists(self.path): 
				...
				if new_format in ["dict", "Dictionary"]:
					new = Dictionary(self.content).check(default=default, save=False)
					print("NEW:",new,"VS",self.content,"=",new != Dictionary(self.content))
					if new !=  Dictionary(self.content): do = True
				elif new_format in ["list", "Array"]:
					new = Array(default)
					if new != Array(self.content): do = True
				elif new_format in ["str", "float", "int", "String", "Integer", "Boolean"]:
					new = default
					if new != old: do = True
				else: return str(self.content).error(f"Unsupported new format: {new_format}.")
			if do:
				self.content = Formats.initialize(new)
				self.format = Formats.get(Formats.denitialize(self.content), serialize=True)
				if save:
					self.loaded = True
					response = self.save()
					if not response.success: return response
		...
		def load(self):
			...
			if format in ["dict", "list", "Dictionary", "Array"]:
				try:
					content = json.loads(decrypted)
				except Exception as e: return r3sponse.error(f"Failed to serialize content {self.path}, {e}.")
			elif format in ["int", "Integer"]:
				try:
					content = Integer(decrypted)
				except Exception as e: return r3sponse.error(f"Failed to serialize content {self.path}, {e}.")
			elif format in ["float", "Integer"]:
				try:
					content = Integer(decrypted)
				except Exception as e: return r3sponse.error(f"Failed to serialize content {self.path}, {e}.")
			...
			self.content = Formats.initialize(content)
			self.format = format
			self.loaded = True
		...
		def save(self):
			...
			if self.format in ["list", "dict", "Dictionary", "Array"]:
				try:
					content = json.dumps(Formats.denitialize(self.content))
				except Exception as e: return r3sponse.error(f"Failed to dump content {self.path}, {e}.")
			elif self.format in ["str", "int", "float", "String", "Integer", "Boolean"]:
		...
		def items(self):
			...
			if self.format in ["list", "Array"]:
				return self.content
			elif self.format in ["dict", "Dictionary"]:
		...
		def values(self):
			if not self.loaded: raise ValueError("Load the content object first.")
			if self.format in ["dict", "Dictionary"]:
		...
		def assign(self, new):
			if not self.loaded: raise ValueError("Load the content object first.")
			if self.format in ["dict", "Dictionary"] and isinstance(new, (dict, Dictionary)):
				for key,value in new.items():
					self.content[key] = value
			elif self.format in ["list", "Array"] and isinstance(new, (list, Array)):
				self.content = Array(new)
			elif self.format in ["str", "String"] and isinstance(new, (str, String)):
				self.content = String(new)
			elif self.format in ["int", "float", "Integer"] and isinstance(new, (int, float, Integer)):
				self.content = Integer(new)
			elif self.format in ["bool", "Boolean"] and isinstance(new, (bool, Boolean)):
				self.content = Boolean(new)
			else:
		...
		# string format.
		def __str__(self):
			return str(Formats.denitialize(self.content))
		# repr.
		def __repr__(self):
			if not self.loaded: 
