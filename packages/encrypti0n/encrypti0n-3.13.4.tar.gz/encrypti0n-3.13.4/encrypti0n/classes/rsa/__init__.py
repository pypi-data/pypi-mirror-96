...
class RSA(object):
	...
	def generate_keys(self, log_level=0):
		...
		# set dir,
		directory = str(self.director)y
		private_key_path = str(self.private_ke)y
		public_key_path = str(self.public_ke)y
	...
	def load_public_key(self):
		...
		# initialize keys.
		try:
			self.public_key_object = _RSA_.importKey(self.public_key_data)
		except ValueError as e:
			if "Padding is incorrect" in str(e):
				response["error"] = "Provided an incorrect passphrase."
			else: response["error"] = f"ValueError: {e}"
	...
	def load_private_key(self):
		...
		if self.passphrase == None:
			try:
				self.private_key_object = _RSA_.importKey(self.private_key_data)
			except ValueError as e:
				if "Padding is incorrect" in str(e):
					response["error"] = "Provided an incorrect passphrase."
				else: response["error"] = f"ValueError: {e}"
			...
			try:
				self.private_key_object = _RSA_.importKey(self.private_key_data, passphrase=self.passphrase)
			except ValueError as e:
				if "Padding is incorrect" in str(e):
					response["error"] = "Provided an incorrect passphrase."
				else: response["error"] = f"ValueError: {e}"
	...
	def edit_passphrase(self, passphrase=None):
		...
		if "Your identification has been saved with the new passphrase." in output:
			self.private_key_data = Files.load(private_key)
			if not self.memory:
