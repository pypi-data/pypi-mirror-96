...
try: import cl1
	...
	if os.path.exists("/usr/bin/pip3"): os.system(f"/usr/bin/pip3 install cl1 --user {syst3m.defaults.vars.user}")
...
# download.
	...
	if os.path.exists("/usr/bin/pip3"): 
		os.system(f"/usr/bin/pip3 install -r {syst3m.defaults.source_path(__file__, back=3)}/requirements/requirements.txt --user {os.environ.get('syst3m.defaults.vars.user')}")
	else:
...
cache = syst3m.cache.Cache(
	path=gfp.clean(f"{syst3m.defaults.vars.home}/.{ALIAS}/.cache/"))
