...
class SmartCards(object):
	...
	def scan(self, silent=False):
		...
		# response.
		return r3sponse.success(f"Successfully scanned & detected {len(smart_cards)} smart card(s).", {
...
class SmartCard(object):
	...
	def check_smart_card(self):
		...
		if "Application version " in output: 
			return r3sponse.success("Yubikey smart card detected.", {"smart_card":True})
		else: 
			return r3sponse.success("No yubikey smart card detected.", {"smart_card":False})
