...
if CHECKS and not cl1.arguments_present("--reset-cache"):
	...
	if not Files.exists(f"{SOURCE_PATH}/lib"):
		r3sponse.log("Downloading ssht00ls library.")
	...
	if not Files.exists("/usr/local/lib/ssht00ls"):
		r3sponse.log(f"{syst3m.color.orange}Root permission{syst3m.color.end} required to install the ssht00ls library.")
		os.system(f" sudo rsync -azP {SOURCE_PATH}/ /usr/local/lib/ssht00ls")
