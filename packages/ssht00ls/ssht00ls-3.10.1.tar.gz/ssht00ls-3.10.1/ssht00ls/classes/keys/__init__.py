...
class Keys(object):
	...
	def generate(self, path=None, passphrase=None, comment=""):
		...
		if Files.exists(f"{directory}/public_key"): 
			...
			Files.chown(path=directory, owner=os.environ.get("USER"), group=None, sudo=True)
		...
		Files.chown(path=private_key, owner=os.environ.get("USER"), group=None, sudo=True)
		...
		Files.chown(path=public_key, owner=os.environ.get("USER"), group=None, sudo=True)
	...
	def check(self, username=None, public_keys=[], reversed=False):
		...
		ssh_dir = FilePath(f"{HOME_BASE}/{username}/.ssh/")
		auth_keys = FilePath(f"{HOME_BASE}/{username}/.ssh/authorized_keys")
