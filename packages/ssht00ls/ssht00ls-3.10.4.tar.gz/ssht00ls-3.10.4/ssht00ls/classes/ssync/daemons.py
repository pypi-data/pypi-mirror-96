...
class Daemon(syst3m.objects.Thread):
	...
	def index(self, short=False):
		...
		for path, mtime in index.items():
			...
			try:
				...
				return r3sponse.error(f"Should not happen(#278363). (subpath: {subpath}), (path: {path}), (index: {json.dumps(index,indent=4)}), (remote index: {json.dumps(remote_index,indent=4)}).")
		for path, mtime in remote_index.items():
			...
			try:
				...
				return r3sponse.error(f"Should not happen(#739639). (subpath: {subpath}), (path: {path}), (index: {json.dumps(index,indent=4)}), (remote index: {json.dumps(remote_index,indent=4)}).")
	...
	def mismatches(self, clean_index, remote_clean_index, all_paths):
		...
		return r3sponse.error(f"Failed to synchronize [{self.alias}:{self.remote}] & [{self.path}], mismatches: {json.dumps(mismatches, indent=4)}].")
		"""
	def process_mismatches(self, clean_index, remote_clean_index, mismatches):
		...
		for subpath, info in mismatches.items():
			...
			if rmtime == None and lmtime == None: 
				...
				return r3sponse.error(f"No remote & local modification time present. (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
			...
			# one missing.
				...
				# should not happen.
					...
					self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
					return r3sponse.error(f"Should not happen (#3243443). (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
			...
			# both present.
				...
				# should not happen.
					...
					self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
					return r3sponse.error(f"Unable to compare rmtime: {rmtime} & lmtime: {lmtime}. (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")

			# exceptions.
			else:
				self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
				return r3sponse.error(f"Should not happen (#407294). (path: {subpath}), (rmtime: {rmtime}), (lmtime: {lmtime}), (last_rmtime: {last_rmtime}), (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
			...
			if directory and local_to_remote != None and remote_to_local != None and "delete" not in options and not empty_directory:
				...
				if local_to_remote and remote_to_local:
					self.set_last_index(clean_index) ; self.set_last_index(remote_clean_index, remote=True)
					return r3sponse.error(f"Can not synchronize both remote to local & local to remote (rmtime: {rmtime}) (lmtime: {lmtime}), (last_rmtime: {last_rmtime}) (last_lmtime: {last_lmtime}), index: {json.dumps(clean_index,indent=4)}, remote index: {json.dumps(remote_clean_index,indent=4)}.")
	...
	def __sync__(self, multiprocessing=True, max_batch_size=150):
		...
		if not response["success"]:  return response
			...
			if self.log_level >= 3:
				return r3sponse.success(f"Directories [{self.alias}:{self.path}] & [{self.path}] are already synchronized, index: {json.dumps(response.index, indent=4)}, remote index: {json.dumps(response.remote_index, indent=4)}.")
			else:
		...
		if multiprocessing:
			...
			if depths != {}:
				return r3sponse.error(f"Should not happen(#234938). (depths: {depths}), (updates: {json.dumps(updates,indent=4)}).")
			...
			#print("UPDAT BACTHES:",json.dumps(update_batches, indent=4))
			sum = True
			...
			# execute commands.
			#print("COMMANDS:",json.dumps(commands, indent=4))
			if sum:
		...
		if not response["success"]: return response
			...
			return r3sponse.error(f"Failed to synchronize [{self.alias}:{self.remote}] & [{self.path}], mismatches: {json.dumps(response.mismatches, indent=4)}].")
