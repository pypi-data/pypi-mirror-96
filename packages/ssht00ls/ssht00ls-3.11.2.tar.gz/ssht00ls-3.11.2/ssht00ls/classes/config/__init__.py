...
# universal options.
CLI = syst3m.env.get_boolean("CLI", default=False)
INTERACTIVE = not (cl1.arguments_present(["--non-interactive"]) or syst3m.env.get_boolean("NON_INTERACTIVE"))
...
if syst3m.defaults.options.log_level >= 1:
	r3sponse.log(f"{ALIAS} version: {version}")
...
cache = syst3m.cache.Cache(
	path=gfp.clean(f"{HOME}/.{ALIAS}/.cache/"))

# files.
DATABASE = Files.Directory(path=gfp.clean(f"{HOME}/.{ALIAS}"))
CONFIG = Dictionary(path=gfp.clean(syst3m.env.fill(syst3m.env.get_string("SSHT00LS_CONFIG", default=DATABASE.join("config","")))), load=True, default={})
if syst3m.defaults.options.log_level >= 1:
...
if CHECKS and not cl1.arguments_present("--reset-cache"):
	...
	for dir, permission in [
		...
		if not Files.exists(dir): 
			os.system(f"sudo mkdir {dir} && sudo chown {OWNER}:{GROUP} {dir} && sudo chmod {permission} {dir}")

	# files.
	CONFIG.check(save=True, efault={
		"aliases":{
