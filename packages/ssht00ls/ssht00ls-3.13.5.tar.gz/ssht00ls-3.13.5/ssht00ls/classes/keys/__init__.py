...
class Keys(object):
	...
	def generate(self, path=None, passphrase=None, comment=""):
		...
		if Files.exists(f"{directory}/public_key"): 
			...
			Files.chown(path=directory, owner=os.environ.get("syst3m.defaults.vars.user"), group=None, sudo=True)
		...
		Files.chown(path=private_key, owner=os.environ.get("syst3m.defaults.vars.user"), group=None, sudo=True)
		...
		Files.chown(path=public_key, owner=os.environ.get("syst3m.defaults.vars.user"), group=None, sudo=True)
