...
class ULA(BaseKernel):
    ...
    def forward(self, z, target_logprob, x=None, reverse_kernel=None, mu_amortize=None):
        """
        Input:
        ...
        with torch.no_grad():
            ...
            if self.ula_skip_threshold > 0.:
                ...
                a, _, _ = get_acceptance_ratio(log_t, log_1pt, use_barker=False)
        ...
        return {"z_new": z_new,
                "decisions": a.to(torch.float32),
                "current_log_alphas": proposal_density_numerator - proposal_density_denominator + self.log_jac,
                "forward_grad": forward_grad,
                "score_match_cur": score_match_cur}
