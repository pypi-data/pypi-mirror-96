...
class Config:
    ...
    def get(self):
        if self.CONFIG_FILE.exists():
            return pickle.load(self.CONFIG_FILE.open('rb', encoding='utf-8'))
        else:
            return {}

    def put(self, config):
        pickle.dump(config, self.CONFIG_FILE.open('wb', encoding='utf-8'))
                ...
                if self.case_ws[developer_cell].value:
                    ...
                    task_id = self.excel.tapd.create_tapd_task(
                        ...
                        description=template.render(
                            ...
                            expected=self.case_ws[expected_cell].value or ''
                        )
                    )
                    if task_id:
