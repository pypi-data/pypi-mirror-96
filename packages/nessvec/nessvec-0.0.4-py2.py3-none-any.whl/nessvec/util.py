...
GLOVE_URL_TEMPLATE = 'http://nlp.stanford.edu/data/glove.{size}B.zip'
...
def unzip(zip_filepath, dest_filename=None, dim=50):
    ...
    size = int(re.search(r'[.](\d{1,3})B[.]', zip_filepath).groups()[0])
    dest_filename = dest_filename or GLOVE_FILENAME_TEMPLATE.format(dim=dim, size=size)
    zip_filepath = os.path.readlpath(os.path.expanduser(zip_filepath or DATA_DIR))

    with ZipFile(zip_filepath, 'r') as zipobj:
        zipobj.extract(dest_filename, path=DATA_DIR)
    dest_filepath = os.path.join(
        os.path.join(*os.path.split(zip_filepath)[:-1]),
        dest_filename)
...
def load_glove(dim=50, size=6, filepath=None):
    ...
    filepath = filepath or os.path.join(DATA_DIR, GLOVE_FILENAME_TEMPLATE.format(
        dim=dim, size=size))
