...
def deserialize(json: Dict[str, Any], target: Type[T]) -> T:
    if not decorator_utils.has_decorator(target, serializable):
        ...
        try:
            logger.debug(f'Parsing json "{str(json)} to "{str(target)}"')
            ...
            for field in fields(target):
                ...
                logger.debug(f'Parsing field "{field.name}", type="{field.type}", json="{json_value}"')
                logger.debug(get_origin(field.type))
                logger.debug(get_origin(field.type) is list)
                if get_origin(field.type) is list and decorator_utils.has_decorator(get_args(field.type)[0], serializable):
                    logger.debug(f'Parsing list item')
                    ...
                    logger.debug(f'Parsing non-list item')
