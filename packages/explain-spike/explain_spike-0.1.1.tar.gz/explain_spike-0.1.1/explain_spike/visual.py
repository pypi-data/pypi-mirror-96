...
def plot_VI(features_names: list, features_values: list, n: int = 10, height: int = 300):
...
def shap_local_exp(shap_values, id_: list, data: frame = None, plot_type='bar', max_display: int = 10, labels=None):
    ...
    if plot_type=='bar':
        if type(data)==type(None):
            ...
            if len(id_)==1:
                id_ = data.index.get_loc(id_[0])
            else:
                id_ = [data.index.get_loc(i) for i in id_]
        return shap.plots.bar(shap_values[id_], show=False, max_display=max_display)

    elif plot_type=='force_plot':
        if type(data)==type(None):
            if len(id_)==1:
                id_=id_[0]
                return shap.plots.force(shap_values[id_], show=False)
            else:
        ...
        if type(data)==type(None):
            if len(id_)==1:
                id_=id_[0]
                return shap.decision_plot(shap_values.base_values[id_], shap_values.values[id_], shap_values.data, show=False)
            else:
            ...
            if len(id_)==1:
                id_ = data.index.get_loc(id_[0])
                return shap.decision_plot(shap_values.base_values[id_], shap_values.values[id_], data, show=False)
            else:
                id_ = [data.index.get_loc(i) for i in id_]
        return shap.decision_plot(shap_values.base_values[0], shap_values.values[id_,:], data,
                   legend_labels=labels, legend_location='lower right')
    elif plot_type=='waterfall':
        if type(data)==type(None):
            if len(id_)==1:
                id_=id_[0]
                return shap.plots.waterfall(shap_values[id_], show=False, max_display=max_display)
            else:
...
def shap_comparation(shap_contrib, shap_logloss, X, n: int = 10):
    contrib_features, contrib_values = shap_importance(pd.Index(X), shap_contrib, n=n)
    logloss_features, logloss_values = shap_importance(pd.Index(X), shap_logloss, n=n)
