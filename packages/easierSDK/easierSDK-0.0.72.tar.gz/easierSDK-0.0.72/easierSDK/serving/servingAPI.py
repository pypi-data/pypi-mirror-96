...
class ServingAPI():
    ...
    def create_model_serving(self, repo_name:str, category:Categories, model_name:str, experimentID:int, namespace=None):
        ...
        with kubernetes.client.ApiClient() as api_client:
            ...
            config_map['metadata']['name'] += '-' + self._easier_user + '-' +  random_name
            ...
            try:
                api_response = api_instance.create_namespaced_config_map(namespace, config_map, pretty='false')
                print(api_response)
            ...
            deployment['metadata']['name'] += '-' + self._easier_user + '-' + random_name
            deployment['metadata']['labels']['app'] += '-' + self._easier_user + '-' + random_name
            deployment['spec']['selector']['matchLabels']['app'] += '-' + self._easier_user + '-' + random_name
            deployment['spec']['template']['metadata']['labels']['app'] += '-' + self._easier_user + '-' +  random_name
            deployment['spec']['containers'][0]['name'] += '-' + self._easier_user + '-' + random_name
            deployment['spec']['containers'][0]['envFrom'][0]['configMapRef']['name'] += '-' + self._easier_user + '-' + random_name
    
            try:
                api_response = api_instance.create_namespaced_pod(namespace, deployment, pretty='false')
                print(api_response)
            except ApiException as e:
            ...
            service['metadata']['name'] += '-' + self._easier_user + '-' +  random_name
            service['metadata']['labels']['app'] += '-' + self._easier_user + '-' +  random_name
            service['spec']['selector']['app'] += '-' + self._easier_user + '-' + random_name

            try:
                api_response = api_instance.create_namespaced_service(namespace, service, pretty='false')
                print(api_response)
            except ApiException as e:
            ...
            ingress['metadata']['name'] += '-' + self._easier_user + '-' + random_name
            ingress['metadata']['labels']['app'] += '-' + self._easier_user + '-' + random_name
            ingress['spec']['rules'][0]['host'] = model_name + '-' + 'easier-serving' + '-' + self._easier_user + '-' + random_name + '.easier-ai.eu'
            ingress['spec']['rules'][0]['http']['paths'][0]['backend']['serviceName'] += '-' + self._easier_user + '-' + random_name

            try:
                api_response = networking_v1_beta1_api.create_namespaced_ingress(namespace, ingress, pretty='false')
                print(api_response)
            except ApiException as e:
