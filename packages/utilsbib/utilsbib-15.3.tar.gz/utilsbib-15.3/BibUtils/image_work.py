...
def replace_img_part(original_img, substitutes,origin):
    #print("min x =",origin)
    ...
    for substitute in substitutes:
        ...
        #  print(substitute[i])#.coords)
            mask = substitute[i][-2].convert('L')#.resize(im1.size)
            original_img.paste(substitute[i][-2], pos,mask)
    return original_img
...
def apply_errors_on_image(fname, results):
    ...
    origin = results[0][0][0]
    substitutes = []
    for i in results:
        substitutes.append(i[1])
    return replace_img_part(img,substitutes,origin)
