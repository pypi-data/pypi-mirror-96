...
class Face_cropper:
    ...
    def isolate_mouth_cv(self, filename, mouth_only=True, preImg=None, pre_shape=None, crop_lips=False, keep_teeh=False,lips_only = False): #keep teeh if crop_lips
        pos_origin = []
        try:
            if preImg is None:
                img = cv2.imread(filename)
...
def generate_out_slices(filename, faceCropper, nb_tiles, part_out=0):
    fc = faceCropper
