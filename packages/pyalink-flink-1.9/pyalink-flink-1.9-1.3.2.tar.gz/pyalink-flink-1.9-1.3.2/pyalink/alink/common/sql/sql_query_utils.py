...
def sql_query(query: str, op_type: str, need_alias: bool = False):
    if op_type == "batch":
        from pyalink.alink.batch.common import SqlCmdBatchOp
        table_name_map = batch_table_name_map
        sql_cmd_op_cls = SqlCmdBatchOp
    elif op_type == "stream":
        table_name_map = stream_table_name_map
        from pyalink.alink.stream.common import SqlCmdStreamOp
        sql_cmd_op_cls = SqlCmdStreamOp
    else:
        raise Exception("op_type should be 'batch' or 'stream'.")
    ...
    if need_alias:
        sql_cmd_op = sql_cmd_op.setAlias(alias)
