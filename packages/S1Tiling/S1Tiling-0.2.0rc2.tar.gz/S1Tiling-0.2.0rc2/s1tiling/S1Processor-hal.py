...
#     import s1tiling
#     s1tiling.__setattr__(name, value)
#
# def get_var(name):
#     import s1tiling
#     return s1tiling.__getattr__(name)
...
def setup_worker_logs(config, dask_worker):
    ...
    # restarted = Variable(name='restarted')
    # restarted = dask_worker.client.run(get_var, 'restarted')
    # d_logger.warning("Restarted: %s", restarted)
    tracemalloc.start()
...
def process_one_tile(
    ...
    if debug_otb:
        ...
        # restarted = Variable(name='restarted')
        # restarted.set(False)
        # restarted = client.run(attach_var, 'restarted', False)
        nb_tries = 2
        for run in range(1, nb_tries+1):
            try:
                ...
                # restarted.set(True)
                # restarted = client.run(attach_var, 'restarted', True)
...
def main(searched_items_per_page, dryrun, debug_otb, watch_ram, debug_tasks, config_filename):
    ...
    with S1FileManager(config) as s1_file_manager:
        ...
        if not debug_otb:
            ...
            # client.run(attach_var, 'restarted', False)
            # restarted = Variable(name='restarted')
            # restarted.set(False)
