...
from azureml.automl.core.shared.exceptions import AutoMLException, UserException, ValidationException, ClientException
...
def prepare_managed_inputs(experiment, local_path, settings_obj, data_params, run_config):
    """
    ...
    parent_run = create_parent_run_for_local_managed(experiment_state, data_params)
...
def local_managed(experiment: Experiment,
    ...
    :param experiment: The AzureML Experiment.
    :param run_config: The run config for submitting this run.
    :param settings_obj: The AutoMLSettings.
    :param data_params: All the data parameters passed in to fit().
    :param show_output: Whether to block the execution and print incremental results.
    :return: The AutoML local run.
    """
    ...
    try:
        with tempfile.TemporaryDirectory() as local_path:
            parent_run, dataset_args, settings, run_config = prepare_managed_inputs(
                experiment, local_path, settings_obj, data_params, run_config)
            if show_output:
