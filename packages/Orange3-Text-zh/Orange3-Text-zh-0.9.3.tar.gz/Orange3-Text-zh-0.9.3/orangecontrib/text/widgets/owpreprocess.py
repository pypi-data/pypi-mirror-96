...
class TokenizerModule(SingleMethodModule):
    ...
    methods = [
        ...
        preprocess.tokenize.Jieba,
        preprocess.tokenize.PKU,
    ]
    REGEXP = 3
    PKU = 6
    pattern = settings.Setting('\w+')
    models = ['混合领域', '新闻', '微博', '医药', '旅游']
    model_name = models[0]
    ...
    def __init__(self, master):
        ...
        pku_label = gui.label(self, self, '领域:')
        # label.setAlignment(Qt.AlignRight)
        self.method_layout.addWidget(pku_label, self.PKU, 1)
        pku_box = widgets.ComboBox(self, 'model_name',
                                        items=self.models)
        pku_box.currentIndexChanged.connect(self.change_model)
        self.method_layout.addWidget(pku_box, self.PKU, 2)
        # self.models[self.SNOWBALL].language = self.snowball_language
    ...
    def change_model(self):
        if self.methods[self.PKU].model_index != self.models.index(self.model_name):
            self.methods[self.PKU].model_index = self.models.index(self.model_name)

            if self.method_index == self.PKU:
                self.change_signal.emit()
...
class OWPreprocess(OWWidget):
    ...
    name = '预处理文本(Preprocess Text)'
    description = ' 构造一个文本预处理流水线.'
