...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    drive_service = build("drive", "v2", http=http, cache_discovery=False)
    media_body = MediaFileUpload(file_path, mimetype=mime_type, chunksize=150*1024*1024, resumable=True)
    body = {
        "title": file_name,
    ...
    drive_service._http.redirect_codes = set(drive_service._http.redirect_codes) - {308}
