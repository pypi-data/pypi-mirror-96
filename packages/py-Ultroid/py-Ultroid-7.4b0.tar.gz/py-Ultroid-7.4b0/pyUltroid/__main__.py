...
from telethon.tl.functions.channels import JoinChannelRequest, LeaveChannelRequest, EditBannedRequest, InviteToChannelRequest
...
async def hehe():
	if Var.LOG_CHANNEL:
		try:
			await ultroid_bot(AddChatUserRequest(Var.LOG_CHANNEL, Var.BOT_USERNAME, fwd_limit=1000000))
		except:
			await ultroid_bot(InviteToChannelRequest(Var.LOG_CHANNEL, [Var.BOT_USERNAME]))
		except:
			await ultroid_bot.asst.send_message(Var.LOG_CHANNEL,f"**Ultroid has been deployed!**\n➖➖➖➖➖➖➖➖➖\n**UserMode**: [{ultroid_bot.me.first_name}](tg://user?id={ultroid_bot.me.id})\n**Support**: @TheUltroid\n➖➖➖➖➖➖➖➖➖",)
		try:
