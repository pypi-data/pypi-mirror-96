...
def xDNNclassifier(Data,Image):
    ...
    data = Data.copy()
    Centre = data[0,]
    #Centre = Centre/np.sqrt(np.sum(np.power(Centre,2)))
    Centre= preprocessing.normalize(Centre, norm='l2')
    ...
    for i in range(2,L+1):
        ...
        if i == 2:
            distance = cdist(data[i-1,].reshape(1,-1),Centre.reshape(1,-1),'minkowski',8)[0]
        else:
            distance = cdist(data[i-1,].reshape(1,-1),Centre,'minkowski',8)[0]
        value,position= distance.max(0),distance.argmax(0)
        value_2=distance.min(0)
