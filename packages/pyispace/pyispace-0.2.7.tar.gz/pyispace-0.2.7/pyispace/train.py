...
def train_is(metadata: pd.DataFrame, opts: dict) -> Model:
    """
    ...
    # model.pilot.Z = adjust_rotation(model.pilot.Z, ~model.data.Ybin.any(axis=1).reshape(-1, 1))
    # model.pilot.Z = adjust_rotation(model.pilot.Z, ~mode(model.data.Ybin, axis=1)[0])
