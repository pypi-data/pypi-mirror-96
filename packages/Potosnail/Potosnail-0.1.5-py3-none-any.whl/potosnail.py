...
class Wrappers: 
    ...
    def CleanStrings(self, df, rankable_cols=[]):
        dh = DataHelper()
        droplist = []
        binlist = []
        ranklist = []
        ...
        for col in cols:
            if type(df[col][0]) == str:
                if len(list(dict(df[col].value_counts()).keys())) == 1:
                    droplist.append(col)
                if len(list(dict(df[col].value_counts()).keys())) == 2:
                    binlist.append(col)
                else: 
                    if col in rankable_cols:
                        ranklist.append(col)
                    else:
                        ohlist.append(col)
        if len(droplist) != 0:
        ...
        if len(binlist) !=0:
            df = dh.Binarize(df, binlist)
        if len(ohlist) != 0:
            enclist = []
            for element in ohlist:
                enclist.append(dh.OHE(df[element]))
            concatlist = enclist + df
            df = pd.concat(concatlist, axis='columns')
        #if len(ranklist) != 0:
        return None
