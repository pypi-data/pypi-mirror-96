...
def read_dataset(ds, path, y_column, feature_selection_support, feature_selection_cols):
    ...
    selected_features = ds.columns[feature_selection_support] if feature_selection_support is not None else feature_selection_cols
    selected_ds = ds[selected_features]

    return selected_ds.drop(y_column, axis=1), ds[y_column]
