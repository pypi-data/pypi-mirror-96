...
class ZMQProducer(object):
    ...
    def close(self, force=True):
        stillInQueue = 0
        print('close zmq producer')
        while self.messageQueue.queue and not force:
            print('queue size = ' + str(len(self.messageQueue.queue)))
            print('indexPerConsumer:' + str(self.messageQueue.indexPerConsumer))
            print('queue content:' + str(self.messageQueue.queue))
            stillInQueue += 1
            time.sleep(1)
