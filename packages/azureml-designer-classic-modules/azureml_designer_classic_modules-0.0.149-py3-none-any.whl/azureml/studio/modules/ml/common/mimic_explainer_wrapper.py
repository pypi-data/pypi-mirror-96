...
class MimicExplainerWrapper:
    ...
    def explain_global(self, initialization_examples: pd.DataFrame, evaluation_examples: pd.DataFrame):
        ...
        surrogate_model, surrogate_model_params = self._pick_surrogate_model_with_params(model=self.model)
        ...
        with TimeProfile("Init mimic explainer"):
            explainer = MimicExplainer(model=internal_model,
                                       ...
                                       explainable_model=surrogate_model,
                                       explainable_model_args=surrogate_model_params,
                                       augment_data=False,
    ...
    def _setup_data(self, data: pd.DataFrame):
        ...
        with TimeProfile("Setup data"):
            ...
            data = self.model._clean_training_data(data)
...
def explain_model(model, initialization_examples, evaluation_examples):
    ...
    try:
        ...
        module_logger.error(f"Failed to initialize ModuleTelemetryHandler instance due to {ex}")
    ...
    try:
        ...
        try:
            ...
            session_id = None
            module_logger.info("Session_id cannot be imported")
        ...
        if telemetry_handler:
            telemetry_handler.log_telemetry(
                event_name="ExplanationTraceback",
