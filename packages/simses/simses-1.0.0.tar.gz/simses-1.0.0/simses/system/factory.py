...
class StorageSystemFactory:
    ...
    def create_acdc_converter(self, converter: str, max_power: float,
        ...
        if converter_type == NoLossAcDcConverter.__name__:
        ...
        elif converter_type == AixControlAcDcConverter.__name__:
            self.__log.debug('Creating acdc converter as ' + converter_type)
            acdc_converter = AixControlAcDcConverter(max_power)
            ...
            options.append(AixControlAcDcConverter.__name__)
    ...
    def create_dcdc_converter(self, converter: str, intermediate_circuit_voltage: float) -> DcDcConverter:
        ...
        elif converter_type == PgsDcDcConverter.__name__:
            self.__log.debug('Creating dcdc converter as ' + converter_type)
            try:
                max_power: float = float(converter_configuration[converter][StorageSystemConfig.DCDC_CONVERTER_POWER])
            except IndexError:
                max_power = 0.0
            return PgsDcDcConverter(max_power, intermediate_circuit_voltage, self.__power_electronics_data_config)
        else:
            options: [str] = list()
            options.append(NoLossDcDcConverter.__name__)
            options.append(PgsDcDcConverter.__name__)
    ...
    def create_power_distributor_ac(self) -> PowerDistributor:
        ...
        elif power_distributor_type == SecondLifeHybridACPowerDistributor.__name__:
            return SecondLifeHybridACPowerDistributor()
        else:
            options: [str] = list()
            options.append(EqualPowerDistributor.__name__)
            options.append(SocBasedPowerDistributor.__name__)
            options.append(SecondLifeHybridACPowerDistributor.__name__)
    ...
    def create_power_distributor_dc(self, dc_systems: [StorageSystemDC],
        ...
        if power_distributor_type == EqualPowerDistributor.__name__:
            ...
            return SocBasedPowerDistributor()
        elif power_distributor_type == SecondLifeHybridDCPowerDistributor.__name__:
            return SecondLifeHybridDCPowerDistributor()
            ...
            options.append(EqualPowerDistributor.__name__)
            options.append(SocBasedPowerDistributor.__name__)
            options.append(SecondLifeHybridDCPowerDistributor.__name__)
