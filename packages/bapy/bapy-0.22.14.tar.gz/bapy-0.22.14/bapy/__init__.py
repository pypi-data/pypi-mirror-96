...
class Obj:
    ...
    def attrinclude(self, name: Union[str, Iterable]) -> Union[bool, list]:
        attrs = self.attrstart
        return attrs.intersection(set(name)) if not self.new(name).str else name in attrs if self.isattr else True

    @property
    def attrs(self) -> list:
        return list(self.asdict(defaults=True).keys())

    @property
    def attrstart(self) -> set:
        return {item for item in self.dir if not self.new(item).start and item not in self.getmroattr()}
    ...
    def clsinspect(self, kind: Optional[AttributeKind] = None,
                   cls: Any = None, exclude: bool = True) -> Optional[Union[dict[str, Attribute], list]]:
        """
        Class attrs info.
        ...
        Examples:
            ...
            >>> Obj(Path).clsinspect()
            >>> Obj(Path).clsinspect(AttributeKind.DATA)
            >>> Obj(Path, swith='_').clsinspect(AttributeKind.PROPERTY)
        ...
        def _include(name):
            if not exclude:
                return True
            return name in attrstart and not obj.memberdescriptor(name) and not obj.getsetdescriptor(name)

        obj = self if cls is None else self.new(cls)
        attrstart = self.attrstart
        ...
        if self.dataclass or self.dataclass_instance:
            return {f.name: f.default if isinstance(
                ...
                f.default_factory, dataclasses._MISSING_TYPE) else f.default_factory() for f in
                    dataclasses.fields(self.getcls)}
            ...
            return {key: inc[1] for key in self.clsinspect(AttributeKind.DATA)
                    if (inc := self.include(key, self.getcls)) is not None}
        return dict()
    ...
    def dir(self) -> set:
        ...
        return {*(dir(self.data) + dir(self.getcls))}
    ...
    def enuminstance(self) -> bool:
        ...
        return not self.cls \
               and any([issubclass(self.getcls, cls) for cls in classes]) \
               and any([isinstance(self.data, cls) for cls in classes]) \
               and hasattr(self.data, 'name') and hasattr(self.data, 'value')
    ...
    def exclude(self, data: Any, key: bool = True) -> bool:
        obj = self.new(data)
        return any([obj.getmodule == typing, obj.getmodule == _abc, obj.module, obj.routine, obj.cls,
    ...
    def getmroattr(self, name: str = '__exclude_attr__') -> tuple:
        return tuple({attr for item in [*self.mro, Obj(), ] for attr in getattr(item, name, list())})
    ...
    def values(self) -> list:
        return list(self.asdict(defaults=True).values())

    @property
    def vars(self) -> dict:
        attrs = (list(vars(self.data)) if self.dataclass_instance or self.dict_instance else list()) + \
                self.clsinspect(AttributeKind.MEMBER) + \
                (self.clsinspect(AttributeKind.PROPERTY) if self.prop else list())
...
class AsDict:
    ...
    __exclude_attr__ = ['asdict', 'kwargs', 'public', 'values', ]
    ...
    def kwargs(self) -> dict:
        ...
        Returns:
            dict:
        """
        return Obj(self, depth=1, prop=False).asdict()
...
class IPLoc(AsDict):
    ...
    @classmethod
    @functools.cache
    @retry_decorator.retry((urllib.error.HTTPError, json.decoder.JSONDecodeError), tries=3, timeout_secs=10,
                           logger=_log)
    def new(cls, ip: IPLike = None) -> IPLoc:
        with urllib.request.urlopen(f'https://geolocation-db.com/json/697de680-a737-11ea-9820-af05f4014d91/'
                                    f'{str(ip)}') as loc:
            try:
                return cls(**json.loads(loc.read().decode()))
    ...
    @classmethod
    @functools.cache
    async def new_aio(cls, ip: IPLike = None) -> IPLoc:
        return await asyncio.to_thread(cls.new, ip)
...
class IPBase(AsDictDescriptor):
    ...
    def __post_init__(self):
        ...
        self.info = self.addr(self.ip)

        if loc.bool and loc.data is True:
            self.loc = IPLoc.new(self.ip)
    ...
    @staticmethod
    @functools.cache
    def addr(ip: Optional[IPLike]) -> Union[IPv4, IPv6]:
        try:
            return IPv4(ip)
    ...
    async def new(cls,
                  ...
                  info: Optional[Union[IPLike, dict]] = None,
                  loc: Optional[Union[IPLoc, bool, dict]] = None,
                  name: Optional[Union[bool, str]] = None) -> IPBase:
        return await asyncio.to_thread(cls, ip, info, loc, name)
    ...
    def password(self, ip: Optional[Union[IPLike, IPBase]] = None) -> Optional[bool]:
        """
        SSH password.
    ...
    async def password_aio(self, ip: Optional[Union[IPLike, IPBase]] = None) -> Optional[bool]:
        """
        SSH password..
        ...
        """
        ip = str(ip) if ip else self.ip
        return await Sem.SSH.sem(asyncio.to_thread(self.password, ip))

    def ping(self, ip: Optional[Union[IPLike, IPBase]] = None) -> Optional[bool]:
        """
         Pings host.
    ...
    async def ping_aio(self, ip: Optional[Union[IPLike, IPBase]] = None) -> Optional[bool]:
        """
        Pings host.
        ...
        """
        ip = ip if ip else self.ip
        return await Sem.PING.sem(asyncio.to_thread(self.ping, ip))
...
class SemEnum(EnumDict):
    ...
    async def sem(self, func: Union[Callable, Coroutine], /, *args, priority: Priority = Priority.LOW,
                  exceptions: tuple = (KeyboardInterrupt,), nap: Nap = Nap.MONGO, **kwargs) -> Any:
        """
        Examples:
            >>> await Sem.OS.sem(executor, IPBase.new, '8.8.8.8')
...
bapy = Package(filename=__file__, _version='0.22.14')
