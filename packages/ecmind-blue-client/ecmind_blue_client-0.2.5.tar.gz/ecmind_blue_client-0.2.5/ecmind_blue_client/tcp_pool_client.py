...
class TcpPoolClient(TcpClient):
    ...
    def __init__(self, connectionString: str, appname: str, username: str, password: str, use_ssl:bool=True, file_cache_byte_limit:int=33554432, pool_size:int=10):
        servers = []
        for serverString in connectionString.split("#"):
            serverParts = serverString.split(":")
            if len(serverParts) != 3:
                raise Exception(
                    f"Connection String invalid, server '{serverString}' must be formatted as hostname:port:weight. For example localhost:4000:1")
            servers.append({
