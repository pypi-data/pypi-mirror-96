...
def read_minhash(minhash_dist) :
    ids, dist = {}, []
    with gzip.open(minhash_dist, 'rt') as fin :
        for i, line in enumerate(fin) :
            if i % 1000000 == 0:
                print(i, len(dist), sys.getsizeof(dist[0]) if dist else 0)
            part = line.strip().split('\t')
            shared = [int(p) for p in part[-1].split('/')]
            ...
            while len(dist) < len(ids) :
                for d in dist :
                    d.append(shared[1])
                dist.append([shared[1] for d in dist[0]] if len(dist)>0 else [shared[1]])
            i1, i2 = ids[part[0]], ids[part[1]]
            x = shared[1]-shared[0]
            dist[i2][i1] = x
            dist[i1][i2] = x

    dist = np.array(dist, dtype=np.uint16)
...
def phierCC(profile, output, n_proc):
    ...
    ids, dist = read_minhash(profile_file)
    max_dist = np.max(dist)
    steps = np.arange(max_dist+1)

    logging.info(
        'Loaded in minhash distances of {0} genomes with maximum distance of {1}. Preparing {2} levels.'.format(
    ...
    np.savez_compressed(cluster_file, hierCC=res, ids=ids, dist=dist)

    id2 = sorted([[i, n] for n, i in ids.items()])

    with gzip.open(output + '.HierCC.gz', 'wt') as fout:
        fout.write('#Genome\t{0}\n'.format('\t'.join(['HC{0}'.format(id) for id in steps])))
