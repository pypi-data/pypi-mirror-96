...
RAY_PROJECTION_RESOLUTION = 0.5 #mm (default = 0.5)
...
if True:
    ...
    supportGenerator.rayProjectionResolution = 0.25
    supportGenerator.innerSupportEdgeGap = 0.25
    supportGenerator.outerSupportEdgeGap = 0.25
    supportGenerator.simplifyPolygonFactor = 1.5
    supportGenerator.triangulationSpacing = 1.0
...
for subregion in overhangSubregions:
    ...
    extruMesh.export('downProjExtr.off')
    subprocess.call([CORK_PATH, '-xor', 'part.off', 'downProjExtr.off', 'c.off'])
    bisectMesh = trimesh.load_mesh('c.off', process=True, validate=True)

    color3 = [random.randint(0, 255), random.randint(0, 255), random.randint(0, 255), 255]
    bisectMesh.visual.face_colors = color3
    supportExtrusionsIntersected += [bisectMesh]
    break
    ...
    upperFaces = sorted(upperFaces, key=lambda student: student.bounds[1,2], reverse=True)[:1]
    for face in upperFaces:
        continue
        i += 1

        extruFace = extrudeFace(face,-1)
        extruMesh.export('extruMesh.off')
        extruFace.export('downProjExtr.off')
        subprocess.call([CORK_PATH, '-diff', 'extruMesh.off', 'downProjExtr.off', 'c.off'])
        extruMesh = trimesh.load_mesh('c.off', process=True, validate=True)
        extruMesh.remove_degenerate_faces()
        extruMesh.remove_duplicate_faces()
        extruMesh.fix_normals()

    extruMesh.visual.face_colors = color
    #extruFace.visual.face_colors = color2
    supportExtrusionsIntersected += [extruMesh]
...
s2 = trimesh.Scene(supportExtrusionsIntersected)
s2.show()
