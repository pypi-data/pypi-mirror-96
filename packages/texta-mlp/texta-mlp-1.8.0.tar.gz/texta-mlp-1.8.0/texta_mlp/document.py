...
class Document:
    ...
    def bound_close_ones(self, facts):
        ...
        new_bound = {'doc_path': self.doc_path, 'fact': Document.FACT_NAME_BOUNDED, 'spans': [], 'str_val': {},
                     "str_values": []}
        new_bounded = []
        for fact in facts:
            if len(new_bound["spans"]) == 0:
                ...
                if abs(fact.spans[0][0] - new_bound["spans"][-1][
                    ...
                    new_bound = {'doc_path': self.doc_path, 'fact': 'BOUNDED', 'spans': [], "str_val": {},
                                 "str_values": []}
                    new_bound["spans"] += fact.spans
                    new_bound["str_values"] = [(fact.fact_value, fact.fact_type)]
                    new_bound["str_val"][fact.fact_type] = [fact.fact_value]
                ...
                for j, span2 in enumerate(new_bound["spans"]):
                    if span1[0] == span2[0] and i != j:
                        if span1[1] > span2[1]:
                            ...
                            to_delete_is += [i]
            new_bound_erased_coinciding = {'doc_path': self.doc_path, 'fact': Document.FACT_NAME_BOUNDED, 'spans': [],
                                           'str_val': {}}
            for i, span1 in enumerate(new_bound["spans"]):
    ...
    def concatenate_subset_bounds(self, new_bounded):
        ...
        for i, new_bound in enumerate(new_bounded):
            ...
            for j, another_bound in enumerate(new_bounded):
                if j not in concatenated_bound_is and i not in concatenated_bound_is and i != j and set(
                        self.key_value_single_pairs(new_bound["str_val"])) <= set(
                        self.key_value_single_pairs(another_bound["str_val"])):
                    issubset = True
