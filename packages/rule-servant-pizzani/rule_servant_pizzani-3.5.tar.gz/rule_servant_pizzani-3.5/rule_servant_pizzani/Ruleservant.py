...
#ruleset = rules.split(",")
    ...
    ruleset_dict_DS = {key:0 for key in DS_ruleset.keys()}
    for index in Rule_Efficiency["Fraud Order ID"].index:
        for oid in Rule_Efficiency["Fraud Order ID"].loc[index].split(" "):
            ruleset_dict_DS[index] += pd.Series(int(oid)).map(x["converted_transaction_amount"].to_dict())[0]
    Rule_Efficiency["DS Fraud Loss"] = Rule_Efficiency.index.map(ruleset_dict_DS)
    
    ruleset_dict_UP = {key:0 for key in UP_ruleset.keys()}
    for index in Rule_Efficiency["Fraud Order ID"].index:
        for oid in Rule_Efficiency["Fraud Order ID"].loc[index].split(" "):
            ruleset_dict_UP[index] += pd.Series(int(oid)).map(x["converted_transaction_amount"].to_dict())[0]
    Rule_Efficiency["UP Fraud Prevented"] = Rule_Efficiency.index.map(ruleset_dict_UP)
...
def rule_giver(ruleset,DS_ruleset,UP_ruleset,primary_rules,rule_scores,rule_mapping):
    ...
    count_UP_autoreject = {key:0 for key in UP_ruleset}
    count_UP_only_fraud_autoreject = {key:0 for key in UP_ruleset}
    ...
    for rul in ruleset:
      ...
      for id in primary_rules[primary_rules["Rules_Tripped_in_RTD"].str.contains(rul)]["Score_FARE"]:
        ...
        if (int(rul) in DS_ruleset.keys()) & (id >= 4000) & (pd.Series(int(rul)).map(DS_ruleset)[0] != 0):
          ...
          elif (int(rul)  in UP_ruleset.keys()) & (id >= 4000)& (pd.Series(int(rul)).map(UP_ruleset)[0] != 0)  :
            if (id + (pd.Series(int(rul)).map(UP_ruleset)[0]) >= 10000)  & (id >= 4000):
                count_UP_autoreject[int(rul)] += 1
                if (id + (pd.Series(int(rul)).map(UP_ruleset)[0]) >= 10000)  & (id >= 4000):
                count_UP_only_fraud_autoreject[rul] = primary_rules[(primary_rules["Rules_Tripped_in_RTD"].str.contains(rul)) & (primary_rules["ResolutionType"] == "Fraud") & ((primary_rules["Score_FARE"] + pd.Series(int(rul)).map(rule_scores)[0] )>=10000) & (primary_rules["Score_FARE"] >= 4000)]["Score_FARE"].count()
    ...
    fraud_orders = pd.DataFrame([[0]],index=ruleset)
    ...
    for rul in data.index:
        if primary_rules["Rules_Tripped_in_RTD"].str.contains(rul).sum() > 0:
            ...
            fraud_orders.update(pd.DataFrame([[" ".join([str(x) for x in primary_rules[(primary_rules["Rules_Tripped_in_RTD"].str.contains(rul)) & (primary_rules["ResolutionType"] == "Fraud")]["Order_ID"].values])]],index=[rul]))
    ...
    data["Real Impact DS"]= data["Real Impact DS"].fillna(0)
    ...
    data["Real Impact Autoreject"] = pd.Series([int(x) for x in data.index],index=ruleset).map(count_UP_autoreject)
    data["Real Impact Autoreject"]= data["Real Impact Autoreject"].fillna(0)


    data["Real Fraud Impact UP"] = data.index.map(count_UP_only_fraud)
    data["Real Fraud Impact DS"] = data.index.map(count_DS_only_fraud)
    data["Real Fraud Impact Autoreject"] = data.index.map(count_UP_only_fraud_autoreject)
    ...
    fraud_orders.columns = ["Fraud Order ID"]
    orders.columns = ["Order ID"]

    
    merge_0 = pd.merge(dist ,dist_fraud, left_index=True, right_index=True)
    merge_0_5 = pd.merge(fraud_orders ,orders, left_index=True, right_index=True)
    merge_1 = pd.merge(merge_0 ,merge_0_5, left_index=True, right_index=True)
