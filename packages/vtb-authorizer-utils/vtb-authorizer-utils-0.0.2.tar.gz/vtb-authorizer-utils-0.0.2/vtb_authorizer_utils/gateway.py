...
from vtb_http_interaction.services import AuthorizationService, RestService
...
class AuthorizerGateway:
    ...
    async def _get_item(self, url_path: List[str], item_id: Any, converter: Callable[[Dict[str, Any]], Any]) -> \
            Optional[Any]:
        """ Получение объекта """
        status, response = await self._create_service(url_path).get(item_id=item_id, cfg={'headers': self.headers})
    ...
    async def _get_list(self, url_path: List[str], converter: Callable[[Dict[str, Any]], Any],
        ...
        status, response = await self._create_service(url_path).get(
            cfg={'params': query_params, 'headers': self.headers})

        return list(map(converter, response['data'])) if status == 200 else None

    def _create_service(self, url_path: List[str]) -> RestService:
        """ Создание объекта RestService """
        url = _join_str(self.base_url, *url_path)

        return RestService(url) if self.access_token else AuthorizationService(url, self.keycloak_config,
