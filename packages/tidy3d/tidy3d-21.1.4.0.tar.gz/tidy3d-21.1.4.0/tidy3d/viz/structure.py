...
from ..constants import xyz_dict, xyz_list, fp_eps
from ..utils import log_and_raise


def _eps_cmap(eps_r, alpha=1, cmap=None, clim=None):
    """ Note: we use -1e6 to code for PEC and -3e6 to code for PMC.
    """
    if cmap == None:
        cmap = "Greys"
    ...
    if clim is None:
        # Get the minimum value that's larger than the -1e6 PEC value
        cl = [np.amin(eps_r[eps_r > -1e6]), np.amax(eps_r) + fp_eps]
    else:
        cl = list(clim)

    # Make sure we're larger than the -1e6 PEC value
    ...
    bounds = np.hstack((-4e6, -2e6, np.linspace(cl[0], cl[1], 256)))
...
def _structure_png(self, folder_path, aspect_ratio=None):
    ...
    for norm_ind, normal in enumerate(["x", "y", "z"]):
        ...
        ims.append(
            self.viz_eps_2D(
                normal=normal, position=self.center[norm_ind] - 0.5, ax=ax
            )
        )
        eps_r = ims[norm_ind].get_array()
        clims[norm_ind, 0] = np.amin(eps_r[eps_r > 2e-6])
        clims[norm_ind, 1] = np.amax(eps_r)
