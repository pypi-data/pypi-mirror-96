...
from rocket.daisy.utils.config import Config
from rocket.daisy.utils import loader

from rocket.daisy.utils import crypto
from rocket.daisy.devices import manager
from rocket.daisy.protocols import rest
from rocket.daisy.protocols import http
from rocket.daisy.protocols import coap
from rocket.daisy.devices.digital.gpio import NativeGPIO
from rocket.daisy.utils.logger import info, exception, debug, warn
...
class Server():
    def __init__(self, port=8000, coap_port=5683, login=None, password=None, passwdfile=None, configfile=None, scriptfile=None):
        ...
        for (name, source) in scripts:
            debug("### daisy, evosoft, loader is getting source: %s" % source)
        ...
        if http_passwdfile != None:
            if os.path.exists(http_passwdfile):
                ...
                f.close()
                if len(auth) > 0:
                    info("Access protected using %s" % http_passwdfile)
                else:
                    info("Passwd file %s is empty" % http_passwdfile)
            else:
                error("Passwd file %s not found" % http_passwdfile)
    ...
    def addMacro(self, macro):
        debug("### daisy, gulliversoft, addMacro %s" % macro)
