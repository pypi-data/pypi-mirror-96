...
def _correct_classname(cwd, logger):
    with open("idk.json", "r") as config_file:
        ...
        if not issubclass(cls, insight.InsightGenerator):
            logger.error(
                f"Can't find {config['insight_class']} in {config['insight_name']} (specified in idk.json).\n" +
                f"{30*' '}Looking for: \"idk.insight.InsightGenerator\" as a subclass of Inight class "
            )
            return False
    return True
