...
from .ofxerrors import (
    OfxParserException,
    EmptyOFXException,
    EmptyTransactionNameException,
    InvalidTransactionDateException,
    NoTransactionAmountException,
)
...
class OfxFile(object):
    def __init__(self, fh):
        ...
        if not isinstance(first, bytes):
            self.fh = six.BytesIO(self.fh.read().encode("latin-1", "ignore"))
    ...
    def read_headers(self):
        ...
        # Treat banks sending ofx in one line
        head_data_lines = head_data.splitlines()
        if len(head_data_lines) == 1:
            head_data_lines = head_data_lines[0].split(b" ")
            head_data_lines = [x for x in head_data_lines if x]

        for line in head_data_lines:
            # Newline?
            if line.strip() == six.b(""):
                break
    ...
    def handle_encoding(self):
        ...
        ascii_headers = odict.OrderedDict(
            (key.decode("ascii", "replace"), value.decode("ascii", "replace"))
        ...
        self.fh = codec.streamreader(self.fh, errors="ignore")
...
class OfxParser(object):
    ...
    def parse(cls, file_handle, fail_fast=True, custom_date_format=None):
        ...
        if ofx.find("ofx") is None:
            raise EmptyOFXException("The ofx file is empty!")
        ...
        if hasattr(bal_tag, "contents"):
            ...
            if hasattr(balamt_tag, "contents"):
                try:
                    ...
                    if cls.fail_fast:
                        setattr(statement, bal_attr, decimal.Decimal(0))
    ...
    def parseTransaction(cls, txn_ofx):
        ...
        if hasattr(name_tag, "contents"):
            try:
                ...
                raise EmptyTransactionNameException(six.u("Empty transaction name"))
            except TypeError:
                raise EmptyTransactionNameException(
        ...
        if hasattr(amt_tag, "contents"):
            try:
                ...
                if transaction.amount > 0 and transaction.type.lower() == "debit":
                    transaction.amount = -1 * transaction.amount
            except IndexError:
                raise InvalidTransactionDateException("Invalid Transaction Date")
            except decimal.InvalidOperation:
                # Some banks use a null transaction for including interest
                # rate changes on your statement.
                if amt_tag.contents[0].strip() in ("null", "-null", "False", "false"):
                    transaction.amount = 0
                else:
                    raise NoTransactionAmountException(
        ...
        if hasattr(date_tag, "contents"):
            try:
                ...
                raise InvalidTransactionDateException(
        ...
        if hasattr(user_date_tag, "contents"):
            try:
                ...
                raise InvalidTransactionDateException("Invalid Transaction User Date")
            except ValueError:
                ve = sys.exc_info()[1]
                raise OfxParserException(str(ve))
            except TypeError:
