...
def semisuperviseLabel(inifile,framedir,targets,threshold_list):
    ...
    frameDirBaseName = os.path.basename(framedir)
    ...
    class MainInterface:
        ...
        def advance_frame(self,frame):
            ...
            try:
                ...
                max_size = 1080, 650
                IDlabelLoc = []
                current_image = cv2.imread(self.framesin[self.currentframeNo])
                try:
                    maxResDimension = max(current_image.shape[1], current_image.shape[0])
                    ...
                    if animalsNo > 1:
                        ...
                        for currAnimal in IDlabelLoc:
                            cv2.putText(current_image, str(currAnimal[0]), (int(currAnimal[1]), int(currAnimal[2])), cv2.FONT_HERSHEY_COMPLEX, fontScale, currAnimal[3], 4)
                except NameError:
                    pass
                ...
                for i in self.targets:
                    ...
                    if checkStatus == 1:
                        current_image = cv2.copyMakeBorder(current_image, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=[0,0,255])
                    break
