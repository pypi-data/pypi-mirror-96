...
class SyncToLocalProcesses(object):
    ...
    def get_start_time_of_last_export(self, profileguid, dbtools):
        # type: (str, DB_Tools) -> str
        """
        ...
        sql_ = """
            SELECT TOP (1) taskid FROM assetic_sync_manager
            WHERE profileid = '{0}'
            ORDER BY date_initiated DESC
        """.format(profileguid) # todo is there a scenario where this would fail if there is something else in progress?

        error_code, result = dbtools.execute_select(sql_)

        if len(result) == 0:
            # e.g. a row hasn't been added for this profile before
            return datetime.datetime(2000, 1, 1, 0, 0, 0).isoformat()

        bg_worker_guid = result[0][0]
    ...
    def profile_contains_modified_column(self, profile):
        ...
        return profile.lastmodified_property_value in [l['Label'] for l in resp['ResourceList']]
    ...
    def add_non_processed_syncrow(self, dbtools, profile):
        ...
        sql_select = """
			...
			date_initiated, date_last_modified, status
        ...
        sql_insert = """
            ...
            ,status,date_initiated,date_last_modified, documentid)
            values (NEWID(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """
    ...
    def dbsync_initiate(self, dbserver, dbname, profiles, incremental=True,
        ...
        for profile in profiles:
            ...
            if contains_lastmodified:
                ...
                start_time = self.get_start_time_of_last_export(profile.profileguid, dbtools)

                # get number of records where ComplexAssetLastModified greater than
                # start time of previous export
                ...
                if modified_records == 0:
                    e = ("Not initiating bulk export of profile {0} "
                         "({1}) as no information has changed since last export "
                         "on {2}.").format(
...
class SearchProfileRepresentation(object):
    ...
    def __init__(self, profileguid=None, profilename=None, tablename=None,
        ...
        } # todo ValuationLastModified, AssessmentLastModified (not in oData, so look up advanced search profile)
