...
class PartSegmentationIoU(Metric):
    def __init__(self, use_only_category_logits: bool, category_to_parts_map: Dict[str, List[int]]):
        ...
        for b in range(batch_size):
            ...
            for p in parts:
                ...
                if union == 0:
                    ...
                    part_ious[p - parts[0]] = float(intersection) / float(union)

            self.ious[cat].append(sum(part_ious) / len(part_ious))
    ...
    def compute(self) -> Tuple[float, float]:
        ...
        for cat in self.ious:
            ...
            mIoUs[cat] = sum(self.ious[cat]) / len(self.ious[cat])

        mIoU_per_cat = list(mIoUs.values())
        mean_class_iou = sum(mIoU_per_cat) / len(mIoU_per_cat)

        mean_instance_iou = sum(instance_ious) / len(instance_ious)
