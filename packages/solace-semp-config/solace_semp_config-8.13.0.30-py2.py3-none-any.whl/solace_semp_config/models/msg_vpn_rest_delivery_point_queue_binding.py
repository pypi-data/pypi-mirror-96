...
class MsgVpnRestDeliveryPointQueueBinding(object):
    ...
    def __init__(self, gateway_replace_target_authority_enabled=None, msg_vpn_name=None, post_request_target=None, queue_binding_name=None, rest_delivery_point_name=None):  # noqa: E501
    ...
    def gateway_replace_target_authority_enabled(self):
        ...
        Enable or disable whether the authority for the request-target is replaced with that configured for the REST Consumer remote. When enabled, the router sends HTTP requests in absolute-form, with the request-target's authority taken from the REST Consumer's remote host and port configuration. When disabled, the router sends HTTP requests whose request-target matches that of the original request message, including whether to use absolute-form or origin-form. This configuration is applicable only when the Message VPN is in REST gateway mode. The default value is `false`. Available since 2.6.  # noqa: E501
    ...
    def queue_binding_name(self):
        ...
        The name of a queue within this Message VPN.  # noqa: E501
