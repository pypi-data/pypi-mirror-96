...
class DmrClusterLink(object):
    ...
    def queue_reject_msg_to_sender_on_discard_behavior(self):
        ...
        The message discard behavior, providing the condition for returning a negative acknowledgement (NACK) to the sender. The default value is `\"always\"`. The allowed values and their meaning are:  <pre> \"always\" - Always return a negative acknowledgment (NACK) to the sending client on message discard. \"when-queue-enabled\" - Only return a negative acknowledgment (NACK) to the sending client on message discard when the Queue is enabled. \"never\" - Never return a negative acknowledgment (NACK) to the sending client on message discard. </pre>   # noqa: E501
