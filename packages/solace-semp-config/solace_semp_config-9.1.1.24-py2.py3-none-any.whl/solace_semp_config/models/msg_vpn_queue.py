...
class MsgVpnQueue(object):
    ...
    def dead_msg_queue(self):
        ...
        The name of the Dead Message Queue (DMQ) used by the Queue for discarded messages. The default value is `\"#DEAD_MSG_QUEUE\"`. Available since 2.2.  # noqa: E501
    ...
    def reject_msg_to_sender_on_discard_behavior(self):
        ...
        Enable or disable the return of negative acknowledgements (NACKs) to sending clients on message discards. Note that NACKs cause the message to not be delivered to any destination and transacted-session commits to fail. The default value is `\"when-queue-enabled\"`. The allowed values and their meaning are:  <pre> \"always\" - Always return a negative acknowledgment (NACK) to the sending client on message discard. \"when-queue-enabled\" - Only return a negative acknowledgment (NACK) to the sending client on message discard when the Queue is enabled. \"never\" - Never return a negative acknowledgment (NACK) to the sending client on message discard. </pre>  Available since 2.1.  # noqa: E501
