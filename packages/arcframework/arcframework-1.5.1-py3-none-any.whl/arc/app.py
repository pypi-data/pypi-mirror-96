...
class App:
    def __init__(self, templates_dir="templates", static_dir="static", exception_handler=None, host="127.0.0.1", port=5000, default_middleware=True):
        ...
        # path = os.path.join(self.dir_path, "selfpages")
        # dashboard_env = Environment(
        #     loader=FileSystemLoader(os.path.join(self.dir_path, "selfpages")))
        
        # def default_app_dashboard(req, res):
        #     res.body = dashboard_env.get_template("default-dashboard.html").render({"app_host": self.host, "app_port": self.port}).encode()
        
        # self.add_route("/default-app-dashboard", default_app_dashboard)
