...
class App:
    ...
    def run(self):
        ...
        print(f"{inspect.stack()[1][1].replace('py', '')}:{self.name}")
        uvicorn.run(f'{inspect.stack()[1][1].replace(".py", "")}:{self.name}', host="127.0.0.1", port=5000, log_level="error", interface="wsgi")
