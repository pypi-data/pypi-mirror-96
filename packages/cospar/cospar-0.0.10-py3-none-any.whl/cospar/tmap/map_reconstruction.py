        ...
        for iRound in range(round_of_smooth):
            ...
            if save_subset: 
                if SM%5==0: # save when SM=5,10,15,20,...
                    ...
                    logg.info("Save the matrix at every 5 rounds")
                    ssp.save_npz(file_name+f'_SM{SM}.npz',similarity_matrix)
            else: # save all
                
                logg.info("Save the matrix at every round")
                ssp.save_npz(file_name+f'_SM{SM}.npz',similarity_matrix)
...
def infer_Tmap_from_multitime_clones(adata_orig,clonal_time_points,
    ...
    extend_Tmap_space: `bool` optional (default: `False`)
        ...
        whether they have clonal barcodes or not. Otherwise, the initial and later state
        space of the Tmap will be restricted to cells with multi-time clonal information
        alone. The latter case usually speeds up the computation, which is recommended. 
    save_subset: `bool`, optional (default: True)
        If true, save only Smatrix at smooth round [5,10,15,...];
        Otherwise, save Smatrix at each round. 
...
def Tmap_from_highly_variable_genes(adata,min_counts=3,min_cells=3,
    ...
    noise_threshold=0.2,normalization_mode=1,use_full_Smatrix=True,
...
def infer_Tmap_from_one_time_clones_private(adata,initialized_map,Clone_update_iter_N=1,
    ...
    use_full_Smatrix=True,trunca_threshold=0.001,compute_new=True,
