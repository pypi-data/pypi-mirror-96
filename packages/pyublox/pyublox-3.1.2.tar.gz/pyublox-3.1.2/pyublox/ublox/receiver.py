...
class Receiver(threading.Thread):
    ...
    def __submit_configuration__(self, doc=None, ucenter_file=None):
        ...
        try:
            ...
            need_to_restart_logging = False
            if self.is_alive():
                logger.warning("Stopping recording session in order to submit configuration")
                self.stop()
                need_to_restart_logging = True
    ...
    def reset(self):
        ...
        if self.is_alive():
            logger.warning("Stopping recording session in order to reset receiver")
            self.stop()
    ...
    def run(self):
        ...
        self.serial_stream_for_recording = serial.Serial(self.serial_port, self.baudrate)
        if self.serial_stream_for_recording:

            incoming_epoch_suffix = None
            num_packets = 0
            for packet in core.ublox_packets(self.serial_stream_for_recording, core.TIMEOUT_DEF):
    ...
    def stop(self):
        ...
        if self.serial_stream_for_recording:
            self.serial_stream_for_recording.close()
            serial_port = self.connection_config.get(SERIAL_PORT_STR, None)
            logger.info('Closed serial stream from [ {} ]'.format(serial_port))
