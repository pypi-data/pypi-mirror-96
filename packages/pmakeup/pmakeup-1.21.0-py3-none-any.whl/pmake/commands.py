...
from pmakeup import LinuxOSSystem, version
from pmakeup import PMakeupModel
from pmakeup import WindowsOSSystem
...
class SessionScript(abc.ABC):
    ...
    def __init__(self, model: "PMakeupModel"):
        ...
        if self.on_windows():
            self._platform = WindowsOSSystem.WindowsOSSystem(model)
        elif self.on_linux():
            self._platform = LinuxOSSystem.LinuxOSSystem(model)
        else:
    ...
    def ensure_condition(self, condition: Callable[[], bool], message: str = "") -> None:
        ...
        if not condition():
            raise AssertionPMakeupException(f"pmake needs to generate a custom exception: {message}")
    ...
    def semantic_version_2_only_core(self, filename: str) -> Version:
        ...
        return Version.parse(m.group(0))
    ...
    def require_pmake_version(self, lowerbound: str) -> None:
        ...
        """
        system_version = Version.parse(version.VERSION)
        script_version = Version.parse(lowerbound)
            ...
            for subfolder in self.ls_only_directories(p):
                ...
                try:
                    if folder_format == "semver2":
                        folders[Version.parse(subfolder_id)] = subfolder
