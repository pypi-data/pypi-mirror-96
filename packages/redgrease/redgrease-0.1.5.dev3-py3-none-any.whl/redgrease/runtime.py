...
from redgrease.gears import GearFunction, PartialGearFunction
...
def execute(command: Union[str, GearFunction], *args) -> GearFunction:
    ...
    """
    if isinstance(command, GearFunction):
        if command.input_function:
            input_builder = execute(command.input_function, args[0])
        else:
            input_builder = args[0] if args else GearsBuilder
        return command.operation.add_to(input_builder)
    else:
