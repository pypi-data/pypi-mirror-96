...
class OverseaFundNavAnalysis:
    ...
    def process_recent_ret(self):
        try:
            ...
            end_date = self.fund_nav.index[-1]
            for fund_id in self.fund_nav:
                nav = self.fund_nav[[fund_id]].dropna()
                ret_i = CalculatorBase.calc_recent_ret(dates=nav.index,values=nav[fund_id].values)
                ret_i['datetime'] = end_date
                ret_i['codes'] = fund_id
                res.append(ret_i)
            df = pd.DataFrame(res)
            self._data_helper._upload_raw(df, QSFundRecentRet, to_truncate=True)
        except Exception as e:
            print(e)
            traceback.print_exc()
            return False