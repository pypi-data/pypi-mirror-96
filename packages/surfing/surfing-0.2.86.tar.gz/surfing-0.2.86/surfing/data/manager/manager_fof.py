...
class FOFDataManager(metaclass=Singleton):
        ...
        if is_from_excel:
            ...
            for date in self._date_list:
                try:
                    ...
                    investor_pos_list.append(scale_data.loc[scale_data.share.isnull(), ['fof_id', 'investor_id', 'amount', 'share']])

                    # 汇总今天所有的申购资金
                    total_amount = scale_data.amount.sum()
                    if not math.isclose(total_amount, 0):
                        share_increased = scale_data.share.sum()
                    # try:
                    #     # 用申购金额除以确认日的净值以得到份额
    ...
    def dump_fof_nav_and_pos_to_db(self, debug_mode=False) -> bool:
        ...
        nav = fof_nav.array[-1]
        # TODO: 减去赎回的份额
        mv = self._fof_scale.share.sum() * nav
        ...
        for fund_id in fund_list:
            try:
                ...
                if not fund_auv.empty:
                    res_status = Calculator.get_stat_result(fund_auv.index, fund_auv.array)
                    mv = (pur_sub_aa.share.sum() - redempt_aa.share.sum() - aa_insentive_info.share.sum()) * fund_nav.net_asset_value.array[-1]
                    ...
                    holding_data = {
                        ...
                        'v_nav': round(fund_nav.v_net_value.array[-1], 4),
    ...
    def get_investor_return(fof_id: str, *, investor_id_list: Tuple[str] = (), start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None) -> Optional[pd.DataFrame]:
        ...
        for row in fof_scale_info.itertuples(index=False):
            ...
            try:
                ...
                init_water_line = 1
            investor_mvs[row.investor_id].append({'start_date': actual_start_date, 'amount': row.amount, 'share': row.share, 'init_mv': init_mv, 'init_water_line': init_water_line})
...
if __name__ == "__main__":
    ...
    # fof_dm.pull_hedge_fund_navs()
    fof_dm.calc_fof_nav(debug_mode=True)
    # fof_dm.calc_all()
