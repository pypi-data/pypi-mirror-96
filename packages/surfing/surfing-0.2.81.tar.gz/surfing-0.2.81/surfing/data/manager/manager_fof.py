...
from ...constant import FundStatus, HoldingAssetType, FOFStatementStatus
...
class FOFDataManager(metaclass=Singleton):
        ...
        for row in nav.itertuples():
            try:
                ...
                for one_in_date in one.itertuples():
                    if one_in_date.status == FundStatus.INSENTIVE_DONE:
                    ...
                    if one_in_date.status == FundStatus.DONE:
    ...
    def init(self, debug_mode=False):
        def _calc_water_line_and_confirmed_nav(x):
            ...
            return pd.Series({'confirmed_nav': json.dumps(x[['unit_total', 'nav']].to_dict(orient='records')),
                              'water_line': json.dumps(x[['unit_total', 'water_line']].to_dict(orient='records'))})
        ...
        # 这里不需要在途的持仓
        positions = self._asset_allocation.loc[self._asset_allocation.status == FundStatus.DONE, :]
    ...
    def calc_fof_nav(self, dump_to_db=True, is_from_excel=False, debug_mode=False):
        ...
        if is_from_excel:
            ...
            fof_position_details_list = [asset_type, all_pos, self._total_cost.rename('total_cost'), self._all_latest_nav.rename('nav'),
                                         self._all_latest_acc_nav.rename('acc_nav'), self._hedge_latest_v_nav.rename('v_nav'), latest_mv.rename('latest_mv')]
            # for one in fof_position_details_list:
            #     print(one)
    ...
    def dump_fof_nav_and_pos_to_db(self, debug_mode=False) -> bool:
        ...
        if investor_return_df is not None:
            ...
            if now_df is not None:
                ...
                # merge on all columns
                df = investor_return_df.drop(columns=['total_rr', 'v_nav']).reset_index().round(4).merge(now_df.round(4), how='left', indicator=True, validate='one_to_one')
                df = df[df._merge == 'left_only'].drop(columns=['_merge'])
            if debug_mode:
                print(df)
            if not df.empty:
        ...
        for fund_id in fund_list:
            try:
                ...
                holding_data = {
                    ...
                    'sharpe': round(res_status.sharpe, 6),
                    'annualized_ret': f'{round(xirr(aa_info.amount.to_list() + [-mv], aa_info.confirmed_date.to_list() + [fund_nav.datetime.array[-1]]) * 100, 2)}%',
                    'mdd': f'{round(res_status.mdd * 100, 2)}%',
                    'annualized_vol': round(res_status.annualized_vol, 6),
                }
