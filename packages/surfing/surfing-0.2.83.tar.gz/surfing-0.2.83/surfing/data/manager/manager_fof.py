...
from ..view.basic_models import HedgeFundInfo, FOFInfo, FOFManually, HedgeFundNAV, FOFInvestorPosition, FOFEstimateFee, FOFEstimateInterest, FOFTransitMoney
...
class FOFDataManager(metaclass=Singleton):
        ...
        for row in nav.itertuples():
            try:
                ...
                for one_in_date in one.itertuples():
                    if one_in_date.event_type in (FOFTradeStatus.DEDUCT_REWARD_AND_DIVIDEND_VOLUME, FOFTradeStatus.DEDUCT_REWARD) and one_in_date.status == FundStatus.INSENTIVE_DONE:
                    ...
                    if one_in_date.event_type in (FOFTradeStatus.PURCHASE, FOFTradeStatus.SUBSCRIBE, FOFTradeStatus.DIVIDEND_VOLUME) and one_in_date.status == FundStatus.DONE:
                        datas_to_calc_v[one_in_date.fund_id].append((one_in_date.fund_id, one_in_date.Index, one_in_date.unit_total, one_in_date.water_line))
    ...
    def init(self, debug_mode=False):
        def _calc_water_line_and_confirmed_nav(x):
            ...
            return pd.Series({'confirmed_nav': json.dumps(x[['unit_total', 'nav']].to_dict(orient='records')),
                              'water_line': json.dumps(x_with_water_line[['unit_total', 'water_line']].to_dict(orient='records'))})
    ...
    def calc_fof_nav(self, dump_to_db=True, is_from_excel=False, debug_mode=False):
        ...
        fund_nav = fund_nav.pivot(index='datetime', columns='fund_id').reindex(index=self._date_list).ffill()
        ...
        if is_from_excel:
            ...
            for date in self._date_list:
                try:
                    ...
                    if not math.isclose(total_amount, 0):
                        share_increased = scale_data.unit_total.sum()
                    # try:
                    #     # 用申购金额除以确认日的净值以得到份额
                    ...
                    if cash_list.empty:
                        ...
                        if date == deposited_date:
                            ...
                            total_cash = cash_list.iat[-1]
                            if not pd.isnull(deposited_date):
                ...
                try:
                    ...
                    for row in today_asset_alloc.itertuples():
                        ...
                        if row.event_type in (FOFTradeStatus.DEDUCT_REWARD, FOFTradeStatus.DEDUCT_REWARD_AND_DIVIDEND_VOLUME):
                ...
                if incidental_trading is not None:
                    fee_transfer = incidental_trading.loc[incidental_trading.event_type.isin([FOFStatementStatus.OUT_MANAGEMENT, FOFStatementStatus.OUT_CUSTODIAN, FOFStatementStatus.OUT_ADMINISTRATIVE]), 'amount']
                    if not fee_transfer.empty:
                        fee_transfer = round(fee_transfer.sum(), 2)
                        total_cash -= fee_transfer
                        misc_fees -= fee_transfer
            ...
            fof_position_details_date = self._all_latest_nav.name
            fof_position_details_list = [asset_type, all_pos, total_ret.rename('total_ret'), total_rr.rename('total_rr'), self._total_cost.rename('total_cost'), self._all_latest_nav.rename('nav'),
            ...
            self._fof_position_details['datetime'] = fof_position_details_date
    ...
    def dump_fof_nav_and_pos_to_db(self, debug_mode=False) -> bool:
        ...
        fof_nav = fof_nav.nav
        nav = fof_nav.array[-1]
        mv = (self._fof_scale.unit_total.sum() - self._fof_scale.share.sum()) * nav
        ...
        if self._fof_position_details is not None:
            ...
            if now_df is not None:
                ...
                # merge on all columns
                df = self._fof_position_details.merge(now_df, how='left', indicator=True, validate='one_to_one')
                df = df[df._merge == 'left_only'].drop(columns=['_merge'])
            else:
        ...
        if investor_return_df is not None:
            investor_return_df = investor_return_df.drop(columns=['total_rr', 'latest_mv', 'v_nav'])
            now_df = FOFDataManager().get_fof_investor_position([FOFDataManager._FOF_ID])
            if now_df is not None:
                now_df = now_df.drop(columns=['v_nav', 'update_time', 'create_time', 'is_deleted'])
        ...
        for fund_id in fund_list:
            try:
                ...
                if not fund_auv.empty:
                    res_status = Calculator.get_stat_result(fund_auv.index, fund_auv.array)
                    mv = (pur_sub_aa.unit_total.sum() - redempt_aa.share.sum() - aa_insentive_info.share.sum()) * fund_nav.net_asset_value.array[-1]
                    total_cost = pur_sub_aa.amount.sum()
                    holding_data = {
                        'start_date': res_status.start_date.isoformat(),
                        'days': (fund_auv.index.array[-1] - res_status.start_date).days,
                        'mv': mv,
                        # TODO 这里需要考虑正常赎回的情况以及现金分红的情况
                        'total_ret': f'{round((mv / total_cost - 1) * 100, 2)}%',
                        'avg_cost': round(total_cost / (pur_sub_aa.unit_total.sum() - aa_insentive_info.share.sum()), 4),
                        'sharpe': round(res_status.sharpe, 6),
    ...
    def get_investor_return(fof_id: str, *, investor_id_list: Tuple[str] = (), start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None) -> Optional[pd.DataFrame]:
        ...
        for investor_id, datas in investor_mvs.items():
            ...
            v_nav = FOFDataManager._do_calc_v_net_value(latest_nav, datas.share, latest_nav, datas.init_water_line, 0.2, 4)
            ...
            if not math.isclose(amount_sum, 0):
                total_rr = latest_mv / amount_sum - 1
            else:
                total_rr = np.nan
            if not math.isclose(total_share, 0):
                avg_nav_cost = amount_sum / total_share
            else:
                avg_nav_cost = np.nan
            investor_returns[investor_id] = {'datetime': datas.start_date.array[-1], 'fof_id': fof_id, 'v_nav': avg_v_nav, 'cost_nav': avg_nav_cost, 'total_rr': total_rr, 'amount': amount_sum, 'shares': total_share, 'latest_mv': latest_mv}
