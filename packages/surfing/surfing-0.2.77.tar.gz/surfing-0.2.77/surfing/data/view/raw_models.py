...
class QSFundRecentRet(Base):
    ...
    codes = Column(CHAR(15), primary_key=True) # 基金代码
    datetime = Column(DATE, primary_key=True) # 日期标签
    recent_natural_year_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近自然年收益
    recent_1w_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近一周收益
    recent_1m_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近一月收益
    recent_3m_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近三月收益
    recent_6m_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近六月收益
    recent_1y_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近一年收益
    recent_2y_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近两年收益
    recent_3y_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近三年收益
    recent_4y_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近四年收益
    recent_5y_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 最近五年收益
    history_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 历史收益
    history_annual_ret = Column(DOUBLE(asdecimal=False), nullable=False) # 历史年化收益