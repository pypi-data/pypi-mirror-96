...
def check_and_validate_fname(fname, target_directory=None, rename=None):
    ...
    if target_directory == None:
        ...
        if type(target_directory) != Path:
            logger.error(f'target directory must be a valid Path, got {type(target_directory)}')
    if rename == None:
    ...
    unique_name = Path(target_directory, clean_name)
    if unique_name.is_file():
        if rename:
            unique_name = new_name_if_exists(unique_name)
        else:
            return None
    return unique_name
