...
from .util import Run, Solution, run_to_string
...
def reduce_and_solve(s, solver, verbosity=0):
    ...
    if verbosity >= 1:
        print(
            "Solving instance: {} ({} runs, {} characters)".format(
                run_to_string(s), len(s), len(set([run.char for run in s]))
            )
        )
...
def solve_instance(s, solver, verbosity=0):
    if verbosity >= 1 and len(s) > 1:
        print(
            "Solving sub-instance: {} ({} runs, {} characters)".format(
                run_to_string(s), len(s), len(set([run.char for run in s]))
            )
        )
...
def get_first_and_last_occurences(s):
    """
    Returns two dictionaries, which for each character in s contain the first
    and last run index (respectively) of its occurences
    """
    first_occ = dict()
    last_occ = dict()
...
def reduce_concat(s, solver, verbosity=0):
    ...
    # take first run and extend it to the left, until secluded prefix is found
    pos = 0
    current_end = 0
    while pos < len(s):
        # invariant: all positions before current_end (exclusive) have been
        # processed
        current_start = current_end
        current_end = last_occ[s[current_start].char] + 1
        pos = current_start + 1
        while pos < current_end:
            # invariant: all runs before pos (exclusive) have been used to
            # extend the interval
        ...
        partial_solutions.append(
            reduce_nested(s[current_start:current_end], solver, verbosity)
        )
...
def reduce_nested(s, solver, verbosity=0):
    ...
    # 2. for every other character inside the current interval, extend borders
    ...
    for char in sigma:
        ...
        # initialize the candidate interval with first and last occurence of
        # examined char
        left_bound = first_occ[char]
        right_bound = last_occ[char] + 1  # right_bound is exclusive
        chars_in_interval = set([char])

        # these pointers indicate up to which position (from left_bound) we
        # already read
        left = left_bound - 1
        right = left_bound + 1

        # if both pointers reached the bounds, the interval is closed
        # borders itself must be characters we have already seen, so no check
    ...
    # sort independent intervals by position (they cannot overlap, so left
    # bound is sufficient)
    independent.sort(key=lambda interval: interval[0])

    # contract adjacent independent intervals and run concat-rule on them
    # again, if longer than one block
    ...
    for interval in independent:
        if interval[0] == right:
            ...
            # interval does not extend current sub-instance. solve latter
            # (if not singleton block) and open new sub-instance
            if right - left >= 2:
                intervals.append((left, right))
                partial_solutions.append(
                    reduce_concat(s[left:right], solver, verbosity)
                )
            left = interval[0]
            right = interval[1]
    ...
    # again, if no sub-instance was solved, solve entire string as one instance
    if len(intervals) == 0:
        return solve_instance(s, solver, verbosity)

    # replace all sub-instance intervals with a single run of a new character
    # length = solution size of sub-instance
    s0 = []
    pos = 0
    ...
    # for every solution in modified instance: replace any run of $-chars with
    # all solutions of the respective sub-instance
    final_subseqs = []
    for seq in sol.subsequences:
        # start with just the solution of the outer problem
        expanded_solutions = [seq]
        for i in range(len(intervals)):
            # replace placeholder block for every solution in
            # expanded_solutions with every sub-instance solution
            ...
            for expanded_solution in expanded_solutions:
                ...
                if idx >= 0:
                    for subsol in partial_solutions[i].subsequences:
                        expanded_expanded_solutions.append(
                            expanded_solution[:idx]
                            + subsol
                            + expanded_solution[idx + 1:]
                        )
                else:
