...
class IptSplitOverlappedEllipses(IptBaseAnalyzer):
    ...
    def hough_detection(self, wrapper, mask, contours):
        ...
        return {"count": total_ellipses, "demo_image": dbg_img}
    ...
    def median_area_detection(self, wrapper, mask, contours):
        ...
        return {"count": total_ellipses, "demo_image": img}
    ...
    def concave_detection(self, wrapper, mask, contours):
        ...
        approx_factor = self.get_value_of("approx_factor")
        total_ellipses = 0
        dbg_img = np.dstack(
            (
                np.zeros_like(mask),
        ...
        for cnt in contours:
            ...
            if handled_ is True:
                ...
                work_img = self.apply_morphology(
                    mask=work_img[y : y + h, x : x + w][:, :, 0],
                    morph_op="erode",
                    kernel_size=5,
                )
                wrapper.store_image(
                    image=work_img,
                    text=f"object at , x:{x}, y:{y}, w:{w}, h:{h}",
                )
                count_ = len(
                    [
                        c
                        ...
                        and (cv2.contourArea(c) >= residue_size)
                    ]
                )
                if count_ < 2:
                    count_ += 1
