...
def determine_symlink_mode(symlink_arg):
    if symlink_arg == 'yes':
        return True
    elif symlink_arg == 'no':
        return False
...
def main():
    ...
    if args.action == 'install':
        ...
        print("Installing Jupyhai notebook extension ({using_symlinks})...".format(
            using_symlinks=("using symlinks" if symlink_mode else "without symlinks"),
        ))
        serverextensions.toggle_serverextension_python('jupyhai', enabled=True)
        nbextensions.install_nbextension_python('jupyhai', symlink=symlink_mode, user=True)
