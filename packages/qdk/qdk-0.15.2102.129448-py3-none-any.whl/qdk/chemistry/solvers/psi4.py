...
def create_input_deck(
    ...
    :param symmetry: Molecule symmetry
    (example options: "C1", "C2v", "D6h", "Cs"),
         defaults to "C1". When requesting Broombridge, make sure to set this
         to "C1" because Broombridge does not support group symmetry
    :type symmetry: str, optional
    :param method: Molecule method (options: "SCF", "HF", "DFT", "CC2",
        "CCSD", "CASSCF", "MP2"), defaults to "SCF"
    :type method: str, optional
    :param driver: Driver method ("energy" or "optimize"), defaults to "energy"
    :type driver: str, optional
    :param scf_type: SCF solver type ("DIRECT", "DF", "PK",
        "OUT_OF_CORE" or "PS"), defaults to "PK"
    ...
    :rtype: str
    """
    assert basis in Basis._value2member_map_, f"Unknown basis: {basis}. \
        Please choose from the following: {Basis._value2member_map_.values()}"
    optional_energy = PSI4_TEMPLATE_ENERGY.format(
        driver=driver, method=method) if method.upper() != "SCF" else ""
    template = PSI4_TEMPLATE_FCIDUMP.format(
        driver=driver, optional_energy=optional_energy)
