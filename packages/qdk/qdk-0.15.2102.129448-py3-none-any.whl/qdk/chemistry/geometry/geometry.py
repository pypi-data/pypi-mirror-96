...
"""Module that defines the Geometry and Element classes for keeping track of
XYZ data for molecules
"""
...
from qdk.chemistry.geometry.rdkit_convert import _mol_to_coordinates
from qdk.chemistry.geometry.xyz import (
    coordinates_to_xyz,
    element_coords_to_xyz
)
...
XYZ_PATTERN = f"(\\w)\\s+{FLOAT_PATTERN}\\s+{FLOAT_PATTERN}\\s+{FLOAT_PATTERN}"
ATOMS_PATTERN = "\\s*(?P<num_atoms>\\d+)\\s*\n"


@dataclass
class Element:
    """Class for keeping track of element XYZ
    coordinates for molecule geometry
    """
    name: str
    x: float
    y: float
    ...
    def from_tuple(cls, value: Tuple[str, float, float, float]):
        ...
        :param value: Tuple with values (element name,
...
class Geometry(List[Element]):
    ...
    def from_mol(cls, mol: "Mol", num_confs: int = 10):
        """Classmethod for constructing a geometry instance from an RDKit
        molecule object

        :param mol: RDKit molecule object
        :type mol: Mol
        :param num_confs: Number of molecular conformers to generate, defaults
            to 10
    ...
    def from_xyz(cls, xyz: str):
        ...
        num_atoms = re.match(ATOMS_PATTERN, xyz)
...
number of atoms '{num_atoms.group('num_atoms')}' \
does not match number of elements found {len(match)}"
