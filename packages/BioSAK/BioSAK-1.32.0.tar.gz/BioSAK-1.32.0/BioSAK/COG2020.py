...
wget https://ftp.ncbi.nih.gov/pub/COG/COG2020/data/cog-20.org.csv
wget https://ftp.ncbi.nih.gov/pub/COG/COG2020/data/cog-20.patt.txt
wget https://ftp.ncbi.nih.gov/pub/COG/COG2020/data/cog-20.tax.csv
...
def COG2020(args):
    ...
    # # check whether db index exist
    # if run_diamond is False:
    #     unfound_db_index_file = []
    #     for db_index in ['phr', 'pin', 'pog', 'psd', 'psi', 'psq']:
    #         pwd_db_index = '%s.*%s' % (pwd_cog_20_fa, db_index)
    #         if not os.path.isfile(pwd_db_index):
    #             unfound_db_index_file.append(db_index)
    #     if len(unfound_db_index_file) > 0:
    #         print(datetime.now().strftime(time_format) + 'DB index files not found, please refers to the help info for db preparation')
