...
import twine2
from twine2 import _installed
from twine2 import importlib_metadata

args = argparse.Namespace()


def _registered_commands(
    group: str = "say4.registered_commands",
...
def dispatch(argv: List[str]) -> Any:
    ...
    print(registered_commands)
    # parser = argparse.ArgumentParser(prog="twine")
    parser = argparse.ArgumentParser(prog="say5")
    parser.add_argument(
        "--version",
        action="version",
        version="%(prog)s version {} ({})".format(twine2.__version__, dep_versions()),
    )
    parser.add_argument(
        "--no-color",
        default=False,
