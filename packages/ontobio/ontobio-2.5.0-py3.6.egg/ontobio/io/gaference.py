...
def load_gaferencer_inferences_from_files(gaferencer_out_list: List[os.PathLike], fail_on_error=False) -> Dict[AnnotationKey, InferenceValue]:
    loaded_inferences = dict() # type: Dict[AnnotationKey, InferenceValue]

    for inference_file in gaferencer_out_list:
        inferences = load_gaferencer_inferences_from_file(inference_file)
        if inferences is not None:
            loaded_inferences.update(inferences)
        else:
            if fail_on_error:
                return None

    return loaded_inferences
