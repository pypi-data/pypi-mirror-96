...
class LogisticRegression(GeneralizedLinearModelClassifier, BaseEstimator, ClassifierMixin):
    ...
    def param_check(self):
        for varname, value in self.__dict__.items():
            if varname not in ['is_regression', 'params', 'standard_predict_type',
                               'model_size', 'labels_flag', 'n_features_in_']:
    ...
    def predict_proba(self, X, n_jobs=0):
        ...
        is_single, proba, pred_vars = self.pre_predict_is_single(X, _mpi_enabled, PredictTypes.logistic_probabilities)

        if is_single == False:
            # Multi-row or snap-data or MPI
            proba, proba_metadata = libsnapml.lr_predict_proba(pred_vars.num_ex,
                                                               pred_vars.num_ft,
                                                               pred_vars.indptr,
                                                               pred_vars.indices,
                                                               pred_vars.data,
