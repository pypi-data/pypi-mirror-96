...
def copy_as_gpu_cmatrix(in_cuDF):
    ...
    if((len(in_cuDF.dtypes.unique()) != 1) or
       (np.dtype('float32') not in in_cuDF.dtypes.unique())):
        raise TypeError("snapML only supports float32 dtype. Found dtypes : {}.".format(in_cuDF.dtypes))

    from snap_ml.utils_gpu import _create_device_array, _copy_DeviceNDArray, _cuda_synchronize
