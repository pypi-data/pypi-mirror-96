...
class Heating:
    ...
    def get_heating(self, device):
        ...
        if device["deviceData"]["online"]:
            ...
            dev_data = {
                ...
                "min_temp": self.min_temperature(device),
                "max_temp": self.max_temperature(device),
                "status": {
                    "current_temperature": self.current_temperature(device),
                    "target_temperature": self.target_temperature(device),
                    "action": self.current_operation(device),
                    "mode": self.get_mode(device),
                    "boost": self.boost(device),
                },
    ...
    def min_temperature(self, device):
    ...
    def minmax_temperatures(self, device):
        """Min/Max Temp."""
        state = None
        final = None
    ...
    def target_temperature(self, device):
    ...
    def get_state(self, device):
        ...
        try:
            current_temp = self.current_temperature(device)
            target_temp = self.target_temperature(device)
    ...
    def current_operation(self, device):
    ...
    def get_boost_time(self, device):
        """Get heating boost time remaining."""
        if self.boost(device) == "ON":
            state = None
    ...
    def get_schedule_now_next_later(self, device):
        ...
        online = self.session.attr.online_offline(device["device_id"])
        current_mode = self.get_mode(device)
        state = None
    ...
    def set_target_temperature(self, device, new_temp):
    ...
    def set_mode(self, device, new_mode):
        """Set heating mode."""
        self.session.hiveRefreshTokens()
        final = False
    ...
    def turn_boost_on(self, device, mins, temp):
        """Turn heating boost on."""
        if mins > 0 and temp >= self.min_temperature(device):
            if temp <= self.max_temperature(device):
                self.session.hiveRefreshTokens()
                final = False
    ...
    def turn_boost_off(self, device):
