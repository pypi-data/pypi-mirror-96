...
class Light:
        ...
        if device["deviceData"]["online"]:
            ...
            dev_data = {
                ...
                "status": {
                    ...
                    "brightness": self.get_brightness(device),
                },
            ...
            if device["hiveType"] in ("tuneablelight", "colourtuneablelight"):
                dev_data.update(
                    {
                        "min_mireds": self.get_min_color_temp(device),
                        "max_mireds": self.get_max_color_temp(device),
                    }
                )
                dev_data["status"].update(
                    {"color_temp": self.get_color_temp(device)}
                )
            if device["hiveType"] == "colourtuneablelight":
                mode = self.getColourMode(device)
                if mode == "COLOUR":
                    dev_data["status"].update(
                        {
                            "hs_color": self.get_color(device),
                            "mode": self.getColourMode(device),
                        }
                    )
                else:
    ...
    def get_min_color_temp(self, device):
    ...
    def get_max_color_temp(self, device):
    ...
    def get_color_temp(self, device):
    ...
    def turn_on(self, device, brightness, color_temp, color):
        ...
        if (
            ...
            if color_temp is not None:
                return self.set_color_temp(device, color_temp)
    ...
    def set_brightness(self, device, n_brightness):
    ...
    def set_color_temp(self, device, color_temp):
        """Set light to turn on."""
        final = False
    ...
    def set_color(self, device, new_color):
        """Set light to turn on."""
        final = False
