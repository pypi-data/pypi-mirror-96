...
def get_max_partition_offset_messages(consumer, messages):
    """
    Allows you to find the messages with the latest offsets per partition in order to be able to commit them.
    """
    partition_msgs = {p: [] for p in [p_obj.partition for p_obj in consumer.assignment()]}
    for message in messages:
        partition_msgs[message.partition()] += [message]
    return {p: max(msgs, key=lambda msg: msg.offset()) for p, msgs in partition_msgs.items() if msgs}
...
def commit_message_batch(consumer, messages, asynchronous=False):
    ...
    if env_vars()['CONSUMER_ENABLE_AUTO_COMMIT'].lower() == 'true':
        asynchronous=True
    latest_partition_offsets = get_max_partition_offset_messages(consumer, messages)
    LOGGER.debug(f'Commiting (partition, offset) list:\n{[(p, msg.offset()) for p, msg in latest_partition_offsets.items()]}')
    for message in latest_partition_offsets.values():
        consumer.commit(message, asynchronous=asynchronous)
    # Keeping this approach here just to have it, will remove once commited, didn't fully work.
    # partition_offsets_to_commit = [TopicPartition(message.topic(), message.partition(), message.offset()) for message in messages]
    # consumer.commit(offsets=partition_offsets_to_commit, asynchronous=asynchronous)
