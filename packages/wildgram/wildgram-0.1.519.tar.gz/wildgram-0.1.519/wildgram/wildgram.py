...
def assignParents(tokens):
    ...
    ranks["-"] = commonorder
    ranks["\|"] = commonorder
    ranks["(\s|^)and(\s|^)"] = commonorder
    ranks["(\s|^)or(\s|^)"] = commonorder
    ranks["(\s|^)by(\s|^)"] = commonorder
    ranks["(\s|^)in(\s|^)"] = commonorder
    ranks["(\s|^)on(\s|^)"] = commonorder
    ranks["(\s|^)was(\s|^)"] = commonorder
    ranks["(\s|^)is(\s|^)"] = commonorder
    ranks["(\s|^)but(\s|^)"] = commonorder

    order = [":\n", "\n", "\. ", ";", ": ", ",", "-", "\|", "(\s|^)and(\s|^)", "(\s|^)or(\s|^)", "(\s|^)by(\s|^)", "(\s|^)in(\s|^)", "(\s|^)on(\s|^)", "(\s|^)was(\s|^)", "(\s|^)is(\s|^)","(\s|^)but(\s|^)"]
    ...
    for i in range(len(tokens)):
        ...
        for ord in order:
            if re.search(ord, tokens[i]["token"].lower()):
