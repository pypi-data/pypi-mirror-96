...
def run_example(device_id, amplitude=0.1, do_plot=False):
    ...
    while not zoomfft.finished():
        ...
        print(f"Individual zoomFFT progress: {progress[0]:.2%}.", end="\r")
    ...
    assert len(samples) == loopcount, "The zoomFFT returned an unexpected number of FFTs: `%d`. Expected: `%d`." % (
        len(samples),
        loopcount,
    )
    print(f"Number of lines in the first zoomFFT: {len(samples[0][0]['grid']):d}.")

    if do_plot:
        import matplotlib.pyplot as plt

        plot_idx = 0
        print(f"Will plot result of the zoomFFT result {plot_idx:d}.")
