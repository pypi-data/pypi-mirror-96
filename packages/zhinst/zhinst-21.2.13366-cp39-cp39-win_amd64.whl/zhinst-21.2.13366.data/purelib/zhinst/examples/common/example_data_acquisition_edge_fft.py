...
def run_example(device_id, amplitude=0.25, do_plot=False):
    ...
    if do_plot and samples:
        ...
        for index, sample in enumerate(samples):
            ...
            rDb_compensated = 20 * np.log10(
                (sample["value"][0] / filter_compensation["value"][0]) * np.sqrt(2) / amplitude
            )
            axs[0].plot(frequencies, rDb)
            axs[1].plot(frequencies, rDb_compensated)
        axs[0].grid()
        title = f"Data Acquisition's read() returned {len(samples)} FFTs each with {len(samples[0]['value'][0])} bins"
