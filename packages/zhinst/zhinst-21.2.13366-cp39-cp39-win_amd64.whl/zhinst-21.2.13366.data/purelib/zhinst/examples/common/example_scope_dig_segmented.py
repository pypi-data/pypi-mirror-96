...
def run_example(device_id, do_plot=False, scope_length=8192, module_historylength=1):
    ...
    print(f"Actual scope length set on the device: {scope_length_set} (requested {scope_length})")
    ...
    for index, amplitude in enumerate(np.linspace(0.2, 1.0, num_measurements)):
        ...
        print(f"Segment count set on the device: {segment_count_set} (requested {segment_counts[index]}).")
        ...
        if wave_nodepath not in data:
            print(
                f"[error]: The subscribed data `{wave_nodepath}` for measurement {index} ({amplitude}) "
                "was not returned."
            )
        else:
    ...
    if do_plot and data[wave_nodepath]:
        ...
        for index, records in enumerate(data[wave_nodepath]):
            ...
            ax.set_title(f"{num_measurements} Scope Records (consisting of different segment counts)")
