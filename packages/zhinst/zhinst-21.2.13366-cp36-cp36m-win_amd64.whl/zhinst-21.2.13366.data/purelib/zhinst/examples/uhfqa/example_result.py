...
def run_example(device_id, result_length=2600, num_averages=1, do_plot=True):
    ...
    daq, device, _ = zhinst.utils.create_api_session(
        device_id, apilevel_example, required_devtype=required_devtype, required_options=required_options
    )
    ...
    for i in channels:
        ...
        daq.setVector(f"/{device:s}/qas/0/integration/weights/{i}/real", w)
        daq.setVector(f"/{device:s}/qas/0/integration/weights/{i}/imag", w)
    ...
    daq.setInt(f"/{device:s}/qas/0/result/source", ResultLoggingSource.TRANS)
    ...
    paths = []
    for ch in channels:
        path = f"/{device:s}/qas/0/result/data/{ch:d}/wave"
        paths.append(path)
    daq.subscribe(paths)
