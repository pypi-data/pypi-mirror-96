...
def run_example(
    ...
    if do_plot:
        ...
        def plot_scope_records(plot_axes, scope_records, scope_input_channel, scope_time=0):
            ...
            """
            ax = plot_axes
            colors = [cm.Blues(np.linspace(0, 1, len(scope_records))), cm.Greens(np.linspace(0, 1, len(scope_records)))]
        ...
        ax1.set_title(
            f"{len(data_no_trig[wave_nodepath])} Scope records from 2 channels ({device}, triggering disabled)"
        )
        ...
        ax.set_title(f"FFT of {len(data_fft[wave_nodepath])} scope records from 2 channels ({device})")
        ax.set_xlabel("f [MHz]")
        fig2.show()
