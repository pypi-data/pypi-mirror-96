...
def run_example(device_id, do_plot=False, amplitude=0.25, repetitions=1, save=0):
    ...
    while n < num_pulses * repetitions:
        ...
        print(
            f"Data Acquisition Module progress (acquiring {trigger_count:d} triggers): {progress[0]:.2%}.", end="\r",
        )
    ...
    assert signal_path in data, f"no data recorded: data dictionary has no key `{signal_path}`."
    ...
    if do_plot and samples:
        ...
        ax.axvline(trigger_duration + trigger_delay, linewidth=2, linestyle=":", color="k", label="duration + delay")
        ax.axhline(trigger_level, linewidth=2, linestyle="-", color="k", label="level")
        ax.axhline(trigger_level - trigger_hysteresis, linewidth=2, linestyle="-.", color="k", label="hysteresis")
