...
def run_example(device_id, amplitude=0.25, do_plot=False):
    ...
    while n < num_pulses:
        ...
        print(f"Recorder progress (acquiring {trigger_count:d} triggers): {progress[0]:.2%}.", end="\r")
    ...
    assert trigger_path in data, f"no data recorded: data dictionary has no key `{trigger_path}`."
    samples = data[trigger_path]
    print(f"Software Trigger's read() returned {len(samples)} signal segments.")
    assert (
        len(samples) == trigger_count
    ), f"Unexpected number of signal segments returned: `{len(samples)}`. Expected: `{trigger_count}`."
    ...
    print(f"The first signal segment contains {dt_seconds:.3f} seconds of demodulator data.")
    np.testing.assert_almost_equal(
        dt_seconds,
        trigger_duration,
        decimal=2,
        err_msg="Duration of demod data in first signal segment does not match the expected duration.",
    )
    ...
    if do_plot and samples:
        ...
        ax.axvline(
            trigger_duration + trigger_delay, linewidth=2, linestyle=":", color="k", label="/0/duration + /0/delay"
        )
        ax.axhline(trigger_level, linewidth=2, linestyle="-", color="k", label="/0/level")
        ax.axhline(trigger_level - trigger_hysteresis, linewidth=2, linestyle="-.", color="k", label="/0/hysteresis")
        ...
        ax.set_title(
            (
                f"Software Trigger's read() returned {len(samples)} segments of demodulator data\n",
