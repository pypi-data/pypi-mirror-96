...
def run_example(device_id, do_plot=False, num_grids=3):
    ...
    err_msg = (
        "This example requires either an HF2/UHF Instrument with the Multifrequency (MF)"
    ...
    if do_plot:
        z_min = np.inf
        z_max = -np.inf
        # pylint: disable=W0611 # unused-import (required by subplot projection='3d')
        ...
        surf1 = ax.plot_surface(
            xx, yy, R, rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False  # pylint: disable=E1101
        )
        num_ticks = 5
        ticks = np.linspace(0, num_cols, num_ticks)
        ticklabels = [f"{trigger_delay + trigger_duration * tick / num_cols:0.3f}" for tick in ticks]
        ax.set_xticks(ticks)
    ...
    while not trigger.finished():
        ...
        if (triggerpath in data_read) and data_read[triggerpath]:
            ...
            print(f"Overall progress: {trigger.progress()[0]}. Grid {num_finished_grids} flags: {flags[0]}.")
            if do_plot:
                ...
                surf1 = ax.plot_surface(
                    ...
                    rstride=1,
                    cstride=1,
                    cmap=cm.coolwarm,  # pylint: disable=E1101
                    linewidth=0,
                    antialiased=False,
                )
                R_max = np.nanmax(R)
        ...
        if time.time() - t0 > timeout:
            ...
            if num_finished_grids == 0:
                ...
                trigger.finish()
                raise RuntimeError(f"Failed to record any grids before timeout ({timeout} seconds). ")
            print(
