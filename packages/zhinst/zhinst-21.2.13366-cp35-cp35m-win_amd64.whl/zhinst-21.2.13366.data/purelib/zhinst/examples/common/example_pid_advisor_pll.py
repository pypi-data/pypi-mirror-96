...
def run_example(device_id, do_plot=False):
    ...
    if props["devicetype"].startswith("MF") and ("MD" not in props["options"]):
        raise RuntimeError(
            "Required option set not satisfied. On MF Instruments this example requires both the PID and the MD "
            f"Option. Device `{device}` reports devtype `{props['devicetype']}` and options `{props['options']}`."
        )
    ...
    if result is not None:
        ...
        print(f"The pidAdvisor calculated the following gains, P: {p_advisor}, I: {i_advisor}, D: {d_advisor}.")
    ...
    if do_plot:
        ...
        ax[0].set_title(
            ...
            "P = %0.1f, I = %0.1f,\n"
            "D = %0.5f and bandwidth %0.1f kHz" % (p_advisor, i_advisor, d_advisor, bw_advisor * 1e-3)
        )
