...
def run_example(device_id, monitor_length=4000, num_averages=2 ** 8, do_plot=True):
    ...
    for i in range(4):
        ...
        daq.setVector(f"/{device:s}/qas/0/integration/weights/{i}/imag", np.zeros(integration_length))
        daq.setVector(f"/{device:s}/qas/0/integration/weights/{i + 4}/real", np.zeros(integration_length))
    ...
    def combine_results(channels):
        ...
        waves = [result_data[f"/{device:s}/qas/0/result/data/{ch:d}/wave"] for ch in channels]
