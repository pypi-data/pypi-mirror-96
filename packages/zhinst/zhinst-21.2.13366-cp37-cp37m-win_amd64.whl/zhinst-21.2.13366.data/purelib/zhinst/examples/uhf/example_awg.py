...
def run_example(device_id, do_plot=False):
    ...
    (daq, device, props) = zhinst.utils.create_api_session(
        device_id, apilevel_example, required_devtype="UHF", required_options=["AWG"], required_err_msg=err_msg
    )
    ...
    assert wave_nodepath in data_read, f"Error: The subscribed data `{wave_nodepath}` was returned."
    ...
    for n in range(0, len(data["channelenable"])):
        ...
        if p:
            ...
            x_measured = (
                np.arange(-data["totalsamples"], 0) * data["dt"] + (data["timestamp"] - data["triggertimestamp"]) / f_s
            )
    ...
    print(f"Measured and expected signals agree, normalized correlation coefficient: {norm_correlation_coeff}.")
    return data_read
