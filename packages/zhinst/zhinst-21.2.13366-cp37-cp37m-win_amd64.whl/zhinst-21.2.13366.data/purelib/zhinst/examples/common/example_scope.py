...
def run_example(
    ...
    if scope_inputselect == 0:
        ...
        raise Exception(
            f"This example only supports signal inputs; it does not support scope inputselect {scope_inputselect}. "
            "Use 0 or 1 instead."
        )
    ...
    assert wave_nodepath in data_with_trig, f"The Scope Module did not return data for {wave_nodepath}."
    print(f"Number of scope records returned with triggering enabled: {len(data_with_trig[wave_nodepath])}.")
    ...
    assert wave_nodepath in data_fft, f"The Scope Module did not return data for {wave_nodepath}."
    print(f"Number of scope records returned with triggering enabled (and FFT'd): {len(data_fft[wave_nodepath])}.")
    ...
    if do_plot:
        ...
        def plot_scope_records(plot_axes, scope_records, scope_input_channel, scope_time=0):
            ...
            for index, record in enumerate(scope_records):
                ...
                if record[0]["flags"] & 7:
                    print(f"Skipping plot of record {index}: record flags={record[0]['flags']} indicate corrupt data.")
                    continue
        ...
        ax1.set_title(f"{len(data_no_trig[wave_nodepath])} Scope records from {device} (triggering disabled)")
        ...
        ax2.set_title(f"{len(data_with_trig[wave_nodepath])} Scope records from {device} (triggering enabled)")
        ...
        ax.set_title(f"FFT of {len(data_fft[wave_nodepath])} scope records from {device}")
        ax.set_xlabel("f [MHz]")
