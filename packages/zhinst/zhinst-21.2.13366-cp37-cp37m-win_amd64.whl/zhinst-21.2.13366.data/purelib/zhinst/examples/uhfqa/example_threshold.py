...
def run_example(device_id, threshold=500, result_length=1500, num_averages=1, do_plot=True):
    ...
    for i, ch in enumerate(channels):
        ...
        daq.setVector(f"/{device:s}/qas/0/integration/weights/{ch}/real", w)
        daq.setVector(f"/{device:s}/qas/0/integration/weights/{ch}/imag", w)
    ...
    for ch in channels:
        daq.setDouble(f"/{device:s}/qas/0/thresholds/{ch:d}/level", threshold)
    ...
    for result_source in (ResultLoggingSource.TRANS, ResultLoggingSource.THRES):
        ...
        if result_source == ResultLoggingSource.TRANS:
            for ch in channels:
                num_ones = daq.getInt(f"/{device:s}/qas/0/result/statistics/data/{ch:d}/ones")
                num_flips = daq.getInt(f"/{device:s}/qas/0/result/statistics/data/{ch:d}/flips")
    ...
    for i, ch in enumerate(channels):
        print(f"{ch:15d}  {statistics[i]['ones']:10d} {statistics[i]['flips']:10d}")
    ...
    if do_plot:
        ...
        for path, samples in result_data[ResultLoggingSource.TRANS].items():
            ax[0].plot(samples, label=f"{path}")
        ...
        for path, samples in result_data[ResultLoggingSource.THRES].items():
            ax[1].plot(samples, label=f"{path}")
