...
class BinaryMatrixRankTest(Test):
    ...
    def _execute(self,
        ...
        for i in range(blocks_number):
            ...
            # TODO: fix this
            rank: int = self.rank(self._cols_number, self._rows_number, block, i)
            # rank: int = numpy.linalg.matrix_rank(matrix)
            #print(rank)
            # Count the result
    ...
    @staticmethod
    def rank(M, Q, matrix, block_num):
        lm = BinaryMatrixRankTest.row_echelon(M, Q, matrix, block_num)
        rank = 0
        for i in range(Q):
            nonzero = False
            for bit in lm[i]:
    ...
    @staticmethod
    def row_echelon(M, Q, matrix, block_num):
        import copy
