...
class DataPreparerFromDataSet(DataPreparer):
    ...
    def _parse(self, dataprep_json_obj: Dict[str, Any]) -> None:
        ...
        with silent_deprecation_warning():
            # TODO: It is currently unknown which scenario is directly populating 'datasetId' directly on the
            #       json object, instead of passing in as 'training_data'
            training_dataset_id = dataprep_json_obj.get('datasetId')

            if training_dataset_id is None and dataprep_json_obj.get('training_data') is not None:
                training_data = cast(Dict[str, Any], dataprep_json_obj.get('training_data'))
                training_dataset_id = training_data.get('datasetId')
