...
from azureml.automl.core.shared import log_server, logging_utilities as log_utils
...
from azureml.automl.runtime.shared.datasets import ClientDatasets, DatasetBase
...
class _AdbAutomlExperiment:
    ...
    def setup_iteration(self, verifier: VerifierManager, current_run: Run, run_id: str) -> None:
        """
        ...
        fit_iteration_parameters_dict, experiment_observer, feature_config_manager = \
            featurization_entrypoint_util._prep_input_data(current_run, "setup", self.automl_settings,
        ...
        featurization_entrypoint_util.cache_onnx_init_metadata(cache_store, fit_iteration_parameters_dict,
                                                               self.parent_run_id)
        ...
        self._gc_object(fit_iteration_parameters_dict)
    ...
    def model_explain_iteration(self, verifier: VerifierManager, current_run: Run) -> None:
        """
        ...
        with _AMLLogUploader(current_run, self.worker_id):
            try:
                ...
                if run_id.endswith("setup"):
                    self.setup_iteration(verifier, current_run, run_id)
                elif run_id.lower().endswith("ModelExplain".lower()):
                    self.model_explain_iteration(verifier, current_run)
                else:
                    self.train_iteration(verifier, current_run, pkg_ver,
                                         pipeline_dto, run_id)
