...
def dm_from_this_guild(guild_id: int) -> commands.check:
    """
    ...
    def predicate(ctx):
        if ctx.guild is not None:
            raise PrivateMessageOnly("Command was called from a guild instead of a direct message.")

        elif guild_id not in [guild.id for guild in ctx.bot.guilds
                              if ctx.author.id in [member.id for member in guild.members]]:
            raise NotMemberOfCorrectGuild("Command author couldn't be found within the specified guild.")
        return True
    return commands.check(predicate)
