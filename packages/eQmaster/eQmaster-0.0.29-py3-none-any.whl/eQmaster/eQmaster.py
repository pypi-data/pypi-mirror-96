...
def convert_stock_symbol(stock_symbol):
    if stock_symbol.endswith('.tw'):
        return stock_symbol.replace('.tw', '')
    return ''
...
def get_current_quote(stock_symbol):
    ...
    stock_symbol = convert_stock_symbol(stock_symbol)
    api = sj.Shioaji()
    api.login(
...
def get(stock_list, from_date, to_date):
    ...
    return df.loc[mask][['Date','Open','High','Low','Close','Volume','Adj Close','Outstanding Share','PER','PBR','CDIVR','Open Adj','High Adj','Low Adj']]
...
def setup(settings=None):
    ...
    if 'key' in os.environ and 'credential' in os.environ:
        ew = eWarrant.eWarrant(os.environ['key'], credential=os.environ['credential'])
        os.environ['u'] = ew.username()
        os.environ['p'] = ew.password()
