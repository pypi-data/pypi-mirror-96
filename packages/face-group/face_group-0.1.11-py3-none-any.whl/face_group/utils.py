...
def _get_timestamp(name: str) -> str:
    return name.split("_")[-1].split(".jpg")[0]


def _sort_path_by_encodings(
    np_face: np.ndarray, np_path: np.ndarray, tolerance: float = 0.6
) -> List[List[Path]]:
    ...
    while len(np_face) > 0:
        ...
        indexes = face_distance(np_face, current) <= tolerance
...
def _process_images_in_one_cpu(images, model, verbose):
    if verbose:
        results = [
            _process_image(image, model, verbose)
            for image in track(images, "Processing...")
        ]
    else:
