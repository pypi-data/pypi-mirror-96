...
class LearnerPlayer(Player):
    ...
    def __init__(self, symbol: Symbol, name: str, greed_rate: float = 0.7) -> None:
    ...
    @staticmethod
    def __valid_actions(state: np.ndarray) -> List[int]:
        actions = np.where(state == 0)[0].tolist()
        return actions

    def __random_action(self, state: np.ndarray) -> int:
        return np.random.choice(self.__valid_actions(state))
    ...
    def action(self, state: np.ndarray) -> int:
        if np.random.uniform(0, 1) <= self.greed_rate:
            action = self.__random_action(state)
        else:
            q_max: float = 0
            first_run: bool = True
            for position in self.__valid_actions(state):
