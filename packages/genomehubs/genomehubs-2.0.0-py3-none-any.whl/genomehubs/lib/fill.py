...
def stream_missing_attributes_at_level(es, *, nodes, attrs, template, level=1):
    ...
    for node in nodes:
        ...
        target_attr_count = len(attrs)
        for desc_node in desc_nodes:
            if desc_node["_source"]["taxon_id"] == "109039":
                print(desc_node["_source"]["taxon_id"])
                print(desc_node["_source"].get("filled", False))
            skip_attrs = set()
            ...
            desc_attr_count = len(skip_attrs)
            for key, anc_attribute in anc_attributes.items():
                if key not in skip_attrs:
                    ...
                    desc_attr_count += 1
            if desc_attr_count == target_attr_count:
                desc_node["_source"]["filled"] = True
