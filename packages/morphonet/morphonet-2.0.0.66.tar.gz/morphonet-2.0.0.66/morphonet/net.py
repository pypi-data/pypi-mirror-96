...
class Net:
    ...
    def get_guy_by_name(self,name): # RETURN NAME + SURNAME + LOGIN FORM SPECIFIC ID
        ...
        values = name.split(' ')
        if len(values)!=2:
            print(strred("Please put <name> <surname>"))
            return -1
        u_name = str(values[0])
    ...
    def get_group_by_name(self,name): # RETURN NAME + SURNAME + LOGIN FORM SPECIFIC ID
        ...
        id_group=-1
        if not nodata(data) and "id" in data:
            id_group=int(data['id'])
        if id_group==-1:
            print(strred('Unknown group "name"'))
        return id_group
    ...
    def delete_info_by_id(self,id_info):
        ...
        if self._is_dataset():
            ...
            if nodata(data) or "id" not in data:
