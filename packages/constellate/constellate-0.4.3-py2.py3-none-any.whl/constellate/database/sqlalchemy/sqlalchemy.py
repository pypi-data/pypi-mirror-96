...
@contextmanager
def session_scope(self, session_maker: sessionmaker = None, **kwargs):
...
class SQLAlchemy:
    ...
    def __int__(self):
        self._session_type = _SessionType.UNKNOWN
        self._multi_session_maker = None
        self._instances = {}

    def _engines(
        self, options: Dict, types: List[DBConfigType] = [DBConfigType.INSTANCE]
    ) -> Iterator[Tuple[SQLAlchemyDBConfig, Dict]]:
        engine_id_to_options = {
            engine_options.get("id") for engine_options in _get_engine_options(options)
        }
    ...
    def setup(self, options: Dict = {}):
        ...
        try:
            for engine_options in _get_engine_options(options):
                ...
                if self._session_type == _SessionType.UNKNOWN:
                    self._session_type = _SessionType.MONO_ENGINE
                elif self._session_type == _SessionType.MONO_ENGINE:
                    self._session_type = _SessionType.MULTI_ENGINE
                ...
                instance.logger.debug(f"Using db: started")
                yield self._setup(instance=instance, options=engine_options)
                self._setup_engine_session_maker(instance=instance)
                instance.logger.debug(f"Using db: completed")
                self._instances.update({instance.identifier: instance})
                setup_engines_options.append(engine_options)
            self._setup_multi_engine_session_maker()
        finally:
    ...
    def dispose(self, options:Dict = {}):
        for instance in self._engines(options):
    ...
    def _setup_engine_session_maker(self, instance: SQLAlchemyDBConfig = None):
        instance.session_maker = self._create_session_maker(
            type=_SessionType.MONO_ENGINE, bind=instance.engine, autoflush=True, autocommit=False
        )

    def _setup_multi_engine_session_maker(self):
        self._multi_session_maker = self._create_session_maker(
            type=_SessionType.MULTI_ENGINE, autoflush=True, autocommit=False
        )

    def _create_session_maker(self, type: _SessionType = _SessionType.UNKNOWN, **kwargs):
        session_class = self._get_session_class(type=type)
        return sessionmaker(class_=session_class, **kwargs)

    def _get_session_class(self, type: _SessionType = _SessionType.UNKNOWN) -> Session.__class__:
        return MultiEngineSession

    def _new_session(self):
        return self._multi_session_maker
