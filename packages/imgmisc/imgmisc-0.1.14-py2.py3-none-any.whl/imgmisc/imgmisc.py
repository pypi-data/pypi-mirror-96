...
def props_to_DataFrame(regionprops, include=['label']):
    r"""
    ...
    for k in include:
        try:
            if k != 'centroid':
                ...
                centroids = np.array([r[k] for r in regionprops])
                d['centroid_z'] = centroids[:, 0] 
                d['centroid_y'] = centroids[:, 1] 
                d['centroid_x'] = centroids[:, 2] 
        except ValueError:
...
def regionprops_3D(im, resolution=[1,1,1]):
    # regprops = regionprops_3D(seg_img)
    results = regionprops(im)
    for ii in range(len(results)):
        mask = results[ii].image
        ...
        # Slice indices
        results[ii].slice = results[ii]._slice
        # ----------------------------------------------------------------
        # Volume of regions
        results[ii].volume = results[ii].area * np.product(resolution)
        # ----------------------------------------------------------------
        # Volume of bounding box
        results[ii].bbox_volume = np.prod(mask.shape) * np.product(resolution)
        # ----------------------------------------------------------------
        # Create an image of the maximal inscribed sphere
        r = dt.max()
        results[ii].max_inscribed_sphere_radius = r
        # ----------------------------------------------------------------
        ...
        results[ii].surface_mesh_vertices = verts
        results[ii].surface_mesh_faces = faces
        area = mesh_surface_area(verts, faces)
        results[ii].surface_area = area

        # ----------------------------------------------------------------
        # Do PCA to extract principal components
        try:
            pca = PCA(n_components=3)
            pca.fit(np.array(list(np.where(tmp))).T * resolution)
            results[ii].pca_components = pca.components_
            results[ii].pca_explained_variance = pca.explained_variance_
            sorted_pca_var = np.sort(pca.explained_variance_)
            results[ii].pca_major_axis_length = sorted_pca_var[2]
            results[ii].pca_medial_axis_length = sorted_pca_var[1]
            results[ii].pca_minor_axis_length = sorted_pca_var[0]
        except:
            results[ii].pca_components = []
            results[ii].pca_explained_variance = []
            results[ii].pca_major_axis_length = np.nan
            results[ii].pca_medial_axis_length = np.nan
            results[ii].pca_minor_axis_length = np.nan
        # ----------------------------------------------------------------
        # Calculate sphericity
        vol = results[ii].volume
        r = (3/4/np.pi*vol)**(1/3)
        a_equiv = 4*np.pi*(r)**2
        a_region = results[ii].surface_area
        results[ii].sphericity = a_equiv/a_region
        # ----------------------------------------------------------------
        # Convex hull stuff
        # ----------------------------------------------------------------
        # Can cause problems if the object voxels are in a single plane
        try:           
            results[ii].convex_volume = results[ii].convex_area
        except:
            results[ii].convex_volume = np.nan
    return results
