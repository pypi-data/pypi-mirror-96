...
from pypsql_api.wire.errors import NoData
...
class ReadingIO:
    ...
    def read_bytes(self, n) -> bytes:
        data = self.buff.read(n)
        if not data:
            raise NoData(f"No data. Expected {n} bytes", expected_bytes=n, got_bytes=0)
        if len(data) != n:
            raise NoData(f"Not enough data. Expected {n} bytes got {len(data)}", expected_bytes=n, got_bytes=len(data))
    ...
    def read_int32_delim_message(self, timeout=5) -> Tuple[Optional[Any], int]:
        ...
        try:
            ...
            if e.got_bytes == 0:
                # idle till timeout on the connection
                time.sleep(1)
                return self.read_int32_delim_message(self, timeout=timeout - 1)
