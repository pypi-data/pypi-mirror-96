...
def process_bind_command(context: Context):
    ...
    try:
        ...
        print(f"Error doing bind process {e}")
        traceback.print_exc()
        ErrorResponse.query(str(e)).write(context.output)
        return context, Names.RECEIVE_EXTENDED_QUERY_COMMAND
...
def process_execute_command(context: Context):
    ...
    print(f"Process {msg} in process_execute_command")
    ...
    try:
        ...
        print(f">>>>>>>>>process_execute_command: got response: {resp}")
        print(f"is instance of df: {isinstance(resp, DataFrame)}")
        if isinstance(resp, DataFrame):
            ...
            print(">>>>>>>>>>..Writing extended dataframe")
            return context, Names.WRITE_EXTENDED_DATA_FRAME
        else:
            print(f">>>>>>> writing empty response")
            return context, Names.WRITE_EXTENDED_EMPTY_RESPONSE
    except KeyError:
        error_msg = f"No portal name was found for {msg.portal_name}"
        print(error_msg)
        ErrorResponse.query(error_msg).write(context.output)
        return context, Names.SYNC
    except Exception as e:
        print(f"Error executing {e}, for {msg}")
        # traceback.print_exc()
        ErrorResponse.query(str(e)).write(context.output)
        return context, Names.SYNC
...
def describe_statement_or_portal(context: Context):
    ...
    try:
        ...
        print(f"describe_statement_or_portal {e}")
        ErrorResponse.query(str(e)).write(context.output)
        return context, Names.RECEIVE_EXTENDED_QUERY_COMMAND
