...
def reset(io: ReadingIO) -> ReadingIO:
    io.buff.seek(0)
...
def read_ssl_request(context: Context):
    ...
    try:
        ...
        if ssl_request.request_code == 196608:
            # ancient mode
            return read_startup_message(context, inner_buff=reset(ssl_request.inner_buff))
        else:
...
def read_startup_message(context: Context, inner_buff: Optional[ReadingIO] = None):
    try:
        ...
        startup_front = StartupMessage.read(context.input) if inner_buff is None else StartupMessage.read_from_buff(
            inner_buff)
