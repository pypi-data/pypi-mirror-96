...
class ReadingIO:
    ...
    def read_bytes(self, n) -> bytes:
        ...
        if not data:
            raise Exception(f"No data. Expected {n} bytes")
        if len(data) != n:
            raise Exception(f"Not enough data. Expected {n} bytes got {len(data)}")
    ...
    def read_int32_delim_message(self) -> Tuple[Optional[Any], int]:
        """
        Reads the len = int32,
