...
from pypsql_api.wire.back import SSLNo, AuthenticationCleartextPassword, ReadyForQuery, AuthenticationOk, \
    EmptyQueryResponse, DataFrameRowDescription, DataFrameDataRows, CommandComplete, SSLYes
...
def read_startup_message(context: Context):
    try:
        ...
        print(f"No data received from server")
        return context, Names.CLOSE
    except:
        print(f"Error udring startup read")
        traceback.print_exc()
        raise
