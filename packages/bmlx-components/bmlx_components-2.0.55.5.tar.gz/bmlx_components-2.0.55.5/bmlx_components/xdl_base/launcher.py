...
class XdlLauncher(Launcher):
    ...
    def _report_metrics(self, execution_info):
        ...
        labels = {
            "run_id": execution_info.exec_properties["execution_id"],
            "experiment_id": execution_info.exec_properties["experiment_id"],
            "component_id": self._component.id,
        }
        ...
        fp = "{}/exp_{}/run_{}/{}/final_metrics".format(
            ...
            str(execution_info.exec_properties["execution_id"]),
            self._component.id,
        )
        ...
        sample_ts = metrics.get("sample_ts", int(time.time() * 1000))
        del metrics["sample_ts"]

        for name, value in metrics.items():
            if isinstance(value, list):
                for v, idx in zip(value, range(len(value))):
                    metric_client.report_metric_to_prometheus(
                        name=f"{name}_{idx}",
                        value=v,
                        ts=sample_ts,
                        labels=labels,
                ...
                metric_client.report_metric_to_prometheus(
                    name=name, value=value, ts=sample_ts, labels=labels
                )
