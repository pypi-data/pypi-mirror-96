...
class XdlLauncher(Launcher):
    ...
    def _report_metrics(self, execution_info):
        ...
        labels = {
            "run_id": str(execution_info.exec_properties["execution_id"]),
            "experiment_id": str(
        ...
        sample_ts = int(metrics.get("sample_ts", time.time()) * 1000)
            ...
            if isinstance(value, list):
                for v, idx in zip(value, range(len(value))):
                    prometheus_data.append(
                        {
                            "n": f"bmlx_metric_{name}_{idx}".replace("/", "_"),
                            "ls": labels,
                            "ts": int(sample_ts),
                            "v": float(v),
                        }
                ...
                prometheus_data.append(
                    {
                        "n": f"bmlx_metric_{name}".replace("/", "_"),
                        "ls": labels,
                        "ts": int(sample_ts),
                        "v": float(value),
                    }
