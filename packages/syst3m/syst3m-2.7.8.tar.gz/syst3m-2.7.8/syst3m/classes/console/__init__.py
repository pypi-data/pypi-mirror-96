...
def execute(
	...
	class Output(ResponseObject):
		...
		def __eq__(self, dictionary):
			if isinstance(dictionary, str):
				return self.str == dictionary
			elif isinstance(dictionary, String):
				return self.str == dictionary.string
			elif isinstance(dictionary, dict):
				return str(self.sort()) == str(Files.Dictionary(dictionary).sort())
			elif not isinstance(dictionary, (self.__class__, Dictionary)):
				return False
	...
	if serialize:
		...
		return Output(message=f"Succesfully executed {response_str}, (output: {output}), (error: {error_}).", attributes={
			"output":output,
			"process":proc,
			#"pid":proc.pid, # only for subprocess.Popen
		})
