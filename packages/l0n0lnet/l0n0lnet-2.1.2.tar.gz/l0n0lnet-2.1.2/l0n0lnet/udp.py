...
class base_udp:
    ...
    def __init__(self, ip: bytes, port: int, buffer_size: int = 4 * 1024):
        ...
        @buffer_size:int:接收消息缓冲区大小。当消息大于buffer_size时会出现错误\n
        """
        create_udp(self.__class__.__name__, ip, port, buffer_size)
        set_udp_on_read(self.__class__.__name__, self.on_read)
    ...
    def send_to(self, tip: bytes, tport: int, data: bytes):
        ...
        @return:bool:发送的结果\n
        """
        return send_udp_msg(self.__class__.__name__, tip, tport, data)

    def close(self):
        """
        关闭udp
        """
        close_udp(self.__class__.__name__)
