...
class Type(type):
    ...
    def __new__(cls, *_T, name=None, shape=tuple(), inv=False, ext=False, itypes=None):
        if len(_T) == 1 and not isatype(_T): _T = _T[0]
        if len(_T) > 0 and not isatype(_T): raise SyntaxError("Wrong parameter type. ")
        _T = Type.extractType(_T)

        self = super().__new__(cls, cls.__name__ + '.' + '/'.join(map(typename, _T)), tuple(t for t in _T if inheritable(t)), {})
        self.inv = inv
        self.types = _T
        self.name = name
        self.shape = shape
        ...
        if isoftype(arg, self.types):
            ...
            if self.itemtypes != None:
                ...
                if isinstance(self.itemtypes, dict):
                    ...
                    import random
                    for _ in range(min(len(arg), 5)):
                        try: x = random.choice(arg)
                        except: x = arg.pop()
