...
class App:
	...
	def _get_merged_action_metadata(self):
		...
		for action_handler in connector_class.get_handlers():
			identifier = action_handler.action_identifier
			action_data = actions_metadata.setdefault(identifier, {})
			handler_method = action_handler.handler_method
			...
			if docstring is not None:
				...
				action_data.setdefault('description', ' '.join(doc_tokens))

			parameters_data = action_data.pop('parameters', {})
			parameter_items = self._populate_parameter_defaults(
					method=handler_method, parameters_data=parameters_data)
			yield identifier, action_data, parameter_items

	def _populate_parameter_defaults(self, method, parameters_data):
		...
		for name, method_parameter in signature.parameters.items():
			...
			try:
				...
				try:
					data_type = self.ANNOTATION_MAP[annotation]
				except KeyError:
					logger.error(
						f'Unable to infer parameter {name!r} type from'
