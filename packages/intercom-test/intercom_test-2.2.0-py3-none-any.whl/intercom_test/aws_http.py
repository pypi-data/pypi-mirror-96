...
class RESPONSE_BODY_EXPECTATIONS:
    """Key class for customized response body expectations
    
    For cases where a strict equality test of the response body generated by
    the handler to the response body specified in the interface data is not
    desirable, set the entry in the case :class:`dict` keyed by this class
    itself (not an instance of it) to a callable that takes the parsed JSONic
    data as its only argument and returns a :class:`bool` indicating whether
    the response expectations are met.
    
    This can, for example, be used to integrate a JSONic data comparison
    library with more flexible matching.
    
    A common place to set the entry in the case keyed with this class is in
    the callable passed in the *case_env=* keyword to
    :meth:`.ServerlessHandlerMapper.case_tester`.
...
class ServerlessHandlerMapper(HandlerMapper):
    ...
    Note that the ``case_env=`` handler can set the :class:`.RESPONSE_BODY_EXPECTATIONS`
    key in the case to customize validation of the generated response body if
...
def confirm_expected_response(handler_result: dict, case: dict) -> None:
    ...
    _confirm_response_body(handler_result, case)
...
def _confirm_response_body(handler_result: dict, case: dict) -> None:
    expected = case['response body']
    if RESPONSE_BODY_EXPECTATIONS in case:
        response_body_expectations = case[RESPONSE_BODY_EXPECTATIONS]
    else:
        def response_body_expectations(actual):
    ...
    try:
        ...
        if isinstance(expected, str):
            ...
            assert response_body_expectations(handler_result['body'])
            ...
            assert response_body_expectations(b64decode(handler_result['body']))
        else:
            actual = json.loads(handler_result['body'])
            assert response_body_expectations(json.loads(handler_result['body']))
