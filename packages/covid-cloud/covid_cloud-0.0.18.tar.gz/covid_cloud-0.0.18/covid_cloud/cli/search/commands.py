...
from covid_cloud.cli.auth.utils import assert_token
...
def query(ctx, q, download, use_json, raw):
    click.echo(search_client.query(ctx.obj['search-url'], q, download, use_json, raw, ctx.obj.get('oauth_token', None)))
