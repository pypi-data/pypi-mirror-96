...
def delete_gcp_cluster(ctx, cluster):
    ...
    if cluster.get("serving_cluster_name"):
        gke_delete_cluster(
            project_id, cluster["serving_cluster_name"], cluster["networking"]["gcp"]["region"]
        )
...
def gke_delete_cluster(project_id, serving_cluster_name, serving_cluster_region):
    zone_or_region, gke_name = extract_gke_cluster_info(project_id, serving_cluster_name)
    ...
    if zone_or_region == serving_cluster_region:
        cmd += ["--region", serving_cluster_region]
    else:
        cmd += ["--zone", zone_or_region]
...
def extract_gke_cluster_info(project_id, cluster_name):
    ...
    # or gke_PROJECT_REGION_CLUSTERNAME for regional clusters
    # GKE clusters created with spell clients older than the March 1 2021 release
    # are zonal -- after March 1 we switched to using regional clusters, which are set up
    # in the same region that the GCP Spell VPC is created in. Unfortunately since we
    # don't store GKE cluster zone info in the db explicitly the only way
    # of getting that info is by manually parsing the "full name", which is sent
    # by the api through the kube config. This code remains here
    # (and exists in all legacy CLI clients) in order to support those clusters
