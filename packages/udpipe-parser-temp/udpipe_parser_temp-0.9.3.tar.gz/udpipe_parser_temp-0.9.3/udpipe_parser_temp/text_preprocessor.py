...
class TextPreProcessor:
  ...
  def run(self,text):
    ...
    print('1: test_preprocessor:',text)
    if 'ё' in text:
    ...
    print('2: test_preprocessor:',temp)
    ...
    print('3: test_preprocessor:',temp)
    result = []
    for w in temp:
      if re.findall(r'\d{5}|\d{6}',w) != []:
        w = "'" + w + "'"
      result.append(w)
    print('4: test_preprocessor:',result)
    result = ' '.join(result)
    result = self.replace(result).strip()
    if result[-1] not in ['.','?','!']:
      result = result + '.'
    print('5: test_preprocessor:',result)
    return result
