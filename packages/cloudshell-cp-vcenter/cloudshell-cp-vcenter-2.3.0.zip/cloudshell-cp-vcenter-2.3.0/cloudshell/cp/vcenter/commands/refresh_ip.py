...
class RefreshIpCommand(object):
    ...
    def refresh_ip(
        ...
        ip_res = self.ip_manager.get_ip(
            vm, default_network, match_function, cancellation_context, timeout, logger
        )

        if ip_res.reason == IpReason.Timeout:
            raise ValueError(
                "IP address of VM '{0}' could not be obtained during {1} seconds".format(
                    resource_model.fullname, timeout
                )
            )
    ...
    def _get_ip_refresh_timeout(custom_params):
        timeout = RefreshIpCommand._get_custom_param(
            custom_params=custom_params, custom_param_name="refresh_ip_timeout"
        )
    ...
    def _get_ip_refresh_ip_regex(custom_params):
        return RefreshIpCommand._get_custom_param(
            custom_params=custom_params, custom_param_name="ip_regex"
        )
