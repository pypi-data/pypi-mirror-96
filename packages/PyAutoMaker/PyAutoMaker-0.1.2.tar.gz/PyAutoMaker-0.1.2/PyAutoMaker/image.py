...
class imageUtil:
    def __init__(self):
        self.dll = windll.LoadLibrary("PythonWrapper.dll")
    ...
    def imageSearchEx_Parallel(self, src, temp, exceptColor = (255, 0, 0), rate = float(0.95)):
        ...
        """if rate == 0.0:
            result = wintypes.RECT()
            ret = self.__imageSearchEx_Parallel(byref(src_data), byref(temp_data), byref(result), exceptColor)
            result = (result.left, result.top, result.right, result.bottom)
        else:
        """
...
def templateMatching(src, temp, thresh):
    #src_gray = cv2.cvtColor(src, cv2.COLOR_BGRA2GRAY if src.shape[-1] == 4 else cv2.COLOR_BGR2GRAY)
    #temp_gray = cv2.cvtColor(temp, cv2.COLOR_BGRA2GRAY if src.shape[-1] == 4 else cv2.COLOR_BGR2GRAY)
    #w, h = temp.shape[::-1]
    h, w, _ = temp.shape
