...
class Html_table:
    ...
    def _table_body_row(self, keys, value):
        ...
        for i in keys:
            ...
            # Add link
            if i in ['@id', 'schema:name'] and record_type and record_id:
                content += self._table_body_cell_link(cell_value, path)
    ...
    def _table_body_cell_link(self, value, link):
        
        content = '<td><a href="{link}">{value}</a></td>'.format(value=value, link=link)
