...
def convert_all_v1_to_new_format(base_folder):
    # problems = ProblemService.read_all_problems_v1(base_folder)
    problems = ProblemService.read_all_problems_v1(base_folder, load_testcase=True, translations=["vi"])

    for folder, problem in problems:
        print(folder)
        base, folder_name = os.path.split(folder)

        ProblemService.save(problem, base + "_new", problem_folder=folder_name, overwrite=True)
        print(problem.name)
...
if __name__ == "__main__":
    ...
    # base_folder = "D:\\projects\\ucode\\weekly-algorithm-problems\\week10"
    # base_folder = "D:\\projects\\ucode\\weekly-algorithm-problems\\competitions\\hsg-hn-2020"
