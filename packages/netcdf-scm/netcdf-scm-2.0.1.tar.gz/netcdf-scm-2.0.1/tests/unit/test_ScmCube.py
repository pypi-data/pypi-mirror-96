...
class TestScmCube:
    ...
    def test_get_scm_timeseries(
        self, test_sftlf_cube, test_cube, kw_args, regions, lazy
    ):
    ...
    @pytest.mark.parametrize(
        "region,area,netcdf_scm_realm,exp_error",
        (
            ("World", 510 * 10 ** 12, "atmos", False),
            ("World", 50 * 10 ** 12, "atmos", True),
            ("World", 50 * 10 ** 12, "land", False),
            ("World", 80 * 10 ** 12, "atmos", True),
            ("World", 80 * 10 ** 12, "ocean", False),
            ("World", 80 * 10 ** 12, "ocnBgchem", False),
            ("World", 5000 * 10 ** 12, "atmos", True),
            ("World|Land", 150 * 10 ** 12, "atmos", False),
            ("World|Land", 10 * 10 ** 12, "atmos", True),
            ("World|Land", 500 * 10 ** 12, "atmos", True),
            ("World|Land", 150 * 10 ** 12, "land", False),
            ("World|Land", 10 * 10 ** 12, "land", True),
            ("World|Land", 500 * 10 ** 12, "land", True),
            ("World|Ocean", 350 * 10 ** 12, "atmos", False),
            ("World|Ocean", 50 * 10 ** 12, "atmos", True),
            ("World|Ocean", 900 * 10 ** 12, "atmos", True),
            ("World|Ocean", 350 * 10 ** 12, "ocean", False),
            ("World|Ocean", 50 * 10 ** 12, "ocean", True),
            ("World|Ocean", 900 * 10 ** 12, "ocean", True),
            ("World|Ocean", 900 * 10 ** 12, "ocnBgchem", True),
        ),
    )
    def test_sanity_check_area(
        self, test_cube, region, area, netcdf_scm_realm, exp_error
    ):
        mock_netcdf_scm_realm = netcdf_scm_realm
        with patch(
            "netcdf_scm.iris_cube_wrappers.ScmCube.netcdf_scm_realm",
            new_callable=PropertyMock,
        ) as mock_netcdf_scm_realm:
            mock_netcdf_scm_realm.return_value = netcdf_scm_realm

            if exp_error:
                area_with_units = area * unit_registry("m**2").to("km**2")
                error_msg = re.escape(
                    "{} area of {:.3f} million km^2 is clearly wrong. ".format(
                        region, area_with_units.magnitude / 10 ** 6
                    )
                    + "`self.info`: {'files': []}"
                )
                with pytest.raises(ValueError, match=error_msg):
                    test_cube._sanity_check_area(region, area)
            else:
                test_cube._sanity_check_area(region, area)
    ...
    @pytest.mark.parametrize(
        "kword_args, expected_type",
        (
            (
                {"cell_weights": "area-surface-fraction"},
                AreaSurfaceFractionWeightCalculator,
            ),
            ({"cell_weights": "area-only"}, AreaWeightCalculator,),
        ),
    )
    def test_get_weight_calculator(self, test_cube, kword_args, expected_type):
        res = test_cube._get_weight_calculator(**kword_args)
        assert isinstance(res, expected_type)

    def test_get_weight_calculator_cache(self, test_cube):
        res = test_cube._get_weight_calculator(cell_weights="area-surface-fraction")
        assert isinstance(res, AreaSurfaceFractionWeightCalculator)

        # hit it again, get same thing i.e. cache gave us back same object
        res_two = test_cube._get_weight_calculator(cell_weights="area-surface-fraction")
        assert res_two == res

        # change kwargs, get a different type i.e. the cache isn't always
        # returning the same thing
        res_three = test_cube._get_weight_calculator(cell_weights="area-only")
        assert isinstance(res_three, AreaWeightCalculator)
...
class TestCMIP6OutputCube(_CMIPCubeTester):
    ...
    @pytest.mark.parametrize(
        ...
        (
            (
                "mag-files-average-year-mid-year/CMIP6/C4MIP/BCC/BCC-CSM2-MR/1pctCO2-bgc/r1i1p1f1/Amon/tas/gn/v20190321/netcdf-scm_tas_Amon_BCC-CSM2-MR_1pctCO2-bgc_r1i1p1f1_gn_1850-2000.MAG",
                "CMIP6.C4MIP.BCC.BCC-CSM2-MR.1pctCO2-bgc.r1i1p1f1.Amon.tas.gn.v20190321",
            ),
            (
                "CMIP6/GeoMIP/MOHC/UKESM1-0-LL/G6solar/r1i1p1f2/Lmon/npp/gn/v20191031/npp_Lmon_UKESM1-0-LL_G6solar_r1i1p1f2_gn_205001-210012.nc",
                "CMIP6.GeoMIP.MOHC.UKESM1-0-LL.G6solar.r1i1p1f2.Lmon.npp.gn.v20191031",
            ),
        ),
    )
    def test_get_instance_id(self, fname, instance_id, test_cmip6_stitch_output):
        assert (
            self.tclass.get_instance_id(join(test_cmip6_stitch_output, fname))
            == instance_id
        )

    def test_get_instance_id_not_cmip6(self, test_data_marble_cmip5_dir):
        fname = "cmip5/piControl/Amon/tas/NorESM1-M/r1i1p1/tas_Amon_NorESM1-M_piControl_r1i1p1_070001-120012.nc"

        error_msg = (
            re.escape(
                "invalid instance_id: "
                "netcdf-scm.tests.test-data.marble-cmip5.cmip5.piControl.Amon.tas.NorESM1-M.r1i1p1 "
                "for filepath "
            )
            + r".*"
            + "tests/test-data/marble-cmip5/cmip5/piControl/Amon/tas/NorESM1-M/r1i1p1/tas_Amon_NorESM1-M_piControl_r1i1p1_070001-120012.nc"
        )
        with pytest.raises(ValueError, match=error_msg):
            self.tclass.get_instance_id(join(test_data_marble_cmip5_dir, fname))
