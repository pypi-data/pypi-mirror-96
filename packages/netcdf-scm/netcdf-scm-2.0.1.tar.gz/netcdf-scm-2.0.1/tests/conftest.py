...
def get_test_cube_lon(lons=None):
    if lons is None:
        lons = np.array([45, 135, 225, 315])
...
def get_test_cube_lat(lats=None):
    if lats is None:
...
def create_cube_hires(cube_cls):
    c = cube_cls()
    test_data = np.ma.masked_array(np.random.random((4, 181, 360)), mask=False,)
    ...
    c.cube = iris.cube.Cube(
        ...
        dim_coords_and_dims=[
            (get_test_cube_time(), 0),
            (get_test_cube_lat(np.arange(-90, 90 + 1)), 1),
            (get_test_cube_lon(np.arange(0, 360)), 2),
        ],
        units=unit.Unit("degC"),
        attributes=get_test_cube_attributes(),
    )
...
def test_all_cubes_hires(request):
    return create_cube_hires(request.param)
...
def create_sftlf_cube_hires(cube_cls):
    c = cube_cls()
    c._loaded_paths.append("test_sftlf.nc")

    test_data = np.ma.masked_array(
        np.random.randint(0, 100 + 1, size=(181, 360)), mask=False
    )
    c.cube = iris.cube.Cube(test_data)
    c.cube.standard_name = "land_area_fraction"

    c.cube.add_dim_coord(get_test_cube_lat(np.arange(-90, 90 + 1)), 0)
    c.cube.add_dim_coord(get_test_cube_lon(np.arange(0, 360)), 1)
