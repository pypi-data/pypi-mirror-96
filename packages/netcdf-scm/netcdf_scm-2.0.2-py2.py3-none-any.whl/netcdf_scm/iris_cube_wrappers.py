...
class ScmCube:  # pylint:disable=too-many-public-methods
    ...
    def lat_lon_shape(self):
        ...
        AssertionError
            ...
            https://gitlab.com/netcdf-scm/netcdf-scm/issues so we can address your use
            case).
        """
    ...
    def get_scm_timeseries(self, **kwargs):
        ...
        **kwargs
        ...
        -------
        :obj:`scmdata.ScmRun`
            :obj:`scmdata.ScmRun` instance with the data in the ``data`` attribute and
    ...
    def get_scm_timeseries_cubes(
        ...
        ------
        InvalidWeightsError
            No valid weights are found for the requested regions
        """
        scm_timeseries_weights = self.get_scm_timeseries_weights(**kwargs)
        if not scm_timeseries_weights:
            regions = kwargs.get("regions", DEFAULT_REGIONS)

            raise InvalidWeightsError(
                "No valid weights for regions: {}".format(regions)
            )

        def crunch_timeseries(region, weights):
            scm_cube, area = take_lat_lon_mean(self, weights)
    ...
    def _add_areas(self, timeseries_cubes, areas):
        for cube in timeseries_cubes.values():
            for region, area in areas.items():
                self._sanity_check_area(region, area)
    ...
    def _sanity_check_area(self, region, area):
        area_with_units = (area * unit_registry(self._area_weights_units)).to("km**2")
        area_million_km2 = area_with_units.magnitude / 10 ** 6
        raise_error = False

        if region == "World|Land" or (
            region == "World" and self.netcdf_scm_realm in ["land"]
        ):
            if not 30 < area_million_km2 < 300:
                raise_error = True

        elif region == "World|Ocean" or (
            region == "World" and self.netcdf_scm_realm in ["ocean", "ocnBgchem"]
        ):
            if not 70 < area_million_km2 < 700:
                raise_error = True

        elif region == "World":
            if not 100 < area_million_km2 < 1000:
                raise_error = True

        if raise_error:
            raise ValueError(
                "{} area of {:.3f} million km^2 is clearly wrong. `self.info`: {}".format(
                    region, area_million_km2, self.info
                )
            )
    ...
    def _add_metadata_to_region_timeseries_cube(self, scmcube, region, weight_kwargs):
        ...
        scmcube.cube.attributes["crunch_netcdf_scm_version"] = (
            "{} (see Nicholls et al. (2021, https://doi.org/10.1002/gdj3.113), "
            "more info at gitlab.com/netcdf-scm/netcdf-scm)".format(__version__)
        )
    ...
    def _get_lat_lon_slice(self):
        try:
            return next(self.cube.slices([self.lat_name, self.lon_name]))
        except ValueError:
            non_lat_lon_dims = self._get_non_lat_lon_dims()
            lat_lon_slice = next(self.cube.slices_over(non_lat_lon_dims))

            return lat_lon_slice
    ...
    def convert_scm_timeseries_cubes_to_openscmdata(
        ...
        """
        Convert dictionary of SCM timeseries cubes to an :obj:`scmdata.ScmRun`
        ...
        out_calendar : str
            Calendar to use for the time axis of the output

        Returns
        -------
        :obj:`scmdata.ScmRun`
            :obj:`scmdata.ScmRun` containing the data from the SCM timeseries cubes
        ...
        return self._add_metadata_to_scmrun(output, scm_timeseries_cubes)

    @staticmethod
    def _add_metadata_to_scmrun(scmdf, scm_timeseries_cubes):
        for i, scm_cube in enumerate(scm_timeseries_cubes.values()):
            for coord in scm_cube.cube.coords():
                ...
                if coord.long_name.startswith("land_fraction"):
                    ...
                    # have a stable solution for parameter handling in ScmRun yet so
...
class CMIP6OutputCube(_CMIPCube):
    ...
    @classmethod
    def get_instance_id(cls, filepath):
        """
        ...
        ----------
        filepath : str
            Full file path including directory structure

        Raises
        ------
        ValueError:
            If the filepath provided results in an instance id which is
            obviously incorrect
        ...
        str
            Instance ID
        """
        instance_id = "{mip_era}.{activity_id}.{institution_id}.{source_id}.{experiment_id}.{member_id}.{table_id}.{variable_id}.{grid_label}.{version}".format(
            **cls().process_path(os.path.dirname(filepath))
        )
        if not instance_id.startswith("CMIP6"):
            raise ValueError(
                "invalid instance_id: {} for filepath {}".format(instance_id, filepath)
            )
