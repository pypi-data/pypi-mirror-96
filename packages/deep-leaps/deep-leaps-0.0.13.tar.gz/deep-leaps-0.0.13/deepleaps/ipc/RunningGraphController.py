...
class RunnableModuleFactory(object):
    ...
    @staticmethod
    def set_global_variable(scripts):
        key, value = scripts.split('=')
        App.instance().set_variables('$'+key, value)

    @staticmethod
    def reload_module(scripts):
    ...
    def start(self):
        threading.Thread(target=self.server_socket_run, daemon=True).start()
    ...
    def sender(self):
        workspace = self.runnable_controller.config.get('save_path', '$directory_root', possible_none=False)
        saved_path = App.instance().nested_dir_path_parser(os.path.join(workspace, 'monitor.txt'))
    ...
    def module_parser(self, name):
        ...
        if '(' in name:
            ...
            if len(modules)==1:
                ...
                self.runnable_controller.all_runnable_module[name].required.update(sub_names)
                return [name]
            else:
...
class RunnableModuleController(SingletoneInstance):
    @classmethod
    def register(cls, *args, **kargs):
        if kargs.get('config',None) is None:
            kargs['config'] = App.instance().config.COMMAND_CONTROLLER
        return super().register(*args, **kargs)
    ...
    def __init__(self, config):
        ...
        RunnableModule.set_global_module(self)
        ...
        while True:
            try:
                ...
                if App.instance().config.App.DEBUG:
                    print('run_exception',traceback.format_exc())
                self.factory.end()
                raise e
