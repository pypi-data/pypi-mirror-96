...
import random as r
import pygame
__import__('os').system('pip install pyfiglet')
import pyfiglet
...
def imhappy():
    ...
    print("    _   _     ")
    print("      _       ")
#Makes a tkinter canvas in a window
def window(HEight, WIdth, backg, titlee):
    window = Tk()
    window.title(titlee)
    c = Canvas(window, height=HEight, width=WIdth, bg=backg)
    c.pack()
#Greeting
def hi():
...
def pleep():
    ...
    if ree == 5:
        ...
        BAckG = input('What background color? If it is two words, then put them together, and also do only lowercases.:')
        TItlee = input('What title?: ')
        window(hEIGHT, wIDTH, BAckG, TItlee)
    if ree == 6:
        hi()
    if ree == 7:
        mathypleep()
