...
class AccountStateBuilder(StateHeaderBuilder):
    ...
    def from_tuple(params):
        return AccountStateBuilder(*params)

    def as_tuple(self):
        return (self.address, self.addressHeight, self.publicKey, self.publicKeyHeight, self.accountType, self.format, self.supplementalPublicKeysMask, self.linkedPublicKey, self.nodePublicKey, self.vrfPublicKey, self.votingPublicKeys, self.importanceSnapshots, self.activityBuckets, self.balances)
