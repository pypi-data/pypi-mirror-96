...
class CogniteGeospatialClient:
    ...
    @api_exception_handler
    async def shape_file_save_async(
        self,
        file,
        layer: str,
    ...
    def shape_file_save(
        ...
        """
        run_func = partial(
            self.shape_file_save_async, file, layer, create_layer, cleanup_old_data, id_field, attributes
        )
        return self._run_sync(run_func, self.ingestion_timeout)
    ...
    def delete_layer(self, layer_name: str):
        ...
        """
        run_func = partial(self.delete_layer_async, layer_name)
        self._run_sync(run_func, self.timeout)

    async def delete_layer_async(self, layer_name: str):
        layer_dto = FeatureLayerNameDTO(name=layer_name)
        self.api.delete_layer(self.project, layer_dto)
