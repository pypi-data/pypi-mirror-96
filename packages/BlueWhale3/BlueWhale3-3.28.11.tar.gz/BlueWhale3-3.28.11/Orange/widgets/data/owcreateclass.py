...
class OWCreateClass(widget.OWWidget):
    ...
    def __init__(self):
        ...
        combo = gui.comboBox(
            self.controlArea, self, "attribute", label=__("box.column"),
        ...
        rules_box.addWidget(QLabel(__("box.instance")), 0, 3, 1, 2)
        self.update_rules()

        gui.lineEdit(
            self.controlArea, self, "class_name",
            label=__("lable_new_class"),
        ...
        gui.rubber(box)
        gui.button(box, self, i18n.t("common.btn.apply"), autoDefault=False, width=180,
                   callback=self.apply)
