...
def search_action(args):
    ...
    for backend in backends:
        table = []
        res = backend.search(args.query)
        for ids, name, system, size in res:
            if not args.category or args.category in system:
                table.append([ids, name, system, size])
                backend_map[ids] = (backend, name)
        if len(table) == 0:
    ...
    backend,name = backend_map.get(ID, (False, False))
    if not backend:
        print("ID not present, Please double check and if it's not a mistake fill a bug report!")
        sys.exit(0)
