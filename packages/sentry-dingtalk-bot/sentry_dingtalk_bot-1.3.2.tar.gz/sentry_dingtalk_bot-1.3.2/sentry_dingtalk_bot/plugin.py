...
class DingtalkForm(notify.NotificationConfigurationForm):
    ...
    custom_keyword = forms.CharField(
        label=_('Custom Keyword'),
        widget=forms.CharField(attrs={'placeholder': 'keywords'}),
        help_text=_('Enter custom keyword.'))
...
class DingtalkPlugin(notify.NotificationPlugin):
    ...
    def is_configured(self, project, **kwargs):
        return bool(self.get_option('webhookUrl', project))
    ...
    def get_webhook_urls(self, project):
        url = self.get_option('webhookUrl', project)
        if not url:
            return ''
        return url 
