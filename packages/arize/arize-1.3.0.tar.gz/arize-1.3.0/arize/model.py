...
class BaseRecord(ABC):
    ...
    def _get_label(self, name: str, value, score=None):
        ...
        if isinstance(score, float):
            return public__pb2.Label(
                score_categorical=public__pb2.ScoreCategorical(
                    categorical=val, score=self._convert_element(score)
                )
            )
...
class Prediction(BaseRecord):
    ...
    def validate_inputs(self):
        ...
        if self.prediction_score is not None and (
            not isinstance(self.prediction_score, float)
            or not isinstance(self.prediction_label, str)
        ):
            raise TypeError(
                f"scored models require a prediction_score (float) and prediction_label (str) but got prediction_score ({type(self.prediction_score)}) and prediction_label ({type(self.prediction_label)})"
            )
...
class BaseBulkRecord(BaseRecord, ABC):
    ...
    def _normalize_inputs(self):
        ...
        if isinstance(self.prediction_scores, pd.DataFrame):
...
class BulkPrediction(BaseBulkRecord):
    ...
    def _validate_scored_categorical(self):
        if self.prediction_scores is None:
            return
        if not isinstance(self.prediction_scores, pd.Series):
            raise TypeError(
                f"prediction_scores is type {type(self.prediction_scores)}, but expect type pd.Series."
            )
