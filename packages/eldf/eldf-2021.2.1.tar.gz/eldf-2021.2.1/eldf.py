...
def to_eldf(obj, indent = 4, level = 0, toplevel = True):
    ...
    def to_str(v, additional_prohibited_character = None, additional_prohibited_character2 = None):
        if type(v) == int:
            return str(v)
        if type(v) == float:
            return str(v).replace('e+', 'e')
        if type(v) == bool:
        ...
        if v == '' or v[0] in " \t['" or v[:2] == '. ' or v[-1] in " \t" or '‘' in v or '’' in v or ';' in v or "\n" in v or v in ('N', 'Н') \
                or (additional_prohibited_character and (additional_prohibited_character in v or \
                  (additional_prohibited_character2 and additional_prohibited_character2 in v))) or v[0].isdigit() or (v[0] == '-' and v[1:2].isdigit()): # ]
    ...
    if type(obj) == dict:
            ...
            if type(value) == list:
                ...
                for index2, subvalue in enumerate(value):
                    if type(subvalue) in (dict, list):
                        raise Error('sorry, but this object can not be represented in ELDF (element `' + str(subvalue) + '` of `' + str(value) + '`)', 0)
                    r += to_str(subvalue, ',')
                ...
                if len(value) == 0:
                    ...
                    r += indent * level * ' ' + '.' + to_eldf(value, indent, level+1, False)[indent * level + 1:]
                    if len(value) > 2 and index < len(obj)-1:
                        r += "\n"
            else:
        ...
        raise Error('sorry, but this object can not be represented in ELDF', 0)
...
def parse(s):
    ...
    if len(s) == 0:
        raise Error('empty ELDF is not allowed', 0)
    if s[0] == '[':
        assert(len(s) >= 2)
        if s[1] == ']':
...
def from_json(json_str):
    return to_eldf(json.loads(json_str))

def to_json(eldf_str):
    return json.dumps(parse(eldf_str), indent = 4)

def reparse(eldf_str):
    return to_eldf(parse(eldf_str))

def test_parse(eldf_str):
    parse(eldf_str)
