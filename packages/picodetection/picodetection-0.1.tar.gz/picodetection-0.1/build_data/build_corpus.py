...
import sys
import argparse
from tags.tag import tags as pico_tags
nlp = spacy.load('en_core_web_sm')

# with open(os.path.join('./tags/tag.json'), 'r') as fp:
...
class DataBuild(object):
    ...
    def read_file(self,raw_data_path):

        data = pd.read_csv(os.path.join(raw_data_path),sep='\t')
        data.fillna('')

        def choose_label_new_label():
            if('NEW_LABEL_STRING' in data.columns):
                if(str(data['NEW_LABEL_STRING'][idx]))!='nan':
        ...
        for idx in data.index:
            ...
            if(str(data['PMID'][idx])) not in data_dict:
                ...
                label_string = choose_label_new_label()
    ...
    def get_batch(self,ind,train_length):
        if(ind < train_length):
            batch = 'train'
        else:
            batch = 'test/gold'
    ...
    def build_data(self,data_dir):
        ...
        # with open(os.path.join('tag.json'), 'r') as fp:
        #     pico_tags = json.load(fp)
    ...
    def call_builds(self):
        parser = argparse.ArgumentParser()
        parser.add_argument("--data_dir", help="enter path for corpus")
        parser.add_argument("--raw_data_path", help="enter file name")
        args = parser.parse_args()
        self.read_file(args.raw_data_path)
        self.build_data(args.data_dir)


if __name__ == "__main__":
    # execute only if run as a script
    data_build = DataBuild()
    data_build.call_builds()
