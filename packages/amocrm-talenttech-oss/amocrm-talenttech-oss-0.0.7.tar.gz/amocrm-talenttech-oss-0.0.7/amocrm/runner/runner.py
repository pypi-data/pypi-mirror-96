...
def run_from_config(
    ...
    """
    Read airflow task parameters (environment variables) to run sequence of tasks
    :param file_dir:              ->path to file to read
    :param etl_name_no_version:   ->etl_name_no_version to generate s3 path
    :param execution_date:        ->execution_date to generate s3 path
    :param args_s3:               ->s3 attributes
    :param sql_credentials:       ->sql variable
    :param amocrm_secrets:        ->
    :return: Nothing
    """
    args_additional = {
...
class AmocrmRunner:
    ...
    def extract(self):
        ...
        if self.api_version == 4:
            from api.api_loader_amocrm_v4 import AmocrmApiLoader as ApiLoader
        elif self.api_version == 2:
            from api.api_loader_amocrm_v2 import AmocrmApiLoader as ApiLoader
    ...
    def load_to_db(self, db):
        ...
        if db == "vertica":
            from db.vertica_uploader import UploaderDB
        elif db == "ch":
            from db.clickhouse_uploader import UploaderDB
    ...
    def parse(self, db, table_parse):
        ...
        swap_inserter = SwapInserter(
            ...
            insert_table=table_parse["insert_table"],
            add_args={
                "database": database,
