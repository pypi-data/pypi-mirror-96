...
def merge_force(look_match, verbose=False):
    ...
    if len(xp.keys())==0 or len(yp.keys()) ==0:
        ...
        out['grd_ob_labeled'] = x["grd_ob_labeled"]
        out['grd_fo_labeled'] = x["grd_fo_labeled"]
        vxunmatched = 0
        if len(xp.keys()) >0:
        ...
        if nX2 > 0:
            ...
            for i in range(nX2):
                ...
                xfeats['labels_' + str(i + 1+nmatches)] = xtmp
                xlabeled[xtmp>0] = -1
                '''
        ...
        if nY2 > 0:
            ...
            for i in range(nY2):
                ...
                yfeats['labels_' + str(i + 1+nmatches)] = ytmp
        ...
        grd_ob_labeled = out["grd_ob"].copy()
        grd_ob_labeled.values[:] = xlabeled[:]
        out['grd_ob_labeled'] = grd_ob_labeled
        grd_fo_labeled = out["grd_fo"].copy()
        grd_fo_labeled.values[:] = ylabeled[:]
        out['grd_fo_labeled'] = grd_fo_labeled
        out['unmatched'] = {'X': vxunmatched, 'Xhat': fcunmatched}
        out['MergeForced'] = True
        out["grid"] = copy.deepcopy(look_match["grid"])
    return out
