...
def feature_props_ob(look,label):
    tmp = look['grd_ob_features']['labels_'+str(label)]
    out = feature_props(tmp,which_comps = ["centroid", "area", "axis"])
    return out


def feature_props_fo(look,label):
    tmp = look['grd_fo_features']['labels_'+str(label)]
    out = feature_props(tmp,which_comps = ["centroid", "area", "axis"])
    return out
