...
#以观测场为准，将预报场匹配到观测场
def bdFun(Id, OB, FC, p, const, N, show):
    #OB = X    #参数取值
    #FC = Xhat    #参数取值
    ...
    for j in range(1, len(OB) + 1):
        for k in range(1, len(FC) + 1):
            ...
            res_0 = censqdelta.censqdelta(x = OB['labels_%d'%j], 
                             y = FC['labels_%d'%k], N = N, const = const, p = p)
...
#以预报场为准，将观测场匹配到预报场
def foFun(Id, OB, FC, p, const, N, show):
    res = np.array(())
    for j in range(1, len(FC) + 1):
        for k in range(1, len(OB) + 1):
            #print("预报目标：%d  ,"%j ,  "观测目标：%d"%k)
            res_0 = censqdelta.censqdelta(x = FC['labels_%d'%j], 
                             y = OB['labels_%d'%k], N = N, const = const, p = p)
            res = np.append(res,res_0)
            if (show):
                print("Calculating Baddeley Delta Metric between forecast feature {0} and observed feature {1}\n".format(j, k))                
    return res
...
def deltammSqcen(x, p, max_delta, const, N, show):
    ...
    xdim = np.shape(x['X'])     #R:引用属性里面的维度(shape)。观测场和预报场的要素数量一致
    if (N == None):
        N = max(xdim)
        if (N % 2 == 0):
            N = N + 1
    ...
    Upsilon = bdFun(Id = ind, OB = X, FC = Xhat, p = p, const = const, N = N, show = show)
    Upsilon = Upsilon.reshape(len(X), len(Xhat))
    #一一对比预报场→观测场
    Upsilon_fo = foFun(Id = ind, OB = X, FC = Xhat, p = p, const = const, N = N, show = show)
    ...
    if (m > 1):
        ...
        tmp = bdFun(Id = ind, OB = X, FC = newobj, p = p, const = const, N = N, show = show)
    ...
    if (n > 1):
        ...
        for i in range(len(ind)):
            ...
            look.append(look_0)
        #look = set(look)    #去掉重复的匹配
        tmp = bdFun(Id = ind, OB = newobj, FC = Xhat, p = p, const = const, N = N, show = show)
    ...
    if (np.all(bigQ > max_delta)):
        ...
        Xhat_number = o_Ksi[:, 0]
        Xmatches_number = pd.DataFrame(np.array((X_number, Xhat_number)).T, columns = ['Ob', 'Fcst'])
        
        #(矩阵排列暂时存在问题)以预报场为准，匹配结果为：
        Xhat_number = np.arange(len(Xhat))
        X_number = o_Ksi[0, :]
        Xhatmatches_number = pd.DataFrame(np.array((Xhat_number, X_number)).T, columns = ['Fcst', 'Ob'])        
        ...
        if len(a_fcst) != len(X):
            ...
            unmatched_X = 'NULL'
        if len(a_obs) != len(Xhat) :
            unmatched_Xhat = set(np.arange(n)) - set(a_obs)    #Xhat未匹配的目标
            #print(unmatched_Xhat)    
        else :
            unmatched_Xhat = 'NULL'
        unmatched = {'X':unmatched_Xhat, 'Xhat':unmatched_X}
        
        matches = {'X': list(Xmatches_number['Fcst']), 'Xhat': list(Xmatches_number['Ob'])}
        #unmatched = pd.DataFrame(unmatched, columns = ["Forecast", "Observed"])
        merges = centmatch.restructuring(matches['X'], matches['Xhat'])
