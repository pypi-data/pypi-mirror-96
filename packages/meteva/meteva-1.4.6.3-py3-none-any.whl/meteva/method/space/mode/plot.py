...
def plot_values(look_ff, cmap ="rain_24h"):
    ...
    ob_array = look_ff["grd_ob"]
    fo_array = look_ff["grd_fo"]
    grid = look_ff["grid"]
    grd_ob = meteva.base.grid_data(grid,ob_array)
    grd_fo = meteva.base.grid_data(grid,fo_array)
...
def plot_ids(look_ff):
    ...
    #a = data_Xlabeled_tr
    #b = data_Ylabeled_tr
    meteva.base.tool.plot_tools.plot_2d_grid_list([grd_ob, grd_fo], cmap = "mode", vmax = np.max((np.max(grd_ob), np.max(grd_fo)))+1,ncol=2)
