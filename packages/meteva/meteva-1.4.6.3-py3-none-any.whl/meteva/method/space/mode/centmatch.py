...
from . import make_spatialVx, feature_finder, data_pre, feature_props
...
def centmatch(look_ff, criteria = 1, const = 14, areafac = 1, show = False):
    ...
    for i in range(m) :
        ...
        if (criteria != 3):
            ...
            Ytmp = {"m": out['grd_fo_features']['labels_%d'%(i+1)]}
            tmpy_i = feature_props.feature_props(grd_feature = Ytmp, which_comps = ["centroid","area"])
            tmpy_num = {'{}'.format(i):tmpy_i}
            tmpy.update(tmpy_num)
            Ay_i = math.sqrt(tmpy[str(i)]['area'])
            Ay = np.append(Ay, Ay_i)
        else :
            tmpy = feature_props.feature_props(grd_feature = Ytmp, which_comps = ["centroid"])
        ...
        for j in range(n):
            if (show):
                print(j)
            if (criteria != 3):
                Xtmp = {"m": out['grd_ob_features']['labels_%d'%(j+1)]}
                tmpx_j = feature_props.feature_props(grd_feature = Xtmp, which_comps = ["centroid","area"])
                tmpx_num = {'{}'.format(j):tmpx_j}
                tmpx.update(tmpx_num)
                Ax_j = math.sqrt(tmpx[str(j)]['area'])
                Ax = np.append(Ax, Ax_j)
            else:
                tmpx = feature_props.feature_props(grd_feature = Xtmp, which_comps = ["centroid"])
