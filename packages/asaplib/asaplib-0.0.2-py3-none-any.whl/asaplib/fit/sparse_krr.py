...
from .krr import KRRSparse

class SPARSE_KRR(RegressorBase):

    def __init__(self, kernel, jitter=10e-20, delta=None, sigma=None, sparse_mode="fps", n_sparse=None):
        """
        Parameters
        ----------
        jitter, delta, sigma: floats
        ...
        self.krr_sparse = KRRSparse(jitter, delta, sigma)
    ...
    def predict(self, X):
        ...
        kNM = self.k_transform.compute(X, self.desc_sbs)
        return self.krr_sparse.predict(kNM)
