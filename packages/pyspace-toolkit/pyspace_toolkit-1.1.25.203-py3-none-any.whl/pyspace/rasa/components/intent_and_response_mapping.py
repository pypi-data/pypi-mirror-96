...
class IntentAndResponseMapper(Component):
    ...
    def __init__(self, component_config: Dict[Text, Any] = None, response_dict=None) -> None:
        ...
        # file_importer = TrainingDataImporter.load_from_config('config-tr.yml', 'domain.yml', ['nlu/tr.md', 'stories.md'])
        # file_importer = TrainingDataImporter.load_from_config('config/v1.yml', 'domain/v1.yml', ['nlu/v1.md', 'story/v1.md'])

        domain = Domain.empty()
        domain = Domain.load('domain/v1.yml')
