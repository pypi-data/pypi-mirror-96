...
try:
    ...
    printIfShown("Matplotlib library not found -> ctrl+clic to select multiple points in graphs has been deactivated", SHOW_INFO)
...
class ScatterPlotItem(GraphicsObject):
    ...
    **Signals:**
    ...
    def getViewMask(self, pts):
        ...
        w = self.data['width']
        # pos_x = np.argwhere(np.logical_not(np.isfinite(pts[0])))
        # pos_y = np.argwhere(np.logical_not(np.isfinite(pts[1])))
        pts_x = pts[0]
        pts_y = pts[1]
        # pts_x[pos_x] = 0
        # pts_y[pos_y] = 0
        mask = ((pts_x + w > viewBounds.left()) &
                (pts_x - w < viewBounds.right()) &
                (pts_y + w > viewBounds.top()) &
                (pts_y - w < viewBounds.bottom())) ## remove out of view points
        # mask[pos_x] = 0
        # mask[pos_y] = 0
        return mask
    ...
    def mouseDragEvent(self, ev):
        if ev.modifiers() and QtCore.Qt.ControlModifier and has_matplotlib:
            ev.accept()
