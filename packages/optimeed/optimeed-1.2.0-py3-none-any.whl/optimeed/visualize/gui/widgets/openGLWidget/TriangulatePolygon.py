...
def InTriangle(a, b, c, p):
    ...
    L[0] = ((b[1] - c[1]) * (p[0] - c[0]) + (c[0] - b[0]) * (p[1] - c[1])) / (((b[1] - c[1]) * (a[0] - c[0]) + (c[0] - b[0]) * (a[1] - c[1])) + eps)
    L[1] = ((c[1] - a[1]) * (p[0] - c[0]) + (a[0] - c[0]) * (p[1] - c[1])) / (((b[1] - c[1]) * (a[0] - c[0]) + (c[0] - b[0]) * (a[1] - c[1])) + eps)
    L[2] = 1 - L[0] - L[1]
...
def reformatXYtoList(xList, yList):
    polyList = [None]*len(xList)
    for i in range(len(xList)):
        polyList[i] = [xList[i], yList[i]]
    return polyList


def meshPolygon(xList, yList):
    pListIn = reformatXYtoList(xList, yList)
    triList = []
    plist = pListIn[::-1] if IsClockwise(pListIn) else pListIn[:]
    while len(plist) >= 3:
        a = GetEar(plist)
        if not a:
            break
