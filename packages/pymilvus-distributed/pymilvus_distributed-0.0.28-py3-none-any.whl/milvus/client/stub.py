...
class Milvus:
    ...
    @check_connect
    def load_partitions(self, collection_name, partitions_names, timeout=30):
        """
        ...
        :param partitions_names: The specified partitions to load.
        :type  partitions_names: list[str]
        ...
        with self._connection() as handler:
            return handler.load_partitions(db_name="", collection_name=collection_name,
                                           partition_names=partitions_names, timeout=timeout)

    @check_connect
    def release_partitions(self, collection_name, partitions_name, timeout=30):
        """
        ...
        :param partitions_name: The specified partition to release.
        :type  partitions_name: str
        ...
        with self._connection() as handler:
            return handler.release_partitions(db_name="", collection_name=collection_name,
                                              partitions_name=partitions_name, timeout=timeout)
        ...
        :type  params: dict
            ...
            NSG:
                `{
                    "metric_type": "NSG",
                    "index_type": "L2",
                    "params": {"out_degree": 50, "candidate_pool_size": 50,
                                "search_length": 50, "knng": 10}
                }`
            ...
            NGT_PANNG:
                `{
                    "metric_type": "NGT_PANNG",
                    "index_type": "L2",
                    "params": {"edge_size": 20, "forcedly_pruned_edge_size": 40, "selectively_pruned_edge_size": 10}
                }`

            NGT_ONNG:
                `{
                    "metric_type": "NGT_ONNG",
                    "index_type": "L2",
                    "params": {"edge_size": 20, "outgoing_edge_size": 10, "incoming_edge_size": 40}
                }`
    ...
    def drop_index(self, collection_name, field_name, timeout=30):
        ...
        with self._connection() as handler:
            return handler.drop_index(collection_name=collection_name, field_name=field_name, timeout=timeout)
