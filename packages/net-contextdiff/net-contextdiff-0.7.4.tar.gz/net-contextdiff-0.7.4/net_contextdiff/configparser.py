...
class IndentedContextualConfigParser:
    ...
    def parse_file(self, filename, debug=False):
        ...
        for line in file:
            ...
            if debug:
                # print the line with a space at the start (to separate it from
                # the previous line) and don't print a newline, as the line
                # will already contain one
            ...
            if debug:
                print(">> context:", context_name or "(unknown)",
                      file=sys.stderr)
            ...
            if context_name:
                for command in self._commands[context_name]:
                    ...
                    if debug:
                            print(("=> matches" if match else "-> no match"),
                                  "/^" + str(command["text"]) + "$/",
                                  file=sys.stderr)
